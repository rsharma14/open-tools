...
Answer: A
Explanation: S3 Transfer Acceleration uses Amazon CloudFront’s globally distributed edge locations to accelerate uploads, making it ideal for large, distributed data sources.
Multipart uploads allow efficient, reliable, and parallel uploads of large files.
This approach minimizes operational complexity (no need to manage replication, EC2, or Snowball devices).
The sites have high-speed Internet, so direct upload is feasible and fast.
-END-
Question:2
...
Answer: C
Explanation: Amazon Athena allows you to run SQL queries directly on data stored in Amazon S3 without the need for data movement or additional infrastructure. It is serverless, requires no setup, and is ideal for ad-hoc querying with minimal operational overhead. This meets the requirement for simple, on-demand queries with the least changes to the existing architecture.
-END-
Question:3
...
Answer: A
Explanation: The aws:PrincipalOrgID global condition key in an S3 bucket policy restricts access to only principals (users, roles) that are part of the specified AWS Organization. This is the simplest and most operationally efficient way to limit access to all accounts in the organization, without managing individual user or account permissions.
-END-
Question:4
...
D. Create an Amazon AP| Gateway API with a private link to access the S3 endpoint.

Answer: A
Explanation: A gateway VPC endpoint for S3 enables private connectivity between EC2 instances and S3 without traversing the public internet. This provides secure, private access to S3 from within the VPC, meeting the requirement for no internet connectivity.
-END-
Question:5
...
B. Configure theApplication Load Balancer to direct a user to the server with the documents
C. Copy the data from both EBS volumes to Amazon EFS. Modify the application to save new documents to Amazon EFS
D. Configure the Application Load Balancer to send the request to both servers. Return each document from the correct server

Answer: C
Explanation: Amazon EFS (Elastic File System) provides a shared file system that can be mounted by multiple EC2 instances across different Availability Zones. By moving user documents to EFS, both EC2 instances can access the same set of documents, ensuring users always see all their documents regardless of which instance serves their request. This solves the data consistency issue and supports scalability and availability.
-END-
Question:6
...
Answer: B
Explanation: AWS Snowball Edge is designed for large-scale data migrations with minimal network bandwidth usage. By transferring data to the Snowball Edge device on-premises and shipping it to AWS, the company avoids saturating its network and can migrate 70 TB of data quickly and efficiently.
-END-
Question:7
...
C. Write the messages to Amazon Kinesis Data Streams with a single shard. Use an AWS Lambda function to preprocess messages and store them in Amazon DynamoDB. Configure theconsumer applications to read from DynamoDB to process the messages.
D. Publish the messages to an Amazon Simple Notification Service (Amazon SNS) topic with multiple Amazon Simple Queue Service (Amazon S0S) subscriptions. Configure the consumer applications to process the messages from the queues.

Answer: D
Explanation: Amazon SNS with multiple SQS subscriptions allows for scalable, decoupled message delivery to many consumers. Each consumer application can process messages from its own SQS queue, and the system can handle sudden spikes in message volume, meeting the scalability and decoupling requirements.
-END-
Question:8
A company is migrating a distributed application to AWS. The application serves variable workloads. The legacy platform consists of a primary server that coordinates jobs across multiple compute nodes. The company wants to modernize the application with a solution that maximizes resiliency and scalability. How should a solutions architect design the architecture to meet these requirements?

A. Configure an Amazon Simple Queue Service (Amazon SQS) queue as a destination for the jobs. Implement the compute nodes with Amazon EC2 instances that are managed in an Auto Scaling group. Configure EC2 Auto Scaling to use scheduled scaling.
B. Configure an Amazon Simple Queue Service (Amazon SQS),quéue-as-adestination for the jobs. Implement the compute nodes with Amazon EC2 instances that are managed in an Auto Scaling group. Configure EC2 Auto Scaling based on the size of the queue.
C. Implement the primary server and the compute nodes with Amazon EC2 instances that are managed in an Auto Scaling group. Configure AWS CloudTrail as a destination for the jobs. Configure EC2 Auto Scaling based on the load on the primary server.
D. Implement the primary server and the compute nodes with Amazon EC2 instances that are managed in an Auto Scaling group. Configure Amazon EventBridge (Amazon CloudWatch Events) as a destination for the jobs. Configure EC2 Auto Scaling based on the load on the compute nodes.

Answer: B
Explanation: Using Amazon SQS as a job queue and EC2 Auto Scaling based on queue size provides a resilient, scalable architecture. This decouples job coordination from compute resources, allowing the system to automatically scale and recover from failures, which modernizes the legacy design.
-END-
Question:9
...
Answer: B
Explanation: Amazon S3 File Gateway extends on-premises storage to the cloud, providing low-latency access to recent files. An S3 Lifecycle policy can automatically transition older files to S3 Glacier Deep Archive, optimizing storage costs and managing file lifecycles as data grows.
-END-
Question:10
A company is building an ecommerce web application on AWS. The application sends information about new orders to an Amazon AP| Gateway REST API to process. The company wants to ensure that orders are processed in the order that they are received. Which solution will meet these requirements?

A. Use an API Gateway integration to publish a message to an Amazon Simple Notification Service (Amazon SNS) topic when the application receives an order. Subscribe an AWS Lambda function to the topic to perform processing.
B. Use an API Gateway integration to send a message to an Amazon Simple Queue Service (Amazon SQS) FIFO queue when the application receives an order. Configure the SQS FIFO queue to invoke an AWS Lambda function for processing.
C. Use an API Gateway authorizer to block any requests while the application processes an order.
D. Use an AP| Gateway integration to send a message to an Amazon Simple Queue Service (Amazon SQS) standard queue when the application receives an order. Configure the SQS standard queue to invoke an AWS Lambda function for processing.

Answer: B
Explanation: An SQS FIFO queue preserves the order of messages, ensuring that orders are processed in the order received. Integrating API Gateway with an SQS FIFO queue and Lambda for processing meets the requirement for ordered, reliable processing of incoming orders.
-END-
Question:11
...
Answer: A
Explanation: AWS Secrets Manager securely stores and manages database credentials, and can automatically rotate them, reducing operational overhead and improving security compared to storing credentials locally or in files.
-END-
Question:12
...
Answer: A
Explanation: Amazon CloudFront can use both the S3 bucket (for static data) and the ALB (for dynamic data) as origins, improving performance and reducing latency globally. Route 53 can route traffic to the CloudFront distribution using the company’s domain name.
-END-
Question:13
...
Answer: A
Explanation: AWS Secrets Manager supports multi-Region secret replication and automatic rotation, making it the simplest and most operationally efficient way to manage and rotate RDS credentials across multiple Regions.
-END-
Question:14
...
Answer: C
Explanation: Amazon Aurora with Multi-AZ deployment and Aurora Replicas supports automatic scaling for unpredictable read workloads, high availability, and minimal downtime, which is ideal for applications with more reads than writes.
-END-
Question:15
...
Answer: C
Explanation: AWS Network Firewall is a managed service that provides traffic inspection and filtering for VPCs, replicating the functionality of an on-premises inspection server with minimal operational overhead.
-END-
Question:16
...
Answer: B
Explanation: Amazon QuickSight can connect to multiple data sources (S3 and RDS), create datasets, and publish dashboards. Access can be managed at the user and group level, allowing management to have full access and others to have limited access, meeting the requirements for visualization and access control.
-END-
Question:17
...
Answer: A
Explanation: Attaching an IAM role with S3 access permissions to the EC2 instances is the recommended and secure way to allow EC2 instances to access S3 buckets, following AWS best practices.
-END-
Question:18
...
Answer: A,B
Explanation: Configuring the S3 bucket to send notifications to an SQS queue (A) and having the Lambda function process messages from the SQS queue (B) creates a durable, event-driven, and stateless architecture for image processing. This ensures reliable and automatic processing of uploaded images.
-END-
Question:19
...
Answer: D
Explanation: A Gateway Load Balancer in the inspection VPC can forward all incoming traffic to the third-party firewall appliance for inspection, with minimal operational overhead and seamless integration with the web application.
-END-
Question:20
A company wants to improve its ability to clone large amounts of production data into a test environment in the same AWS Region. The data is stored in Amazon EC2 instances on Amazon Elastic Block Store (Amazon EBS) volumes. Modifications to the cloned data must not affect the production environment. The software that accesses this data requires consistently high 1/0 performance. A solutions architect needs to minimize the time that is required to clone the production data into the test environment. Which solution will meet these requirements?
...
Answer: D
Explanation: EBS fast snapshot restore allows for rapid creation of new EBS volumes from snapshots, minimizing the time required to clone production data into the test environment. This approach ensures high I/O performance and data isolation between production and test environments.
-END-
Question:21
...
Answer: D
Explanation: Hosting static content on S3 with CloudFront for global distribution, using API Gateway and Lambda for backend APIs, and DynamoDB for data storage provides a highly scalable, low-latency, and serverless architecture with minimal operational overhead.
-END-
Question:22
...
C. S3 Standard-Infrequent Access (S3 Standard-1A)
D. S3 One Zone-Infrequent Access (S3 One Zone-lA)

Answer: B
Explanation: S3 Intelligent-Tiering automatically moves data between frequent and infrequent access tiers based on usage patterns, providing cost savings while ensuring resilience to AZ loss and unpredictable access patterns.
-END-
Question:23
...
B. Create an S3 Lifecycle configuration to transition objects from S3 Standard to S3 Glacier Deep Archive after 1T month.
C. Create an S3 Lifecycle configuration to transition objects from S3 Standard to S3 Standard-Infrequent Access (S3 Standard-1A) after 1 month.
D. Create an S3 Lifecycle configuration to transition objects from S3 Standard to S3 One Zone-Infrequent Access (S3 One Zone-IA) after 1 month.

Answer: B
Explanation: Transitioning objects from S3 Standard to S3 Glacier Deep Archive after 1 month using a lifecycle policy is the most cost-effective way to store data that must be kept indefinitely but is rarely accessed after the first month.
-END-
Question:24
...
Answer: B
Explanation: AWS Cost Explorer provides granular filtering and visualization of EC2 costs by instance type, allowing for in-depth analysis and comparison over time with minimal operational overhead.
-END-
Question:25
...
Answer: D
Explanation: Using two Lambda functions with an SQS queue decouples data ingestion from database loading, improving scalability and reliability. This minimizes configuration effort and handles high data volumes efficiently.
-END-
Question:26
...
Answer: A
Explanation: AWS Config can continuously monitor and record S3 bucket configurations and changes, allowing you to detect unauthorized changes and maintain compliance with minimal operational effort.
-END-
Question:27
...
Answer: A
Explanation: CloudWatch dashboards can be shared externally via a shareable link, allowing users without AWS accounts to view dashboards securely and with least privilege, as required.
-END-
Question:28
...
B. Enable AWS Single Sign-On (AWS SS0) from the AWS SSO console. Create a two-way forest trust to connect the company's self-managed Microsoft Active Directory with AWS SSO by using AWS Directory Service for Microsoft Active Directory.
C. Use AWS Directory Service. Create a two-way trust relationship with the company's self-managed Microsoft Active Directory.
D. Deploy an identity provider (IdP) on premises. Enable AWS Single Sign-On (AWS SS0) from the AWS SSO console.

Answer: A
Explanation: AWS SSO can integrate with on-premises Microsoft Active Directory using AWS Directory Service and a one-way trust, enabling centralized user management and SSO across AWS accounts managed by AWS Organizations.
-END-
Question:29
...
B. Deploy an Application Load Balancer (ALB) and an associated target group. Associate the target group with the Auto Scaling group. Use the ALB as an AWS Global Accelerator endpoint in each RegionC. Deploy a Network Load Balancer (NLB) and an associated target group. Associate the target group with the Auto Scaling group. Create an Amazon Route 53 latency record that points to aliases for each NLB. Create an Amazon CloudFront distribution that uses the latency record as an origin.
D. Deploy an Application Load Balancer (ALB) and an associated target group. Associate the target group with the Auto Scaling group. Create an Amazon Route 53 weighted record that points to aliases for each ALB. Deploy an Amazon CloudFront distribution that uses the weighted record as an origin.

Answer: A
Explanation: AWS Global Accelerator with Network Load Balancer endpoints in each region provides low-latency routing and automated failover for UDP-based services, meeting the requirements for global, highly available VoIP deployments.
-END-
Question:30
...
Answer: C
Explanation: Creating a snapshot, terminating the DB instance, and restoring from the snapshot when needed is the most cost-effective solution for infrequent, resource-intensive workloads, as you only pay for the database when it is running.
-END-
Question:31
...
B. Use Cost Explorer todisplay resources that are not properly tagged. Tag those resources manually.
C. Write API calls to check all resources for proper tag allocation. Periodically run the code on an EC2 instance.
D. Write API calls to check all resources for proper tag allocation. Schedule an AWS Lambda function through Amazon CloudWatch to periodically run the code.

Answer: A
Explanation: AWS Config rules can automatically check for required tags on EC2, RDS, and Redshift resources, minimizing manual effort and ensuring compliance across all resources.
-END-
Question:32
...
Answer: B
Explanation: Hosting a static website in an S3 bucket is the most cost-effective solution for serving HTML, CSS, JavaScript, and images, as it requires no servers and incurs minimal costs.
-END-
Question:33
...
Answer: C
Explanation: Kinesis Data Streams can handle high-throughput, near-real-time data ingestion. Lambda can process and sanitize data before storing it in DynamoDB, and other applications can consume the data from the stream, meeting scalability and data privacy requirements.
-END-
Question:34
...
D. Use AWS CloudTrail to track configuration changes and Amazon CloudWatch to record AP calls.

Answer: B
Explanation: AWS Config tracks configuration changes, while AWS CloudTrail records API calls. Using both services together meets compliance, governance, auditing, and security requirements.
-END-
Question:35
...
Answer: D
Explanation: AWS Shield Advanced provides DDoS protection for Elastic Load Balancers and other AWS resources. Assigning the ELB to Shield Advanced ensures detection and mitigation of large-scale DDoS attacks.
-END-
Question:36
...
Answer: B
Explanation: AWS KMS multi-Region keys allow you to use the same key in multiple regions for encryption and decryption. Configuring S3 buckets in each region with replication and using the multi-Region KMS key meets the requirements for encryption and regional redundancy with minimal overhead.
-END-
Question:37
...
Answer: B
Explanation: AWS Systems Manager Session Manager allows secure, auditable, and repeatable remote access to EC2 instances without the need for SSH keys or bastion hosts, following AWS Well-Architected best practices.
-END-
Question:38
...
C. Add an Amazon CloudFront distribution in front of the $3 bucket. Edit/the'Routei53 entries to point to the CloudFront distribution.
D. Enable S3 Transfer Acceleration on the bucket. Edit the Route 53 entries to point to the new endpoint.

Answer: C
Explanation: Amazon CloudFront is a global CDN that caches content at edge locations, reducing latency for users worldwide. It is the most cost-effective way to accelerate a static S3-hosted website.
-END-
Question:39
...
A. Change the storage type to Provisioned I0PS SSD.
B. Change the DB instance to a memory optimized instance class.
C. Change the DB instance to a burstable performance instance class.
D. Enable Multi-AZ RDS read replicas with MySQL native asynchronous replication.

Answer: A
Explanation: Provisioned IOPS SSD storage provides higher and more consistent I/O performance for RDS databases, addressing slow insert operations caused by storage bottlenecks.
-END-
Question:40
...
Answer: A
Explanation: Kinesis Data Firehose can ingest and deliver large volumes of data to S3 with high availability and minimal management. S3 Lifecycle policies can automatically transition data to Glacier after 14 days, meeting retention and cost requirements.
-END-
Question:41
...
C. Create an Amazon EventBridge (Amazon CloudWatch Events) rule for each Saa$S source to send output data. Configure the S3 bucket as the rule's target. Create a second EventBridge (Cloud Watch Events) rule to send events when the upload to the S3 bucket is complete. Configure an Amazon Simple Notification Service (Amazon SNS) topic as the second rule's target.
D. Create a Docker container to use instead of an EC2 instance. Host the containerized application on Amazon Elastic Container Service (Amazon ECS). Configure Amazon CloudWatch Container Insights to send events to an Amazon Simple Notification Service (Amazon SNS) topic when the upload to the S3 bucket is complete.

Answer: B
Explanation: Amazon AppFlow can directly transfer data from SaaS sources to S3, eliminating the need for EC2-based ingestion. S3 event notifications to SNS provide a simple, serverless way to notify users when uploads are complete, improving performance and reducing operational overhead.
-END-
Question:42
...
Answer: C
Explanation: A gateway VPC endpoint for S3 allows EC2 instances in all subnets to access S3 directly without using a NAT gateway, avoiding regional data transfer charges and reducing costs.
-END-
Question:43
...
Answer: B
Explanation: AWS Direct Connect provides a dedicated, high-bandwidth, low-latency connection to AWS, allowing timely backups to S3 without impacting internet bandwidth for internal users.
-END-
Question:44
...
Answer: A,B
Explanation: Enabling versioning protects against accidental deletions by keeping previous versions of objects. MFA Delete adds an extra layer of security, requiring multi-factor authentication to permanently delete objects or versions.
-END-
Question:45
...
Answer: B,E
Explanation: Subscribing an SQS queue to the SNS topic (B) and modifying the Lambda function to read from the SQS queue (E) ensures reliable, durable message delivery and processing, even if network issues occur, preventing data loss.
-END-
Question:46
A company has an application that provides marketing services to stores. The services are based on previous purchases by store customers. The stores upload transaction data to the company through SFTP, and the data is processed and analyzed to generate new marketing offers. Some of the files can exceed 200 GB in size. Recently, the company discovered that some of the stores have uploaded files that contain personally identifiable information (PII) that should not have been included. The company wants administrators to be alerted if Pll is shared again. The company also wants to automate remediation. What should a solutions architect do to meet these requirements with the LEAST development effort?

A. Use an Amazon S3 bucket as a secure transfer point..Use Amazon Inspector-to-scanthe objects in the bucket. If objects contain Pll, trigger an S3 Lifecycle policy to remove the objects that contain PII.
B. Use an Amazon S3 bucket as a secure transfer point. Use Amazon Macie to scan the objects in the bucket. If objects contain PlI, use Amazon Simple Notification Service (Amazon SNS) to trigger a notification to the administrators to remove the objects that contain PlI.
C. Implement custom scanning algorithms in an AWS Lambda function. Trigger the function when objects are loaded into the bucket. If objects contain PII, use Amazon Simple Notification Service (Amazon SNS) to trigger a notification to the administrators to remove the objects that contain PII.
D. Implement custom scanning algorithms in an AWS Lambda function. Trigger the function when objects are loaded into the bucket. If objects contain PII, use Amazon Simple Email Service (Amazon SES) to trigger a notification to the administrators and trigger an S3 Lifecycle policy to remove the meats that contain PII.

Answer: B
Explanation: Amazon Macie automatically scans S3 buckets for sensitive data such as PII. It can trigger SNS notifications for administrators if PII is detected, providing automated alerting and remediation with minimal development effort.
-END-
Question:47
...
Answer: D
Explanation: On-Demand Capacity Reservations guarantee EC2 capacity in specific Availability Zones. Specifying the Region and all required AZs ensures the company has the needed capacity for the event.
-END-
Question:48
...
Answer: D
Explanation: Amazon EFS provides highly available, durable, and shared file storage for EC2 instances, making it suitable for storing a catalog that needs to be accessed by multiple servers.
-END-
Question:49
...
D. Store individual files in Amazon S3 Standard storage. Use S3 Lifecycle policies to move the files to S3 Glacier Deep Archive after 1 year. Store search metadata in Amazon RDS. Query the files from Amazon RDS. Retrieve th files from S3 Glacier Deep Archive.

Answer: B
Explanation: S3 Intelligent-Tiering provides fast access for files less than 1 year old. Lifecycle policies move older files to S3 Glacier Flexible Retrieval, which is cost-effective for infrequently accessed data, and S3 Glacier Select allows querying archived files.
-END-
Question:50
...
Answer: D
Explanation: AWS Systems Manager Run Command allows you to run custom commands across a fleet of EC2 instances quickly, making it ideal for urgent patching of third-party software at scale.
-END-
Question:51
...
Answer: B,D
Explanation: Amazon SES can format and send emails with HTML content (B). An EventBridge scheduled event can invoke a Lambda function (D) to query the API, format the report, and send it via SES, automating the daily reporting process.
-END-
Question:52
...
Answer: C
Explanation: Amazon EFS provides a scalable, highly available, and fully managed file system that supports standard file system access patterns, making it ideal for applications with large, variable-sized output files.
-END-
Question:53
...
Answer: C
Explanation: S3 Object Lock in compliance mode prevents deletion of records for a set period, even by administrators. Lifecycle policies can transition data to Glacier Deep Archive after 1 year, meeting retention and resiliency requirements.
-END-
Question:54
...
Answer: C
Explanation: Amazon FSx for Windows File Server provides highly available, durable, and fully managed Windows file shares with Active Directory integration, preserving existing access patterns and minimizing operational overhead.
-END-
Question:55
...
Answer: C
Explanation: Attaching a security group to the DB instances that allows inbound traffic only from the security group assigned to private subnet instances ensures that only those EC2 instances can access the database, meeting the security requirements.
-END-
Question:56
...
A. Create stage variables in AP| Gateway with Name="Endpoint-URL" and Value="Company Domain Name" to overwrite the default URL. Import the public certificate associated with the company's domain name into AWS Certificate Manager (ACM).
B. Create Route 53 DNS records with the company's domain name. Point the alias record to the Regional APl Gateway stage endpoint. Import the public certificate associated with the company's domain name into AWS Certificate Manager (ACM) in the us-east-1 Region.
C. Create a Regional API Gateway endpoint. Associate the API Gateway endpoint with the company's domain name. Import the public certificate associated with the company's domain name into AWS Certificate Manager (ACM) in the same Region. Attach the certificate to the API| Gateway endpoint. Configure Route 53 to route traffic to the APl Gateway endpoint.
D. Create a Regional API Gateway endpoint. Associate the API Gateway endpoint with the company's domain name. Import the public certificate associated with the company's domain name into AWS Certificate Manager (ACM) in the us-east-1 Region. Attach the certificate to the API Gateway APIs. Create Route 53 DNS records with the company's domain name. Point an A record to the company's domain name.

Answer: C
Explanation: Creating a Regional API Gateway endpoint, associating it with the company’s domain name, and attaching an ACM certificate in the same region allows secure HTTPS access. Route 53 can route traffic to the custom domain, meeting all requirements.
-END-
Question:57
...
Answer: B
Explanation: Amazon Rekognition provides pre-built moderation APIs to detect inappropriate content in images, minimizing development effort and allowing for human review of low-confidence results.
-END-
Question:58
...
Answer: C
Explanation: Amazon ECS on AWS Fargate is a serverless container platform that abstracts infrastructure management, allowing you to focus on application maintenance and scalability.
-END-
Question:59
...
Answer: D
Explanation: Kinesis Data Streams and Kinesis Data Firehose can ingest and process large volumes of clickstream data in real time, delivering it to S3 for analytics in Redshift, providing a scalable and efficient solution.
-END-
Question:60
...
Answer: C
Explanation: An ALB listener rule can redirect all HTTP requests to HTTPS, ensuring all website traffic is encrypted without changing the application or network ACLs.
-END-
Question:61
...
Answer: C
Explanation: Storing database credentials as a secret in AWS Secrets Manager with automatic rotation and granting the EC2 role access to the secret provides secure, automated credential management with minimal operational overhead.
-END-
Question:62
...
Answer: D
Explanation: When using an external CA, you must import the certificate into ACM and manually rotate it before expiration. EventBridge can notify you when the certificate is nearing expiration, ensuring timely manual rotation.
-END-
Question:63
...
Answer: A
Explanation: Saving .pdf files to S3 and using an S3 PUT event to trigger a Lambda function for conversion to .jpg format is scalable, cost-effective, and serverless, meeting the requirements for rapid growth and storage of both original and converted files.
-END-
Question:64
...
Answer: D
Explanation: Deploying Amazon FSx for Windows File Server on AWS and FSx File Gateway on-premises allows seamless, low-latency access to file data from both environments, minimizing changes to access patterns and operational overhead.
-END-
Question:65
...
Answer: C
Explanation: Amazon Textract can extract text from PDF and JPEG reports, and Amazon Comprehend Medical can identify PHI in the extracted text, providing an automated, low-overhead solution for PHI detection.
-END-
Question:66
A company has an application that generates a large number of files, each approximately 5 MB in size. The files are stored in Amazon S3. Company policy requires the files to be stored for 4 years before they can be deleted. Inmediate accessibility is always required as the files contain critical business data that is not easy to reproduce. The files are frequently accessed in the first 30 days of the object creation but are rarely accessed after the first 30 days. Which storage solution is MOST cost-effective?

A. Create an S3 bucket lifecycle policy to move files from S3 Standard to S3 Glacier 30 days from object creation. Delete the files 4 years after object creation.
B. Create an S3 bucket lifecycle policy to move files from S3 Standard to S3 One Zone-Infrequent Access (S3 One Zone-IA) 30 days from object creation. Delete the files 4 years after object creation.
C. Create an S3 bucket lifecycle policy to move files from S3 Standard to S3 Standard-Infrequent Access (S3 Standard-1A) 30 days from object creation. Delete the files 4 years after object creation.
D. Create an S3 bucket lifecycle policy to move files from S3 Standard to S3 Standard-Infrequent Access (S3 Standard-1A) 30 days from object creation. Move the files to S3 Glacier 4 years after object creation.

Answer: C
Explanation: Moving files from S3 Standard to S3 Standard-IA after 30 days and deleting after 4 years is cost-effective for data that is frequently accessed early and rarely accessed later, while ensuring immediate accessibility throughout the retention period.
-END-
Question:67
...
Answer: D
Explanation: Increasing the SQS visibility timeout ensures that messages are not processed more than once before being deleted, preventing duplicate records in the RDS table.
-END-
Question:68
...
Answer: A
Explanation: Provisioning AWS Direct Connect for primary connectivity and a VPN as backup provides highly available, low-latency hybrid connectivity, with cost-effective failover if the primary link fails.
-END-
Question:69
...
Answer: B
Explanation: Using an Auto Scaling group across multiple AZs and configuring Aurora PostgreSQL as Multi-AZ ensures high availability and minimal downtime for both the application and database with minimal operational effort.
-END-
Question:70
...
Answer: C
Explanation: Replacing the NLB with an ALB and enabling HTTP health checks allows the load balancer to detect application errors and trigger Auto Scaling to replace unhealthy instances, improving availability without custom scripts.
-END-
Question:71
...
Answer: B
Explanation: DynamoDB point-in-time recovery allows you to restore the table to any point within the last 35 days, meeting the RPO and RTO requirements for data recovery in case of corruption.
-END-
Question:72
...
Answer: D
Explanation: Deploying an S3 VPC gateway endpoint allows EC2 instances to access S3 directly within the AWS network, eliminating data transfer costs associated with NAT gateways or internet gateways.
-END-
Question:73
...
Answer: C,D
Explanation: The bastion host’s security group should allow inbound SSH only from the company’s external IP range (C). The application instances’ security group should allow inbound SSH only from the private IP of the bastion host (D), ensuring secure, controlled access.
-END-
Question:74
...
A. Configure the security group for the web tier toallow inbound traffic on port 443 from 0.0.0.0/0.
...
Answer: A,C
Explanation: The web tier must allow inbound HTTPS (443) from anywhere (A). The database tier should allow inbound SQL Server traffic (1433) only from the web tier’s security group (C), ensuring secure, tiered access.
-END-
Question:75
...
Answer: A
Explanation: Using API Gateway, Lambda, and SQS for inter-tier communication decouples application layers, prevents dropped transactions during overload, and provides a modern, operationally efficient architecture.
-END-
Question:76
...
Answer: B
Explanation: AWS DataSync over Direct Connect provides secure, reliable, and high-throughput data transfer from on-premises storage to S3, making it ideal for large, sensitive data sets.
-END-
Question:77
A company needs to configure a real-time data ingestion architecture for its application. The company needs an AP, a process that transforms data as the data is streamed, and a storage solution for the data. Which solution will meet these requirements with the LEAST operational overhead?
...
Answer: C
Explanation: API Gateway can ingest data, Kinesis Data Streams and Firehose can process and transform it with Lambda, and Firehose can deliver it to S3, providing a fully managed, real-time ingestion and storage pipeline with minimal overhead.
-END-
Question:78
...
Answer: B
Explanation: AWS Backup can automate backup schedules and retention for DynamoDB tables, ensuring compliance with long-term retention requirements with minimal operational effort.
-END-
Question:79
...
Answer: A
Explanation: DynamoDB on-demand capacity mode automatically scales to handle unpredictable workloads and incurs no charges when not in use, optimizing costs for variable traffic patterns.
-END-
Question:80
...
Answer: B
Explanation: Sharing an encrypted AMI requires modifying the launchPermission property and the KMS key policy to allow the MSP Partner’s AWS account to use the key, ensuring secure access to the AMI and encrypted snapshots.
-END-
Question:81
...
Answer: C
Explanation: Using an SQS queue for job storage and an Auto Scaling group based on the queue size ensures loose coupling, durability, and dynamic scaling of stateless processor nodes, following best practices for distributed, scalable architectures.
-END-
Question:82
...
Answer: D
Explanation: Amazon EventBridge can detect expiring ACM certificates and trigger a Lambda function to send custom SNS alerts, ensuring timely notification to the security team before expiration.
-END-
Question:83
...
Answer: C
Explanation: Amazon CloudFront can cache and accelerate content from on-premises servers for European users, reducing latency without moving the backend, and can be deployed immediately.
-END-
Question:84
A company wants to reduce the cost of its existing three-tier web architecture. The web, application, and database servers are running on Amazon EC2 instances for the development, test, and production environments. The EC2 instances average 30% CPU utilization during peak hours and 10% CPU utilization during non-peak hours. The production EC2 instances run 24 hours a day. The development and test EC2 instances run for at least 8 hours each day. The company plans to implement automation tostop the development and test EC2 instances when they are not in use. Which EC2 instance purchasing solution will meet the company's requirements MOST cost-effectively?
...
Answer: B
Explanation: Reserved Instances are most cost-effective for production workloads running 24/7, while On-Demand Instances are suitable for development and test environments that do not run continuously and can be stopped when not in use.
-END-
Question:85
...
Answer: A
Explanation: S3 Object Lock with versioning enforces write-once-read-many (WORM) protection, preventing modification or deletion of documents after upload, which is required for regulatory compliance.
-END-
Question:86
...
Answer: A
Explanation: Storing database credentials in AWS Secrets Manager and granting web servers IAM permissions to retrieve them allows for secure, frequent credential rotation and access control.
-END-
Question:87
...
Answer: D
Explanation: Storing customer data in an SQS FIFO queue during database upgrades ensures no data is lost. A separate Lambda function can process the queue and write to the database when it becomes available.
-END-
 Question:88
...
Answer: A
Explanation: Enabling Requester Pays on the S3 bucket ensures that the marketing firm pays for data transfer costs when accessing the data, minimizing costs for the data owner.
-END-
 Question:89
...
B. Enable multi-factor authentication (MFA) on the IAM user credentials for each audit team |AM user account.
C. Add an S3 Lifecycle policy to the audit team's IAM user accounts to deny the s3:DeleteObject action during audit dates.
D. Use AWS Key Management Service (AWS KMS) to encrypt the S3 bucket and restrict audit team |AM user accounts from accessing the KMS key.

Answer: A
Explanation: Enabling versioning and MFA Delete on the S3 bucket protects against accidental or unauthorized deletions, providing strong security for confidential audit documents.
-END-
 Question:90
...
Answer: B
Explanation: Creating a read replica and directing the reporting script to query the replica offloads read traffic from the primary database, improving performance for development tasks with minimal operational overhead.
-END-
 Question:91
...
Answer: A
Explanation: Configuring an S3 gateway endpoint allows EC2 instances to access S3 without traversing the internet, meeting security requirements for private connectivity.
-END-
 Question:92
...
Answer: A,C
Explanation: Configuring a VPC gateway endpoint for S3 (A) and a bucket policy that limits access to the application tier in the VPC (C) ensures secure, private access to sensitive S3 data from EC2 instances.
-END-
 Question:93
...
A. Use Amazon Aurora MySQL with Multi-AZ Aurora Replicas for production. Populate the staging database by implementing a backup and restore process that uses the mysgldump utility.
B. Use Amazon Aurora MySQL with Multi-AZ Aurora Replicas for production. Use database cloning to create the staging database on-demand.
C. Use Amazon RDS for MySQL with a Multi-AZ deployment and read replicas for production. Use the standby instance for the staging database.
D. Use Amazon RDS for MySQL with a Multi-AZ deployment and read replicas for production. Populate the staging database by implementing a backup and restore process that uses the mysgldump utility.

Answer: B
Explanation: Aurora MySQL with Multi-AZ replicas and database cloning allows the development team to create a staging database on demand without impacting production, eliminating latency issues during full exports.
-END-
 Question:94
...
Answer: C
Explanation: S3 event notifications to SQS and Lambda provide a serverless, scalable, and low-overhead solution for processing uploaded files and storing transformed data in DynamoDB.
-END-
 Question:95
...
Answer: D
Explanation: Creating read replicas with the same resources as the source database allows the application to separate read and write traffic, optimizing performance quickly and efficiently.
-END-
 Question:96
An Amazon EC2 administrator created the following policy associated with an 1AM group containing several users: { “Version”: “2012-10-17~", “Statement”: [ { “Effect”: “Allow”, “Action”: “ec2:TerminateInstances”, “Resource”: “W*7, “Condition”: { “IpAddress”: { “aws:SourceIp”: “10.100.100.0/24” “Effect”: “Deny”, “Action”: “ec2:*”, “Resource”: “W¥7, “Condition”: { “StringNotEquals”: { “ec2:Region”: “us-east-1” } What is the effect of this policy?

A. Users can terminate an EC2 instance in any AWS Region except us-east-1.
B. Users can terminate an EC2 instance with the IP address 10.100.100.7 in the us-east-1 Region.
C. Users can terminate an EC2 instance in the us-east-1 Region when the user's source IP is 10.100.100.254.
D. Users cannot terminate an EC2 instance in the us-east-1 Region when the user's source IP is 10.100.100.254.

Answer: B
Explanation: The policy allows users to terminate EC2 instances in us-east-1 only if their source IP is 10.100.100.7. The Deny statement blocks all EC2 actions outside us-east-1, and the Allow statement is limited to the specific IP and action.
-END-
 Question:97
...
Answer: D
Explanation: Amazon FSx for Windows File Server is a fully managed, highly available Windows file system that integrates with Active Directory, making it ideal for migrating Windows-based workloads like SharePoint.
-END-
 Question:98
...
Answer: B
Explanation: SQS FIFO queues ensure exactly-once message processing. Using a deduplication ID prevents Lambda from processing the same message multiple times, eliminating duplicate emails.
-END-
 Question:99
...
Answer: D
Explanation: Amazon FSx for Lustre is a fully managed file system that supports the Lustre protocol, meeting the requirements for shared storage with Lustre client access.
-END-
 Question:100
...
C. Create an AWS Key Management Service (AWS KMS) customer managed key. Allow the EC2 role touse the KMS key for encryption operations. Store the encrypted data on Amazon S3.
D. Create an AWS Key Management Service (AWS KMS) customer managed key. Allow the EC2 role to use the KMS key for encryption operations. Store the encrypted data on Amazon Elastic Block Store (Amazon EBS) volumes.

Answer: C
Explanation: Using AWS KMS for encryption and S3 for storage provides secure, highly available, and low-overhead management of encrypted certificates for containerized applications.
-END-
 Question:101
...
Answer: A
Explanation: Creating a NAT gateway in each public subnet and routing private subnet traffic through the NAT gateway in the same AZ provides high availability and enables private subnets to access the internet for updates, following AWS best practices.
-END-
 Question:102
...
B. Install an AWS DataSync agent in the on-premisés-data center.
C. Create a secondary Amazon Elastic Block Store (Amazon EBS) volume on the EC2 instance for the data.
D. Manually use an operating system copy command to push the data to the EC2 instance.
E. Use AWS DataSync to create a suitable location configuration for the on-premises SFTP server.

Answer: A,E
Explanation: Launching the EC2 instance in the same AZ as the EFS file system (A) ensures low-latency access. Using AWS DataSync to create a location configuration for the on-premises SFTP server (E) automates and accelerates the data transfer to EFS.
-END-
 Question:103
...
Answer: A
Explanation: AWS Glue job bookmarks track previously processed data, preventing reprocessing of old data and ensuring only new data is processed in each run.
-END-
 Question:104
...
Answer: A,C
Explanation: AWS Shield Advanced provides DDoS protection, and Amazon CloudFront distributes and caches content globally, absorbing attack traffic and improving availability for both static and dynamic content.
-END-
 Question:105
...
Answer: D
Explanation: The least privilege solution is to add a resource-based policy to the Lambda function with lambda:InvokeFunction as the action and events.amazonaws.com as the principal, allowing only EventBridge to invoke the function.
-END-
 Question:106
...
Answer: D
Explanation: Server-side encryption with AWS KMS keys (SSE-KMS) with automatic rotation provides encryption at rest, logs key usage for auditing, and automatically rotates keys annually, meeting compliance and operational efficiency requirements.
-END-
 Question:107
...
Answer: D
Explanation: Using API Gateway with Kinesis Data Analytics allows ingestion of real-time location data via REST API and supports analytics integration, providing a scalable, multi-tier architecture for tracking and analysis.
-END-
 Question:108
...
Answer: D
Explanation: Subscribing to RDS event notifications and using an SNS topic fanned out to multiple SQS queues allows Lambda functions to update each target system, providing scalable, decoupled integration.
-END-
 Question:109
...
Answer: D
Explanation: S3 Object Lock with versioning and legal holds allows objects to remain unchangeable until the legal hold is removed. Only users with s3:PutObjectLegalHold permission can remove the hold and delete objects, meeting the requirements for data retention and controlled deletion.
-END-
 Question:110
...
Answer: C,D
Explanation: Uploading images directly from the browser to S3 using presigned URLs (C) offloads work from the web server and improves performance. S3 Event Notifications can trigger a Lambda function to resize images after upload (D), reducing coupling and operational overhead.
-END-
 Question:111
...
Answer: D
Explanation: Using Amazon MQ with active/standby brokers across two AZs, an Auto Scaling group for consumers, and RDS for MySQL with Multi-AZ provides the highest availability and lowest operational complexity for the message processing system.
-END-
 Question:112
...
Answer: A
Explanation: AWS Fargate with ECS and Service Auto Scaling allows you to run containers without managing servers, providing scalability and minimal operational overhead. An ALB distributes incoming requests efficiently.
-END-
 Question:113
...
Answer: C
Explanation: Ordering a Snowball Edge Storage Optimized device allows for fast, offline transfer of 50 TB of data when no network bandwidth is available. AWS Glue can then be used for the transformation job in the cloud with minimal operational overhead.
-END-
 Question:114
...
D. Increase the number of EC2 instances to three. Use Provisioned I0PS SSD (i02) Amazon Elastic Block Store (Amazon EBS) volumes to store the photos and metadata.

Answer: C
Explanation: Using Lambda to process photos, S3 to store photos, and DynamoDB for metadata provides a scalable, serverless architecture that can handle variable user loads efficiently.
-END-
 Question:115
A medical records company is hosting an application on Amazon EC2 instances. The application processes customer data files that are stored on Amazon S3. The EC2 instances are hosted in public subnets. The EC2 instances access Amazon S3 over the internet, but they do not require any other network access. A new requirement mandates that the network traffic for file transfers take a private route and not be sent over the intermet. Which change to the network architecture should a solutions architect recommend to meet this requirement?
...
Answer: C
Explanation: Moving EC2 instances to private subnets and using a VPC endpoint for S3 ensures that file transfer traffic remains private and does not traverse the internet, meeting the new compliance requirement.
-END-
 Question:116
...
Answer: A,D
Explanation: Hosting the website as static content in an S3 bucket (D) eliminates server management and patching. Using CloudFront (A) provides HTTPS, scalability, and enhanced security with minimal operational overhead.
-END-
 Question:117
...
Answer: A
Explanation: A CloudWatch Logs subscription can stream logs directly to OpenSearch Service (Elasticsearch) in near-real time, providing a simple, low-overhead solution for log storage and search.
-END-
 Question:118
...
Answer: D
Explanation: Amazon S3 is highly scalable, cost-effective, and can handle 900 TB of text documents with variable demand, making it the best choice for this use case.
-END-
 Question:119
...
Answer: B
Explanation: AWS Firewall Manager allows you to centrally configure and manage AWS WAF rules across multiple accounts and regions, providing protection against SQL injection and XSS with minimal administrative effort.
-END-
 Question:120
...
Answer: B
Explanation: AWS Global Accelerator provides a single entry point and routes user traffic to the optimal endpoint group (NLBs in different regions), improving performance and availability for global users.
-END-
 Question:121
...
Answer: A
Explanation: To ensure encryption for the RDS instance and all future snapshots, you must restore from an encrypted copy of the latest snapshot and replace the existing DB instance with the encrypted one. This ensures all new snapshots are also encrypted.
-END-
 Question:122
...
Answer: B
Explanation: AWS KMS is a fully managed key management service that reduces operational burden by handling key creation, storage, rotation, and access control for encryption keys.
-END-
 Question:123
...
Answer: D
Explanation: Importing the SSL certificate into ACM and using an Application Load Balancer for SSL termination offloads encryption/decryption from the EC2 instances, improving performance and scalability.
-END-
 Question:124
...
Answer: A
Explanation: EC2 Spot Instances are ideal for stateless, interruptible, and cost-sensitive batch jobs, providing significant cost savings and scalability for workloads that can tolerate interruptions.
-END-
 Question:125
...
Answer: A,D
Explanation: Launching EC2 instances and RDS in private subnets (A) ensures they are not exposed to the public internet. Deploying an ALB in a public subnet and using NAT gateways in each AZ (D) provides high availability and internet access for payment processing.
-END-
 Question:126
...
D. Set up an S3 Lifecycle policy to transition objects to S3 One Zone-Infrequent Access (S3 One Zone-lA) immediately and to S3 Glacier Deep Archive after 2 years.

Answer: B
Explanation: Transitioning objects to S3 Glacier Deep Archive after 2 years using a lifecycle policy keeps recent data highly available and minimizes storage costs for long-term retention, meeting both availability and compliance requirements.
-END-
 Question:127
A media company is evaluating the possibility of moving its systems to the AWS Cloud. The company needs at least 10 TB of storage with the maximum possible 1/0 performance for video processing, 300 TB of very durable storage for storing media content, and 900 TB of storage to meet requirements for archival media that is not in use anymore. Which set of services should a solutions architect recommend to meet these requirements?
...
Answer: A
Explanation: Amazon EBS provides high-performance storage for video processing, S3 offers durable storage for media content, and S3 Glacier is ideal for cost-effective archival of infrequently accessed data.
-END-
 Question:128
...
B. Use Spot Instances in an Amazon Elastic Kuberetes Service (Amazon EKS) managed node group.
C. Use On-Demand Instances in an Amazon EC2 Auto Scaling group to run the application containers.
D. Use On-Demand Instances in an Amazon Elastic Kubernetes Service (Amazon EKS) managed node group.

Answer: A
Explanation: Using Spot Instances in an EC2 Auto Scaling group to run stateless containers minimizes cost and operational overhead, as the workload can tolerate interruptions and does not require persistent infrastructure.
-END-
 Question:129
...
Answer: A,E
Explanation: Migrating the PostgreSQL database to Aurora (A) and hosting the web application on AWS Fargate with ECS (E) reduces operational overhead, improves scalability, and eliminates the need for capacity planning and server management.
-END-
 Question:130
...
Answer: B
Explanation: A target tracking scaling policy automatically adjusts the Auto Scaling group to maintain the desired average CPU utilization, ensuring consistent application performance across all instances.
-END-
 Question:131
...
D. Create an origin access identity (OAI). Assign the OAl to the CloudFront distribution. Configure the S3 bucket permissions so that only the OAl has read permission.

Answer: D
Explanation: Creating an origin access identity (OAI) and assigning it to the CloudFront distribution, then granting the OAI read permission on the S3 bucket, ensures files are only accessible through CloudFront and not directly from S3 URLs.
-END-
 Question:132
...
Answer: A
Explanation: Amazon CloudFront with S3 provides global scalability, low latency, and cost-effective delivery of downloadable content, with minimal infrastructure provisioning.
-END-
 Question:133
...
Answer: C
Explanation: Amazon RDS Custom for Oracle allows OS-level access and supports disaster recovery with read replicas in another region, while minimizing operational overhead for upgrades and DR setup.
-END-
 Question:134
...
Answer: A
Explanation: S3 Cross-Region Replication with SSE-KMS (multi-Region keys) and Amazon Athena for querying provides a serverless, encrypted, and replicated solution with minimal operational overhead for analytics.
-END-
 Question:135
...
Answer: B
Explanation: AWS PrivateLink allows private, secure connectivity to a provider's service in another VPC, initiated only from the company's VPC, and restricts access to the target service as required by security.
-END-
 Question:136
...
Answer: A,C
Explanation: Creating an ongoing AWS DMS replication task (A) and a DMS replication server (C) enables continuous, real-time synchronization between the on-premises PostgreSQL database and Amazon Aurora PostgreSQL during migration.
-END-
 Question:137
...
Answer: B
Explanation: AWS Backup automates backup schedules and retention for DynamoDB tables, ensuring compliance with long-term retention requirements with minimal operational effort.
-END-
 Question:138
...
Answer: D
Explanation: Using S3 Intelligent-Tiering provides automatic cost optimization for data with unpredictable access patterns, while maintaining high durability and availability.
-END-
 Question:139
...
A. Create a Lambda function to copy the files to the analysis S3 bucket. Create an S3 event notification for the analysis S3 bucket. Configure Lambda and SageMaker Pipelines as destinations of the event notification. Configure s3:0bjectCreated:Put as the event type.
B. Create a Lambda function to copy the files to the analysis S3 bucket. Configure the analysis S3 bucket to send event notifications to Amazon EventBridge (Amazon CloudWatch Events). Configure an ObjectCreated rule in EventBridge (CloudWatch Events). Configure Lambda and SageMaker Pipelines as targets for the rule.
C. Configure S3 replication between the S3 buckets. Create an S3 event notification for the analysis S3 bucket. Configure Lambda and SageMaker Pipelines as destinations of the event notification. Configure s3:0bjectCreated:Put as the event type.
...
Answer: C
Explanation: Amazon S3 Transfer Acceleration speeds up uploads and downloads to S3 buckets from global locations by using Amazon CloudFront’s edge network, reducing latency for users worldwide.
-END-
 Question:140
...
Answer: B
Explanation: Amazon S3 Lifecycle policies can automatically transition objects to S3 Glacier Deep Archive after a specified period, minimizing storage costs for long-term retention while keeping recent data highly available.
-END-
 Question:141
...
Answer: A
Explanation: CloudFront globally caches static content and accelerates dynamic requests to the ALB origin over the edge network, giving lowest latency worldwide without multi‑Region complexity.
-END-
 Question:142
...
Answer: C
Explanation: AWS Global Accelerator provides static IPs, routes UDP traffic to nearest edge, and fronts a Network Load Balancer with EC2 Auto Scaling for low latency/high availability.
-END-
 Question:143
...
Answer: D
Explanation: ECS with an ALB lets you split the monolith into multiple containerized services, providing scalability and low operational overhead versus managing instances directly.
-END-
 Question:144
...
Answer: D
Explanation: Moving DB to Aurora improves performance; using an Auto Scaling group with a Spot Fleet behind an ALB scales the stateless web tier seamlessly and cost‑effectively.
-END-
 Question:146
...
Answer: B
Explanation: Reserve instances for steady baseline and use Spot for daytime spikes to minimize cost while preserving availability.
-END-
 Question:147
...
Answer: B
Explanation: Store logs in S3 and use a Lifecycle rule to transition objects older than 30 days directly to Glacier Deep Archive for cheapest long‑term retention.
-END-
 Question:148
...
Answer: D
Explanation: Configuring an SQS queue as Lambda failure destination guarantees failed SNS-triggered events are retained and later reprocessed, ensuring eventual processing.
-END-
 Question:149
...
Answer: A
Explanation: SQS FIFO preserves message order with minimal management; Lambda scales to process messages, maintaining ordering per message group.
-END-
 Question:150
...
Answer: A
Explanation: Composite alarms allow triggering only when multiple metric conditions (CPU and read IOPS) are met simultaneously, reducing false positives.
-END-
 Question:151
...
Answer: A,C
Explanation: Control Tower guardrails (A) and Organizations SCPs (C) enforce Region restrictions and prevent internet gateways, satisfying residency and no‑internet requirements.
-END-
 Question:152
...
C. Launch an Amazon EC2 instance. Create an |AM role that grants access to Amazon RDS. Attach the role to the EC2 instance. Configure a cron job to start and stop the EC2 instance on the desired schedule.
...
Answer: D
Explanation: EventBridge scheduled rules invoking Lambda start/stop functions automate RDS instance uptime window, minimizing cost and overhead.
-END-
 Question:153
...
Answer: D
Explanation: Lifecycle policy moving objects from S3 Standard to Standard‑IA after 90 days reduces storage cost while keeping older ringtones quickly accessible.
-END-
 Question:154
...
Answer: B
Explanation: S3 Object Lock compliance mode enforces WORM retention so no user (even root) can modify or delete objects for the defined period.
-END-
 Question:155
...
Answer: C
Explanation: CloudFront places cached content at edge locations worldwide; pairing S3 as origin provides global low‑latency secure delivery.
-END-
 Question:156
...
Answer: A,E
Explanation: Lake Formation blueprints + AWS Glue (E) ingest and structure data in Parquet in S3; Athena + QuickSight (A) handle one‑time SQL queries and KPI dashboards with minimal ops.
-END-
 Question:157
...
Answer: D,E
Explanation: CloudWatch Logs export (D) retains audit logs indefinitely; AWS Backup (E) manages backups with a 5‑year retention policy satisfying data storage/deletion requirements.
-END-
 Question:158
...
Answer: A
Explanation: CloudFront optimizes both live (real‑time) and on‑demand video streaming globally with caching and edge delivery.
-END-
 Question:159
...
Answer: A,C
Explanation: Usage plans with API keys throttle/identify legitimate consumers (A) and AWS WAF rules (C) block malicious bot traffic at the edge.
-END-
 Question:160
...
Answer: C
Explanation: S3 Standard offers millisecond access; Glacier is too slow, and databases/OpenSearch add unnecessary cost/complexity for small JSON dataset.
-END-
 Question:161
...
Answer: B
Explanation: S3 event triggers Lambda to run Python code serverlessly and store results in Aurora, providing high availability and minimal ops effort.
-END-
 Question:162
...
Answer: A
Explanation: FSx for Lustre links a high‑performance POSIX file system with S3 for persistent storage and fast parallel I/O suited to HPC Spot workflows.
-END-
 Question:163
...
Answer: A
Explanation: ECS Fargate with ECR images and target tracking auto scaling removes server management and scales containerized app automatically.
-END-
 Question:164
...
Answer: C
Explanation: SQS provides durable buffering with configurable retention and a dead‑letter queue for failed messages, minimizing operational work.
-END-
 Question:165
...
Answer: D
Explanation: CloudFront with an Origin Access Identity restricts direct S3 access and integrates AWS WAF on the distribution for inspection of all traffic.
-END-
 Question:166
...
Answer: D
Explanation: CloudFront as a CDN caches S3-hosted static pages at global edge locations for massive worldwide view counts efficiently.
-END-
 Question:167
...
Answer: C
Explanation: Reserved Instances handle steady baseline queue processing; Spot Instances cost‑effectively absorb unpredictable surges.
-END-
 Question:168
...
Answer: D
Explanation: An SCP at the root OU centrally enforces service/action restrictions across all accounts, scaling governance from a single control point.
-END-
 Question:169
...
Answer: C
Explanation: AWS Shield Advanced adds advanced DDoS detection/mitigation for ALB beyond standard protections, reducing risk of volumetric attacks.
-END-
 Question:170
...
Answer: C
Explanation: AWS WAF on the ALB with geo match restricts access to traffic originating from only the specified country.
-END-
 Question:171
...
A. Provide an APl hosted on an Amazon EC2 instance. The EC2 instance performs the required computations when the API request is made.
...
Answer: B
Explanation: API Gateway + Lambda scales automatically and elastically; Lambda performs tax computations on demand without server management.
-END-
 Question:172
...
Answer: C
Explanation: CloudFront field‑level encryption protects specific sensitive form fields end‑to‑end and restricts decryption to approved back‑end applications.
-END-
 Question:173
...
Answer: B
Explanation: CloudFront caches and distributes static S3 media globally, reducing origin load and cost compared to compute or memory caches.
-END-
 Question:174
...
Answer: B
Explanation: Distributing Auto Scaling instances across multiple Availability Zones provides high availability without changing application code.
-END-
 Question:175
...
C. Create a read replica for the database in a different AWS Region. Use query string parameters in AP| Gateway to route traffic to the read replica.
...
Answer: B
Explanation: RDS Proxy pools and manages connections, preventing exhaustion and reducing Lambda connection storms with minimal app changes.
-END-
 Question:176
...
Answer: A
Explanation: DynamoDB gateway VPC endpoint lets private subnets access DynamoDB without traversing the public internet, maintaining secure internal traffic.
-END-
 Question:177
...
Answer: B
Explanation: DAX adds an in‑memory cache layer requiring no application refactor (compatible API), improving read latency and offloading DynamoDB.
-END-
 Question:178
...
Answer: A
Explanation: AWS Backup automates and copies backups cross‑Region for EC2 and RDS with minimal management overhead.
-END-
 Question:179
...
Answer: A
Explanation: An IAM role attached to the EC2 instance grants least‑privilege read access and decrypt permission for the KMS key securing the SSM SecureString parameter.
-END-
 Question:180
A company is designing a cloud communications platform that is driven by APIs. The application is hosted on Amazon EC2 instances behind a Network Load Balancer (NLB). The company uses Amazon AP Gateway to provide external users with access to the application through APIs. The company wants to protect the platform against web exploits like SQL injection and also wants to detect and mitigate large, sophisticated DDoS attacks. Which combination of solutions provides the MOST protection? (Choose two.)

A. Use AWS WAF to protect the NLB.
B. Use AWS Shield Advanced with the NLB.
C. Use AWS WAF to protect Amazon AP Gateway.
D. Use Amazon GuardDuty with AWS Shield Standard
E. Use AWS Shield Standard with Amazon AP| Gateway.

Answer: B,C
Explanation: Shield Advanced protects NLB against sophisticated DDoS (B); WAF on API Gateway (C) blocks common web exploits at the API edge layer.
-END-
 Question:181
...
Answer: A
Explanation: SQS decouples producers/consumers, enabling horizontal scaling of ECS microservices without ordering constraints, reducing coupling.
-END-
 Question:182
...
Answer: B
Explanation: RDS MySQL Multi‑AZ synchronously replicates to a standby instance, ensuring durability and automatic failover with minimal data loss.
-END-
 Question:183
...
Answer: A
Explanation: S3 static hosting + API Gateway/Lambda dynamic tier + DynamoDB on‑demand automatically scales reads/writes with no server patching; CloudFront accelerates global delivery.
-END-
 Question:184
...
Answer: A
Explanation: Placing the Lambda function in the VPC with proper security groups lets it reach on‑prem resources via existing Direct Connect without new tunnels.
-END-
 Question:185
...
Answer: B
Explanation: Assigning an IAM role (taskRoleArn) with S3 permissions to the ECS task grants containers secure, scoped access to S3.
-END-
 Question:186
...
Answer: B
Explanation: FSx for Windows File Server provides a fully managed, multi‑AZ Windows shared file system accessible by multiple Windows instances.
-END-
 Question:187
...
Answer: A,D
Explanation: RDS Multi‑AZ ensures HA for the relational layer (A); ECS Fargate (D) handles container scaling without server management for the application tier.
-END-
 Question:188
...
C. Launch an Amazon EC2 instance in a private subnet in a VPInstruct the new partner to upload files to the EC2 instance by using a VPN. Run acron job script, on the EC2 instance to upload files to the S3 data lake.
...
Answer: A
Explanation: AWS Transfer Family offers a managed, highly available SFTP endpoint directly ingesting files into S3 with minimal ops overhead.
-END-
 Question:189
...
Answer: B,D
Explanation: Object Lock compliance mode (B) enforces WORM for 5 years; KMS customer managed keys with automatic rotation (D) provide annual key rotation and encryption at rest.
-END-
 Question:190
...
Answer: B
Explanation: Elastic Beanstalk manages Java/PHP platform, scaling and patching; URL swapping enables rapid feature testing with minimal operational burden.
-END-
 Question:191
...
A. Create aread replica. Move reporting queries to the read replica.
...
Answer: A
Explanation: Offloading long-running reporting queries to a read replica removes contention/timeouts on the primary handling order writes.
-END-
 Question:192
...
Answer: B,E
Explanation: Store extracted structured data in S3 and query with Athena (B); Textract + Comprehend Medical (E) automate text extraction and medical entity detection serverlessly for scale.
-END-
 Question:193
...
Answer: B
Explanation: Redis provides high‑performance in‑memory caching with replication for HA, reducing read load more efficiently than adding more read replicas.
-END-
 Question:194
...
Answer: A
Explanation: Clustering EC2 database nodes across separate AZs with replication provides high availability and automatic failover for a self‑managed database.
-END-
 Question:195
...
Answer: C
Explanation: Decoupling with SQS ensures orders persist during outages; EC2 consumers process from queue, eliminating lost messages and retries.
-END-
 Question:196
...
Answer: D
Explanation: DynamoDB TTL automatically expires (deletes) items after 30 days when a TTL attribute is set, requiring minimal application changes and cost.
-END-
 Question:197
...
Answer: B,E
Explanation: Rehosting .NET app on Elastic Beanstalk (B) minimizes changes; migrating Oracle to RDS Oracle Multi‑AZ with DMS (E) preserves database engine and adds HA.
-END-
 Question:198
...
Answer: D
Explanation: EKS with Fargate removes node management; Amazon DocumentDB offers MongoDB compatibility for existing code with minimal operational overhead.
-END-
 Question:199
...
D. Configure an Amazon Cognito user pool authorizer in APl Gateway to allow Amazon Cognito to validate each request.

Answer: D
Explanation: Cognito user pool authorizer natively validates JWT tokens per request, enforcing user access with least development and managed controls.
-END-
 Question:201
...
C. Use Amazon Simple Queue Service (Amazon SQS)to distribute the: SMSimessages. Use AWS Lambda to process the responses.
...
Answer: B
Explanation: Amazon Pinpoint supports two‑way SMS; events streamed to Kinesis for analysis/archival satisfies reply capture and 1‑year retention.
-END-
 Question:202
...
Answer: B
Explanation: KMS customer managed key with automatic rotation provides yearly key rotation and default bucket encryption with minimal ops.
-END-
 Question:203
...
Answer: D
Explanation: Auto Scaling workers scale on SQS queue depth ensuring invitations processed promptly; queue buffers spikes preventing delays.
-END-
 Question:204
...
Answer: C
Explanation: Lake Formation centralizes S3 + RDS (via Glue JDBC) with fine‑grained table/column permissions and low operational overhead.
-END-
 Question:205
...
Answer: C
Explanation: EventBridge rule directly matches CreateImage API call and publishes to SNS for alerting with least management.
-END-
 Question:207
...
Answer: D
Explanation: SQS decouples ingestion; Lambda consumes and writes to DynamoDB reducing direct write pressure and lost requests.
-END-
 Question:208
...
C. Run the nslookup tool from inside the EC2 instance to obtain the private IP address of the S3 bucket's service APl endpoint. Create a route in the VPC route table to provide the EC2 instance with access to the S3 bucket. Attach a resource policy to the S3 bucket to only allow the EC2 instance's IAM role for access.
D. Use the AWS provided, publicly available ip-ranges.json file to obtain the private IP address of the S3 bucket's service API endpoint. Create aroute in the VPC route table to provide the EC2 instance with access to the S3 bucket. Attach a resource policy to the S3 bucket to only allow the EC2 instance's IAM role for access.

Answer: B
Explanation: Gateway VPC endpoint routes S3 traffic privately; bucket policy restricts access to instance role only—no internet path.
-END-
 Question:209
...
Answer: A
Explanation: ElastiCache centralizes session storage enabling scale out across AZs; sticky sessions wouldn't share across instances.
-END-
 Question:210
...
Answer: D
Explanation: Backlog per instance metric gives right‑sized scaling for both queues without over/under provisioning.
-END-
 Question:211
...
Answer: D
Explanation: Resource Groups Tag Editor can globally query resources by tag quickly across services/Regions.
-END-
 Question:212
...
C. S3 Standard
D. S3 Standard-Infrequent Access (S3 Standard-1A)

Answer: A
Explanation: Intelligent‑Tiering auto moves objects between frequent/infrequent tiers for unpredictable access with instant retrieval.
-END-
 Question:213
...
Answer: A
Explanation: AWS WAF on ALB filters XSS/SQLi with managed rules minimal ops.
-END-
 Question:214
...
Answer: B
Explanation: Glue crawler + ETL job automatically discovers schema and converts CSV to Parquet with little custom code.
-END-
 Question:215
...
Answer: A
Explanation: 700 TB exceeds feasible 500 Mbps transfer window; Snowball devices plus lifecycle to Deep Archive minimize cost.
-END-
 Question:216
...
Answer: B
Explanation: Enable default encryption then Batch Operations copy (in place) encrypts existing objects; future uploads auto‑encrypted.
-END-
 Question:217
...
Answer: A
Explanation: Active‑passive with Route 53 failover and cross‑Region Aurora Replica meets ≤30‑min RTO/RPO at low standby cost.
-END-
 Question:218
...
Answer: A,E
Explanation: SG must allow inbound 443 from all (A); NACL needs inbound 443 and outbound ephemeral high ports (E) for return traffic.
-END-
 Question:219
...
Answer: D
Explanation: Memory‑optimized R5 improves performance; CloudWatch agent custom latency metrics support capacity planning.
-END-
 Question:220
A solutions architect is designing a new APl using Amazon AP| Gateway that will receive requests from users. The volume of requests is highly variable; several hours can pass without receiving a single request. The data processing will take place asynchronously, but should be completed within a few seconds after a request is made. Which compute service should the solutions architect have the APl invoke to deliver the requirements at the lowest cost?
...
Answer: B
Explanation: Lambda scales to zero during idle periods and executes asynchronous tasks within seconds cost‑effectively.
-END-
 Question:221
...
Answer: D
Explanation: S3 offers durable, concurrent access and low cost for long‑term (7‑year) log retention.
-END-
 Question:222
...
Answer: A
Explanation: Cross‑account IAM role with trust to vendor account lets vendor assume role without long‑lived user credentials.
-END-
 Question:223
...
Answer: A,D
Explanation: IRSA role gives pod DynamoDB permissions; VPC gateway endpoint keeps traffic inside AWS network without internet.
-END-
 Question:224
...
Answer: C,E
Explanation: Multi‑AZ deployment (E) plus Route 53 multivalue answer (C) returns random healthy instance IPs achieving HA and distribution.
-END-
 Question:225
...
Answer: A
Explanation: Kinesis Data Stream ingests at scale and delivers to S3 (via Kinesis Data Firehose integration) for durable petabyte storage; querying new and existing data with serverless Athena SQL on S3 provides low overhead versus provisioning Redshift.
-END-
 Question:226
...
Answer: A,E
Explanation: API Gateway + Kinesis Data Stream + Firehose (E) gives elastic ingestion and automatic delivery to S3; AWS Glue (A) provides serverless ETL on the raw S3 data with minimal operations.
-END-
 Question:227
...
Answer: B
Explanation: Versioning keeps previous object versions; lifecycle must explicitly delete previous versions or they accumulate beyond the current object deletion window.
-END-
 Question:228
...
Answer: C
Explanation: SQS decouples bursts, reducing concurrent DB connections; Lambda consumers batch writes ensuring no data loss and smoothing heavy traffic.
-END-
 Question:229
...
Answer: A
Explanation: Aurora Serverless v2 for MySQL auto‑scales capacity, improves performance/durability, and removes manual replication management.
-END-
 Question:230
...
Answer: C
Explanation: NAT Gateways in separate AZs provide managed, scalable, fault‑tolerant outbound access without instance maintenance.
-END-
 Question:231
...
Answer: B
Explanation: VPC peering enables private routing between VPCs in same account securely without exposing DB publicly.
-END-
 Question:232
...
Answer: C
Explanation: VPC Flow Logs + CloudWatch Logs metric filters + alarm notify operations when RDP/SSH flow entries appear, with no instance changes.
-END-
 Question:233
...
Answer: A,B
Explanation: Strong root password plus MFA greatly reduces compromise risk; avoid using root otherwise.
-END-
 Question:234
...
Answer: C
Explanation: Encrypt EBS and Aurora at rest with KMS; attach ACM TLS cert to ALB for in‑transit HTTPS encryption.
-END-
 Question:235
...
Answer: C
Explanation: SCT converts schema; DMS full load + CDC keeps Oracle and Aurora PostgreSQL in sync during phased app migrations.
-END-
 Question:236
...
Answer: D
Explanation: Elastic Beanstalk multi‑AZ for web/app adds HA with minimal code change; RDS Multi‑AZ for DB; S3 stores images scalably.
-END-
Question:237
...
Answer: A
Explanation: VPC peering provides non‑bottleneck private connectivity without single point of failure; simpler than VPN or Direct Connect VIF here.
-END-
Question:238
...
D. Use AWS Cost and Usage Reports to create a report with hourly granularity. Integrate the report data with Amazon Athena. Use Amazon EventBridge to schedule an Athena query. Configure an Amazon Simple Notification Service (Amazon SNS) topic to receive a notification when athreshold is exceeded.

Answer: C
Explanation: AWS Budgets monthly EC2 cost budget with SNS alerts is simplest, granular, and cost‑effective for per‑account thresholds.
-END-
Question:239
...
A. Create an Amazon AP| Gateway REST API. Configure the method to use the Lambda function. Enable IAM authentication on the API.
...
Answer: B
Explanation: Lambda Function URL offers direct HTTPS endpoint; selecting AWS_IAM provides signed IAM authentication with least infrastructure.
-END-
Question:240
...
Answer: D
Explanation: Hosting visualization in same Region and accessing via Direct Connect keeps large query result traffic inside AWS, sending only pages to on‑premises with lowest egress cost.
-END-
Question:241
...
Answer: C
Explanation: Cross‑Region read replica keeps data online in another Region with minimal admin overhead versus self‑managed EC2 cluster.
-END-
Question:242
...
Answer: C
Explanation: Multivalue routing returns multiple healthy IPs based on health checks, satisfying requirement.
-END-
Question:243
...
Answer: A
Explanation: File Gateway presents an SMB/NFS share caching S3 objects locally, giving low‑latency read access while enforcing S3 read‑only.
-END-
Question:244
...
Answer: C
Explanation: Aurora with read replica adds HA/scaling; AMI + ALB + Auto Scaling across AZs provides resilient, scalable web/app tiers.
-END-
Question:245
...
Answer: D
Explanation: Lowering max instance count for dev Auto Scaling group cuts cost while retaining minimum two targets for ALB high availability.
-END-
Question:246
...
Answer: D
Explanation: ALB must reside in public subnets; keeping EC2 in private subnets with routes via ALB fixes reachability without exposing instances.
-END-
Question:247
...
Answer: A,E
Explanation: Read replicas require automated backups (retention >0) and binary logging enabled on source to replicate changes.
-END-
Question:248
A company runs analytics software on Amazon EC2 instances. The software accepts job requests from users to process data that has been uploaded to Amazon S3. Users report that some submitted data is not being processed Amazon CloudWatch reveals that the EC2 instances have aconsistent CPU utilization at or near 100%. The company wants to improve system performance and scale the system based on user load. What should a solutions architect do to meet these requirements?
...
Answer: D
Explanation: SQS buffers jobs; Auto Scaling on queue depth adds EC2 workers only as needed, eliminating CPU saturation.
-END-
Question:249
...
C. Create an Amazon EC2 Windows instance. Install:and configure a-Windows file-sharelrole on the instance. Connect the application server to the file share.
...
Answer: D
Explanation: FSx for Windows File Server is fully managed SMB storage meeting Windows share requirements.
-END-
Question:250
...
D. Use Amazon S3 as the target. Enable an S3 Lifecycle policy to transition the logs to S3 Standard-Infrequent Access (S3 Standard-1A) after 90 days.

Answer: D
Explanation: S3 target stores flow logs cheaply; lifecycle transitions to Standard‑IA after 90 days optimizing cost while keeping access.
-END-
Question:251
...
Answer: B
Explanation: NAT Gateway in public subnet plus route enables private instance outbound internet without inbound exposure.
-END-
Question:252
...
Answer: A
Explanation: EFS provides shared, elastic, multi‑AZ file system mountable by many EC2 instances with built‑in redundancy.
-END-
Question:253
A solutions architect has created two IAM policies: Policy1 and Policy2. Both policies are attached to an IAM group. Policy 1 { "Version": "2012-10-17", "Statement": [ { "Effect": "Allow", "Action": [ "iam:Get*", “iam:List*", "kms:List*", “ec2:*", *daz **, "logs:Get*", "logs:Describe*" ]I "Resource™: "*" Policy 2 { “Version": "2012-10-17", "Statement": [ { "Effect™: "Deny", "Action": "ds:Delete*", “Resource®: "*" A cloud engineer is added as an IAM user to the IAM group. Which action will the cloud engineer be able to perform?
...
Answer: C
Explanation: Allow ec2:* grants delete; deny applies only to ds:Delete* so EC2 instance deletion remains permitted.
-END-
Question:254
...
Answer: B
Explanation: Referencing security group IDs confines traffic strictly to intended tiers, enforcing least privilege.
-END-
Question:255
...
Answer: D
Explanation: SQS FIFO ensures ordered, deduplicated processing; payment reads single message preventing duplicate order creation.
-END-
Question:256
...
Answer: B,D
Explanation: Versioning retains all versions; MFA Delete protects against accidental/unauthorized deletions.
-END-
Question:257
...
Answer: A
Explanation: CloudWatch metric stream to Firehose -> S3 is serverless, near real‑time, and independent of instance launch latency.
-END-
Question:258
...
Answer: D
Explanation: AWS Glue ETL jobs handle large 1 GB conversions efficiently; Lambda triggers Glue per S3 PUT with low operations.
-END-
Question:259
...
Answer: A
Explanation: AWS Backup daily plan with 2‑year retention centrally manages consistent RDS backups and lifecycle.
-END-
Question:260
A company’s compliance team needs to move its file shares to AWS. The shares run on a Windows Server SMB file share. A self-managed onpremises Active Directory controls access to the files and folders. The company wants to use Amazon FSx for Windows File Server as part of the solution. The company must ensure that the on-premises Active Directory groups restrict access to the FSx for Windows File Server SMB compliance shares, folders, and files after the move to AWS. The company has created an FSx for Windows File Server file system. Which solution will meet these requirements?
...
Answer: D
Explanation: Joining FSx file system to on‑prem Active Directory enforces existing AD group ACLs on SMB shares.
-END-
Question:261
...
Answer: A,C
Explanation: CloudFront caches multiple variants; Lambda@Edge inspects User-Agent to select device‑specific objects.
-END-
Question:262
...
Answer: A
Explanation: VPC peering with appropriate routes and ElastiCache SG inbound from app CIDR/SG enables low‑cost connectivity.
-END-
Question:263
...
Answer: A,D
Explanation: ECS cluster + Fargate service (>=2 tasks) delivers HA containers with minimal infrastructure management.
-END-
Question:264
...
Answer: D
Explanation: ALB health checks remove unhealthy instances; Route 53 points to ALB ensuring only healthy backends served.
-END-
Question:265
...
Answer: C
Explanation: Public ALB fronts private EC2; CloudFront serves HTTPS at edge using ALB origin improving latency and security isolation.
-END-
Question:266
...
Answer: A
Explanation: Global Accelerator monitors endpoint health and directs users to closest healthy regional ALB minimizing latency and unfair advantages.
-END-
Question:267
...
Answer: C
Explanation: Firehose encrypts, transforms (Parquet), and delivers streaming app data to S3 with minimal ops; data in S3 is ready for further processing.
-END-
Question:268
...
Answer: A
Explanation: ElastiCache offloads frequent read queries, improving response times without major architectural change.
-END-
Question:269
...
Answer: C
Explanation: Read replica absorbs read‑only analyst queries, preserving performance of primary with minimal application changes.
-END-
Question:270
...
Answer: C
Explanation: Bucket policy enforcing SSE-S3 ensures at-rest encryption before upload; TLS provides in‑transit encryption.
-END-
Question:271
...
Answer: C
Explanation: Scheduled scaling pre‑warms capacity to desired level just before nightly batch start and scales down afterward cost‑effectively.
-END-
Question:272
...
C. Create an Amazon AP| Gateway API that is integrated with the ALB. Configure the API to use the HTTP integration type. Set up an API Gateway stage to enable the API cache based on the Accept-Language request header.
...
Answer: B
Explanation: CloudFront distribution with ALB origin caches responses near users; caching by Accept-Language header serves localized content efficiently.
-END-
Question:273
...
Answer: B
Explanation: Aurora Global Database warm standby keeps near real‑time replication with pre‑provisioned but scaled‑down infra yielding lowest RTO.
-END-
Question:274
...
Answer: B
Explanation: AMIs copied cross‑Region plus CloudFormation automation enable pilot light style rapid recovery under 4h with minimal steady resources.
-END-
Question:275
...
Answer: A
Explanation: Scheduled action raises desired capacity before users arrive eliminating morning cold start while keeping lower cost overnight.
-END-
Question:276
...
Answer: A,D
Explanation: RDS storage Auto Scaling prevents out‑of‑space; CPU metric scaling adds EC2 instances as load grows.
-END-
Question:277
...
Answer: D
Explanation: S3 stores large media cheaply; temporary EBS volumes for processing avoid costly always‑mounted EFS Standard.
-END-
Question:278
...
Answer: B,E
Explanation: DynamoDB supports hierarchical data with low latency; Macie + EventBridge + SNS monthly notifications detect and alert on sensitive financial data.
-END-
Question:279
...
Answer: A
Explanation: AWS Backup plan automates monthly DynamoDB backups, lifecycle transition to cold storage at 6 months, and retention 7 years with minimal custom scripting.
-END-
Question:280
...
Answer: B
Explanation: Athena runs SQL directly on CloudFront log objects in S3; QuickSight visualizes query results with minimal setup.
-END-
Question:281
...
Answer: D
Explanation: DMS CDC tasks stream changes with sub‑second RPO; Multi‑AZ or read replicas do not guarantee <1s RPO for all writes.
-END-
Question:282
...
Answer: B
Explanation: Restrict EC2 SG to allow only ALB SG inbound, blocking all other sources inside/outside private subnets.
-END-
Question:283
...
Answer: D
Explanation: FSx for NetApp ONTAP supports both NFS and SMB on one shared file system, eliminating duplication without code changes.
-END-
Question:284
...
Answer: B
Explanation: Cost Explorer provides user-level billed item breakdowns and exportable reports efficiently versus ad hoc Athena.
-END-
Question:285
...
Answer: B
Explanation: API Gateway + Lambda for low traffic adds dynamic form processing; SES sends messages—lowest cost versus provisioning servers.
-END-
Question:286
...
Answer: C
Explanation: CloudFront caches need invalidation to reflect new S3 website content after deployments.
-END-
Question:287
...
D. Host the application tier and the business tier on Amazon EC2 instances. Host the database tier on Amazon RDS. Use a Provisioned I0PS SSD (i02) Amazon Elastic Block Store (Amazon EBS) volume for file sharing between the tiers.

Answer: B
Explanation: EC2 for app + business tiers and FSx for Windows File Server shares; SQL Server stays on EC2 to use native features not available in RDS.
-END-
Question:288
...
C. Create an Amazon Elastic File System (Amazon EFS) file system. Mountithe EFS file system on all web servers.
...
Answer: C
Explanation: EFS provides shared POSIX file system mountable across Linux web servers with no app changes.
-END-
Question:289
...
Answer: B
Explanation: Lambda execution role with least‑privilege S3 read policy is most secure; avoid bucket‑wide grants or embedded credentials.
-END-
Question:290
...
Answer: C
Explanation: Mix On-Demand for baseline plus Spot for surplus traffic maximizes cost savings without long-term commitment.
-END-
Question:291
...
Answer: A,B
Explanation: Signed URLs work for clients lacking cookie support or hardcoded paths; signed cookies cover standard browsers—least user impact.
-END-
Question:292
...
Answer: A,B
Explanation: Kinesis or MSK for streaming, Kinesis Data Analytics or Glue for transform, Firehose/Glue write to S3, Athena queries Parquet with SQL.
-END-
Question:293
...
Answer: D
Explanation: Stored volume gateway keeps primary data locally while asynchronously backing up to S3—provides local access and secure automated transfer.
-END-
Question:294
...
B. Set up a gateway VPC endpoint for Amazon S3 in.the VRC.
C. Configure the EC2 instances to use a NAT gateway to access the S3 bucket.
D. Establish an AWS Site-to-Site VPN connection between the VPC and the S3 bucket.

Answer: B
Explanation: S3 gateway VPC endpoint routes traffic privately within AWS without internet traversal.
-END-
Question:295
An ecommerce company stores terabytes of customer data in the AWS Cloud. The data contains personally identifiable information (PIl). The company wants to use the data in three applications. Only one of the applications needs to process the Pll. The PIl must be removed before the other two applications process the data. Which solution will meet these requirements with the LEAST operational overhead?
...
Answer: B
Explanation: S3 Object Lambda can filter/redact PII per request so only one app gets raw data; others receive transformed results without extra datasets.
-END-
Question:296
A development team has launched a new application that is hosted on Amazon EC2 instances inside a development VPC. A solutions architect needs to create a new VPC in the same account. The new VPC will be peered with the development VPC. The VPC CIDR block for the development VPC is 192.168.0.0/24. The solutions architect needs to create a CIDR block for the new VPC. The CIDR block must be valid for a VPC peering connection to the development VPC. What is the SMALLEST CIDR block that meets these requirements? A.10.0.1.0/32

B. 192.168.0.0/24 C.192.168.1.0/32 D.10.0.1.0/24

Answer: D
Explanation: 10.0.1.0/24 is smallest valid non‑overlapping CIDR (must be /16-/28) distinct from 192.168.0.0/24 for peering; /32 invalid for VPC.
-END-
Question:297
A company deploys an application on five Amazon EC2 instances. An Application Load Balancer (ALB) distributes traffic to the instances by using atarget group. The average CPU usage on each of the instances is below 10% most of the time, with occasional surges to 65%. A solutions architect needs to implement a solution to automate the scalability of the application. The solution must optimize the cost of the architecture and must ensure that the application has enough CPU resources when surges occur. Which solution will meet these requirements?
...
Answer: B
Explanation: Auto Scaling group with target tracking on average CPU optimizes cost and scales between 2–6 instances automatically.
-END-
Question:298
...
Answer: C
Explanation: Separate subnets in each AZ, distribute EC2 via Auto Scaling, and enable RDS Multi‑AZ for HA of database tier.
-END-
Question:299
...
Answer: B
Explanation: FSx for Lustre persistent SSD delivers sub‑ms latency and >6 GB/s throughput; integrates with S3 for import/export of 8 TB dataset.
-END-
Question:300
...
Answer: C
Explanation: 24x7 steady workload best served by EC2 Reserved Instances; Aurora Reserved Instances offer cost‑effective, scalable managed database layer for growing storage.
-END-
Question:301
...
Answer: C
Explanation: AWS DataSync maximizes transfer performance with built‑in bandwidth throttling controls; 30 TB in 5 days feasible over managed 1 Gbps link.
-END-
Question:302
...
Answer: A,C
Explanation: CloudFront caches/accelerates global mobile delivery; Elastic Transcoder converts raw clips to adaptive streaming formats reducing size and buffering.
-END-
Question:303
...
Answer: D
Explanation: Application Auto Scaling with target tracking on ECS Fargate tasks adjusts capacity automatically optimizing cost.
-END-
Question:304
...
Answer: A
Explanation: DataSync automates periodic large NFS transfers cross‑Region with minimal management.
-END-
Question:305
...
Answer: C
Explanation: FSx for Windows File Server provides fully managed SMB file shares.
-END-
Question:306
...
Answer: A
Explanation: Cluster placement group in same AZ gives highest bandwidth & lowest latency for in‑memory DB traffic minimizing data transfer charges.
-END-
Question:307
...
Answer: D
Explanation: Volume Gateway cached volumes keep recent data locally while storing full dataset in S3 reducing on‑prem scaling needs.
-END-
Question:308
...
Answer: A,C
Explanation: View checks in account running RDS (A); Reserved Instance Optimization identifies cost savings for long‑running On‑Demand RDS (C).
-END-
Question:309
...
Answer: A
Explanation: S3 Storage Lens provides advanced metrics and activity to identify rarely accessed buckets with least setup.
-END-
Question:310
...
Answer: B
Explanation: CloudFront reduces data transfer cost via edge caching (lower origin egress) and improves performance; signed URLs secure access.
-END-
Question:311
...
Answer: C
Explanation: SNS topic with filtered SQS queues separates quote types; queues guarantee delivery & 24h processing window; minimal ops.
-END-
Question:312
...
Answer: C
Explanation: AWS Backup targeting EBS volumes captures instance data; cross‑Region copy enables DR with lowest ops overhead.
-END-
Question:313
...
Answer: C
Explanation: CloudFront distributes streaming content globally; signed URLs enforce authorization at scale.
-END-
Question:314
...
Answer: B
Explanation: Aurora Serverless auto‑scales capacity without choosing instance size, suitable for infrequent access and future growth.
-END-
Question:315
...
Answer: D
Explanation: Amazon Inspector continuously scans EC2 for vulnerabilities; Lambda can compile and distribute reports.
-END-
Question:316
...
Answer: C
Explanation: Migrating polling script to Lambda scales with queue size eliminating EC2 management and reducing cost.
-END-
Question:317
...
B. Develop a Python script that runs on Amazon EC2 instances to convert the .csv files to .sgl files. Invoke the Python script on a cron schedule to store the output files in Amazon S3.
...
Answer: A
Explanation: Scheduled Glue ETL converts CSV to Redshift‑compatible format with minimal code and integrates with S3 source.
-END-
Question:318
...
Answer: A,D
Explanation: CloudTrail records API activity for auditing (A) while AWS Config continuously records resource configurations and evaluates them with rules (D); together they track and audit changes.
-END-
Question:319
...
C. Allow shared SSH access to a set of bastion instances. Configure all-otherinstances to allow only SSH access from the bastion instances.
D. Use an Amazon Cognito custom authorizer to authenticate users. Invoke an AWS Lambda function to generate a temporary SSH key.

Answer: A
Explanation: Systems Manager Session Manager provides audited, keyless, browser/CLI access removing shared SSH keys with least admin overhead.
-END-
Question:320
...
Answer: A
Explanation: Kinesis Data Streams durably buffers near real‑time JSON; Kinesis Data Analytics can run SQL over the stream enabling low‑latency querying with minimal data loss on reboot.
-END-
Question:321
...
Answer: D
Explanation: Bucket policy deny on missing x-amz-server-side-encryption enforces server‑side encryption for all PutObject requests.
-END-
Question:322
...
Answer: C
Explanation: SQS decouples upload acknowledgment from thumbnail processing; user gets fast receipt while workers asynchronously generate thumbnails.
-END-
Question:323
...
Answer: B
Explanation: API Gateway HTTPS endpoint with Lambda + DynamoDB is serverless, HA, and scalable for badge reader messages.
-END-
Question:324
...
Answer: D
Explanation: Volume Gateway stored volumes keep the full dataset on‑prem for low‑latency access and asynchronously back up to AWS for DR.
-END-
Question:325
...
Answer: A
Explanation: Correct IAM role must be assumed via the Cognito identity pool to grant S3 access for protected content.
-END-
Question:326
...
D. Move assets to S3 Standard-Infrequent Access (S3 Standard-lA) after 30 days.
E. Move assets to S3 One Zone-Infrequent Access (S3 One Zone-IA) after 30 days.

Answer: A,B
Explanation: After 30 days access becomes unpredictable so Intelligent-Tiering optimizes storage cost (A); lifecycle cleanup of incomplete multipart uploads avoids orphaned parts cost (B).
-END-
Question:327
...
Answer: A
Explanation: AWS Network Firewall with domain list rule groups allows egress filtering to approved repository domains; SGs/NACLs lack URL/domain inspection.
-END-
Question:328
...
Answer: D
Explanation: CloudFront caches static site assets reducing origin load; SQS decouples bursty sales requests for reliable asynchronous backend processing.
-END-
Question:329
...
Answer: D
Explanation: Amazon Inspector provides automated vulnerability scanning; Systems Manager Patch Manager automates patching and reporting.
-END-
Question:330
...
Answer: A
Explanation: Enable RDS encryption at creation with a KMS CMK to encrypt data at rest transparently.
-END-
Question:331
...
Answer: A
Explanation: 20 TB over 15 Mbps (70% util) cannot finish in 30 days; Snowball provides encrypted, high‑capacity physical transfer.
-END-
Question:332
...
Answer: B
Explanation: FSx for Windows File Server integrates with AD providing secure SMB access; Client VPN secures remote connections.
-END-
Question:333
...
Answer: C
Explanation: Scheduled scaling pre‑provisions capacity just before predictable monthly spike preventing CPU saturation.
-END-
Question:334
...
Answer: A
Explanation: AWS Transfer Family SFTP with AD auth gives managed SFTP access to S3 without app changes.
-END-
Question:335
...
Answer: B
Explanation: Fast Snapshot Restore enables low‑latency EBS volume initialization from the AMI snapshot for fast scale‑out.
-END-
Question:336
...
D. Store afile that contains the credentials in an AWS Key Management Service (AWS KMS) encrypted Amazon S3 bucket that the application uses to load the credentials. Download the file to the application regularly to ensure that the correct credentials are used. Implement an AWS Lambda function that rotates the Aurora credentials every 14 days and uploads these credentials to the file in the S3 bucket.

Answer: A
Explanation: Secrets Manager with KMS encryption and 14‑day rotation automates credential storage and rotation with least effort.
-END-
Question:337
...
Answer: A
Explanation: Aurora MySQL with Aurora Replicas and auto scaling reduces replication lag under peak load with minimal code changes.
-END-
Question:338
...
Answer: A
Explanation: Native MySQL binlog replication to single instance in secondary Region is cheaper than global database while providing DR copy.
-END-
Question:339
...
Answer: C
Explanation: Secrets Manager stores credentials and handles scheduled rotation with minimal code change—application retrieves dynamically.
-END-
Question:340
...
Answer: A
Explanation: AWS WAF on ALB blocks SQL injection patterns; other options do not directly mitigate SQLi.
-END-
Question:341
...
Answer: D
Explanation: Lake Formation ingestion + column‑level permissions enforced via Athena gives least overhead and fine‑grained control to QuickSight.
-END-
Question:342
...
Answer: C
Explanation: Predictive scaling pre‑launches capacity 30 minutes before workload based on forecast, meeting performance with minimal manual analysis.
-END-
Question:343
...
Answer: C
Explanation: Aurora global database offers low replication lag multi‑Region with minimal management and built‑in DR.
-END-
Question:344
...
Answer: A
Explanation: SQS Extended Client Library stores large payloads in S3 and keeps pointers in FIFO/standard messages enabling >256 KB with minimal code changes.
-END-
Question:345
...
Answer: A
Explanation: Cognito handles auth for small user base and scales; Lambda@Edge enforces authorization; CloudFront delivers globally with low latency.
-END-
Question:346
...
Answer: D
Explanation: S3 File Gateway presents SMB/NFS backed by S3 enabling lifecycle policies while preserving existing access patterns.
-END-
Question:347
...
Answer: A
Explanation: Compute Savings Plan provides largest savings with flexibility to change instance families and sizes over 3 years.
-END-
Question:348
...
A. Use provisioned mode and DynamoDB Standard-Infrequent Access (DynamoDB Standard-1A). Reserve capacity for the forecasted workload.
...
Answer: B
Explanation: Predictable constant workload best served by provisioned capacity setting RCUs/WCUs; on‑demand would cost more.
-END-
Question:349
...
Answer: B
Explanation: Add acquiring account to KMS key policy then share encrypted snapshot; maintains encryption and secure sharing.
-END-
Question:350
A company uses a 100 GB Amazon RDS for Microsoft SQL Server Single-AZ DB instance in the us-east-1 Region to store customer transactions. The company needs high availability and automatic recovery for the DB instance. The company must also run reports on the RDS database several times a year. The report process causes transactions to take longer than usual to post to the customers’ accounts. The company needs a solution that will improve the performance of the report process. Which combination of steps will meet these requirements? (Choese two:)
...
Answer: A,C
Explanation: Multi-AZ adds HA and automatic failover; read replica offloads reporting workloads improving performance.
-END-
Question:351
...
Answer: D
Explanation: Step Functions state machine orchestrates serverless workflow invoking Lambda steps with minimal ops.
-END-
Question:352
...
B. Set up AWS Global Accelerator with UDP listenets-and-endpoint groups in.each-Region.
C. Set up Amazon CloudFront with UDP turned on. Configure an origin in each Region.
D. Set up a VPC peering mesh between each Region. Turn on UDP for each VPC.

Answer: B
Explanation: Global Accelerator supports UDP, improves latency and availability across Regions.
-END-
Question:353
A company hosts a three-tier web application on Amazon EC2 instances in a single Availability Zone. The web application uses a self-managed MySQL database that is hosted on an EC2 instance to store data in an Amazon Elastic Block Store (Amazon EBS) volume. The MySQL database currently uses a 1 TB Provisioned I0PS SSD (i02) EBS volume. The company expects traffic of 1,000 I0PS for both reads and writes at peak traffic. The company wants to minimize any disruptions, stabilize performance, and reduce costs while retaining the capacity for double the IOPS. The company wants to move the database tier to a fully managed solution that is highly available and fault tolerant. Which solution will meet these requirements MOST cost-effectively?
...
Answer: B
Explanation: RDS MySQL Multi-AZ with gp2/gp3 meets 1k IOPS and HA at lower cost than io2 Block Express.
-END-
Question:354
...
Answer: B
Explanation: RDS Proxy pools connections lowering timeouts from Lambda concurrency with minimal code changes.
-END-
Question:355
...
Answer: D
Explanation: AWS Batch manages scheduling, provisioning appropriate EC2 compute for hourly CPU‑intensive job with least ops.
-END-
Question:356
...
B. Move the data objects to S3 Standard-Infrequent Access (S3 Standard-1A) after 30 days.
C. Move the data objects to S3 One Zone-Infrequent Access (S3 One Zone-IA) after 30 days.
D. Move the data objects to S3 One Zone-Infrequent Access (S3 One Zone-IA) immediately.

Answer: B
Explanation: Standard-IA after 30 days preserves multi‑AZ durability and immediate access while cutting cost; One Zone or Glacier options reduce availability or access speed.
-END-
Question:357
...
Answer: A,D
Explanation: S3 + CloudFront for static assets; FSx for Windows File Server shares Windows server-side code across EC2 instances with HA.
-END-
Question:358
...
Answer: C
Explanation: Lambda@Edge dynamically resizes and formats images on requests from CloudFront with minimal infrastructure.
-END-
Question:359
...
Answer: C
Explanation: aws:SecureTransport enforces TLS; SSE-KMS with customer-managed keys lets compliance team administer encryption.
-END-
Question:360
A company uses Amazon API Gateway to run a private gateway with two REST APIs in the same VPC. The BuyStock RESTful web service calls the CheckFunds RESTful web service to ensure that enough funds are available before a stock can be purchased. The company has noticed in the VPC flow logs that the BuyStock RESTful web service calls the CheckFunds RESTful web service over the internet instead of through the VPC. A solutions architect must implement a solution so that the APls communicate through the VPC. Which solution will meet these requirements with the FEWEST changes to the code?
...
Answer: B
Explanation: Private API Gateway REST APIs require interface VPC endpoints to route traffic internally instead of internet path.
-END-
Question:361
...
Answer: C
Explanation: DynamoDB + DAX delivers sub‑millisecond reads; table export to S3 allows Athena one‑time SQL queries with minimal ops.
-END-
Question:362
...
Answer: B,E
Explanation: Partitioning Kinesis by payment ID preserves per‑key order (B); SQS FIFO with message group payment ID also maintains order for processing (E).
-END-
Question:363
...
Answer: B
Explanation: SNS FIFO topics support ordered fan‑out to multiple subscribers (leaderboard, matchmaking, auth) guaranteeing ordering.
-END-
Question:364
...
Answer: B,D
Explanation: Use KMS CMKs for SNS and SQS with key policies restricting principals and enforce TLS via condition (D) for queues; SNS encryption with CMK (B).
-END-
Question:365
...
Answer: C
Explanation: Automated backups with PITR allow restore to any point in last retention window (e.g., 5 minutes) within 30 days.
-END-
Question:366
...
Answer: D
Explanation: API usage plans + keys limit premium access without modifying table or complex auth code; integrates with Cognito identities.
-END-
Question:367
...
Answer: A
Explanation: Global Accelerator with NLB endpoints improves UDP performance/availability while keeping on‑prem hosting.
-END-
Question:368
...
Answer: A
Explanation: Account-level password policy enforces complexity and rotation for all new IAM users centrally.
-END-
Question:369
...
Answer: C
Explanation: Converting tasks to Lambda functions scheduled via EventBridge removes single instance bottleneck with minimal ops.
-END-
Question:370
...
Answer: C
Explanation: NAT Gateway in public subnet is managed and minimizes maintenance for private subnet outbound internet access.
-END-
Question:371
...
Answer: C,D
Explanation: Enable default EBS encryption with customer CMK (C) and grant EKS role permissions to use key (D); volumes auto encrypted at creation.
-END-
Question:372
...
Answer: B
Explanation: S3 stores images; DynamoDB key = geo code with S3 URL value scales writes and reads cost‑effectively, single row per code.
-END-
Question:373
...
D. Use the S3 Standard storage class. Create an S3 Lifecycle policy to transition objects to S3 Standard-Infrequent Access (S3 Standard-1A) after 30 days, and then to S3 Glacier Deep Archive after 1 year.

Answer: D
Explanation: Standard then transition to Standard-IA at 30 days (still frequently needed up to 12 months) then to Deep Archive after 1 year balances immediate availability and cost.
-END-
Question:374
...
Answer: D
Explanation: Single Direct Connect + transit gateway allows shared high‑bandwidth, low‑latency connectivity to multiple VPCs cost‑effectively.
-END-
Question:375
...
Answer: A
Explanation: Step Functions orchestrates Lambda, manual approvals, and other services with minimal operational overhead.
-END-
Question:376
...
C. Migrate the DB instance to a different instance class that has:higher I/0|capacity. Configure the users’ applications to use the new DB instance.
D. Configure Multi-AZ for the DB instance. Configure the users’ applications to switch between the DB instances.

Answer: A
Explanation: RDS Proxy pools connections smoothing spikes from serverless apps and reducing rejection errors.
-END-
Question:377
...
Answer: B
Explanation: Auto Scaling lifecycle hooks invoke scripts at launch/terminate enabling immediate reporting to audit system efficiently.
-END-
Question:378
...
Answer: B
Explanation: NLB supports UDP and scales; DynamoDB on‑demand auto scales non‑relational gamer data without management.
-END-
Question:379
...
Answer: B
Explanation: Provisioned concurrency keeps Lambda warm reducing cold start latency loading libraries with minimal operational changes.
-END-
Question:380
...
Answer: D
Explanation: EventBridge scheduled rule invoking Lambda to start/stop EC2 and RDS automates schedule with minimal infra.
-END-
Question:381
...
Answer: B
Explanation: Aurora Replica handles read/report workload without blocking writes; minimal code changes from RDS PostgreSQL migration.
-END-
Question:382
...
Answer: A
Explanation: TLS listener on NLB encrypts traffic in transit; other options don't ensure end‑to‑end encryption for current design.
-END-
Question:383
...
Answer: A
Explanation: Dedicated Reserved Hosts let reuse existing socket/core licenses with cost savings over On‑Demand.
-END-
Question:384
...
C. Use the Amazon Elastic File System (Amazon EFS) Standard storage class. Create a lifecycle management policy to move infrequently accessed data to EFS Standard-Infrequent Access (EFS Standard-1A).
...
Answer: C
Explanation: EFS Standard + lifecycle to IA retains POSIX compliance, HA, durability; S3 not POSIX; One Zone reduces durability.
-END-
Question:385
...
Answer: C
Explanation: Least privilege: web SG allows 443 only from LB SG; DB SG allows 3306 only from web SG.
-END-
Question:386
...
Answer: B
Explanation: ElastiCache caches repeated result sets reducing backend DB workload and improving performance.
-END-
Question:387
...
C. Create a new |AM user for the deployment engineer and add the IAM user to a group that has the AdministratorAccess IAM policy attached.
D. Create a new IAM user for the deployment engineer and add the |AM user to a group that has an IAM policy that allows AWS CloudFormation actions only.
E. Create an IAM role for the deployment engineer to explicitly define the permissions specific to the AWS CloudFormation stack and launch stacks using that 1AM role.

Answer: D,E
Explanation: IAM group policy limited to CloudFormation actions (D) plus role for stack execution with specific permissions (E) enforces least privilege.
-END-
Question:388
...
Answer: D
Explanation: Need inbound rule on DB SG referencing web tier SG to permit connectivity; defaults block it.
-END-
Question:389
...
Answer: A
Explanation: Read replicas offload reporting queries preserving write performance on primary.
-END-
Question:390
...
Answer: B,D
Explanation: DynamoDB or ElastiCache store durable/high‑speed session data decoupled from instances; sticky sessions alone lack durability.
-END-
Question:391
...
Answer: C
Explanation: Stateless tiers recovered from AMIs; RDS automated backups + PITR meet 2‑hour RPO without frequent EBS snapshots.
-END-
Question:392
...
Answer: A
Explanation: Public web SG allows 443 from all (global customers); DB SG restricts 3306 to web SG only.
-END-
Question:393
A payment processing company records all voice communication with its customers and stores the audio files in an Amazon S3 bucket. The company needs to capture the text from the audio files. The company must remove from the text any personally identifiable information (PIl) that belongs to customers. What should a solutions architect do to meet these requirements?

A. Process the audio files by using Amazon Kinesis Video Streams. Use an AWS Lambda function to scan for known Pl patterns.
...
D. Create an Amazon Connect contact flow that ingests the audio files with transcription turned on. Embed an AWS Lambda function to scan for known PlI patterns. Use Amazon EventBridge to start the contact flow when an audio file is uploaded to the S3 bucket.

Answer: C
Explanation: Amazon Transcribe with PII redaction produces text with sensitive data removed stored separately.
-END-
Question:394
A company is running a multi-tier ecommerce web application in the AWS Cloud. The application runs on Amazon EC2 instances with an Amazon RDS for MySQL Multi-AZ DB instance. Amazon RDS is configured with the latest generation DB instance with 2,000 GB of storage in a General Purpose SSD (gp3) Amazon Elastic Block Store (Amazon EBS) volume. The database performance affects the application during periods of high demand. A database administrator analyzes the logs in Amazon CloudWatch Logs and discovers that the application performance always degrades when the number of read and write 10PS is higher than 20,000. What should a solutions architect do to improve the application performance?

A. Replace the volume with a magnetic volume.
B. Increase the number of I0PS on the gp3 volume.
C. Replace the volume with a Provisioned I0PS SSD (io2) volume.
D. Replace the 2,000 GB gp3 volume with two 1,000 GB gp3 volumes.

Answer: B
Explanation: Increasing gp3 provisioned IOPS above 20k improves performance; migration to io2 adds cost beyond need.
-END-
Question:395
...
Answer: C
Explanation: CloudTrail records who made API calls including security group modifications.
-END-
Question:396
...
Answer: A
Explanation: Shield Advanced protects Global Accelerator resources against DDoS; accelerator added as protected resource.
-END-
Question:397
...
B. Create an AWS Lambda function. Create an Amazon API Gateway HTTP AP, and integrate the API with the function. Create an Amazon EventBridge scheduled event that calls the APl and invokes the function.
...
Answer: C
Explanation: ECS Fargate scheduled task via EventBridge handles predictable CPU/memory job with minimal management and supports large objects.
-END-
Question:398
...
Answer: A
Explanation: 600 TB in 2 weeks exceeds practical bandwidth (100 Mbps); Snowball devices provide encrypted high‑speed physical transfer cost‑effectively.
-END-
Question:399
A financial company hosts a web application on AWS. The application uses an Amazon API Gateway Regional APl endpoint to give users the ability to retrieve current stock prices. The company’s security team has noticed an increase in the number of API requests. The security team is concerned that HTTP flood attacks might take the application offline. A solutions architect must design a solution to protect the application from this type of attack. Which solution meets these requirements with the LEAST operational overhead?

A. Create an Amazon CloudFront distribution in front of the APl Gateway Regional API endpoint with a maximum TTL of 24 hours.
...
Answer: B
Explanation: Regional WAF web ACL with rate‑based rule on API Gateway stage automatically mitigates HTTP flood with minimal ops.
-END-
Question:400
...
Answer: C
Explanation: DynamoDB Streams + single SNS topic fan‑out notifies teams asynchronously without impacting table write latency.
-END-
Question:401
...
Answer: C
Explanation: Throughput exceeded shard capacity causing data loss; increasing shard count scales ingestion; retention default fine for 2‑day batch.
-END-
 Question:403
...
D. Create an IAM execution role with the required permissions and attach'the'|AMrole to the Lambda function.

Answer: D
Explanation: Lambda requires execution role with S3 permissions; attach IAM role instead of embedding user keys.
-END-
 Question:404
...
Answer: D
Explanation: SQS decouples S3 event inflow from Lambda processing preventing dropped documents under burst load.
-END-
 Question:405
...
Answer: D,E
Explanation: Target tracking scales on CPU for variable weekday load (D); scheduled scaling sets capacity to zero on weekends (E).
-END-
 Question:406
...
B. Create a security group for the DB instance. Add arule to allow traffic/from the public/subnet CIDR block on port 3306.
...
Answer: C,D
Explanation: Web SG needs inbound 443 from internet (C); DB SG should allow 3306 from web SG not broad CIDR for least privilege (D).
-END-
 Question:407
...
Answer: D
Explanation: FSx for Lustre is fully managed, exposes Lustre endpoints for clients.
-END-
 Question:408
A company runs an application that receives data from thousands of geographically dispersed remote devices that use UDP. The application processes the dataimmediately and sends a message back to the device if necessary. No data is stored. The company needs a solution that minimizes latency for the data transmission from the devices. The solution also must provide rapid failover to another AWS Region. Which solution will meet these requirements?
...
Answer: B
Explanation: Global Accelerator + NLB endpoints support UDP low latency and multi‑Region failover; ECS Fargate service processes packets.
-END-
 Question:409
A solutions architect must migrate a Windows Internet Information Services (1IS) web application to AWS. The application currently relies on a file share hosted in the user's on-premises network-attached storage (NAS):The solutions'architect has proposed migrating the IS web servers to Amazon EC2 instances in multiple Availability Zones that are connected to the storage solution, and configuring an Elastic Load Balancer attached to the instances. Which replacement to the on-premises file share is MOST resilient and durable?
...
Answer: C
Explanation: FSx for Windows File Server provides resilient, durable SMB file share across AZs for IIS.
-END-
 Question:410
...
Answer: B
Explanation: Creating encrypted EBS volumes ensures all data at rest is encrypted; attach to instances; role/tag not enforce encryption.
-END-
Question:411
...
Answer: B
Explanation: RDS MySQL supports existing schema with variable usage; no app change; use scaling options (read replicas) later.
-END-
Question:412
...
B. Use AWS Trusted Advisor to find publicly accessible S3 buckets. Configure email notifications in Trusted Advisor when a change is detected. Manually change the S3 bucket policy if it.allows public aceess!
...
Answer: D
Explanation: Account‑level S3 Block Public Access + SCP prevents disabling, ensuring all objects remain private.
-END-
Question:413
...
Answer: B
Explanation: Amazon SES handles reliable scalable email delivery minimizing operational overhead.
-END-
Question:414
...
Answer: B
Explanation: S3 File Gateway exposes SMB share; writes land directly in S3 near real‑time with minimal admin changes.
-END-
Question:415
...
C. Create an S3 Lifecycle configuration with a rule to-transition the objects inthe S8 bucket to S3 Glacier Instant Retrieval.
...
Answer: A
Explanation: Intelligent‑Tiering auto optimizes cost for unknown/variable access with minimal configuration.
-END-
Question:416
...
Answer: C
Explanation: Compute Savings Plan covers EC2 + Lambda; connecting Lambdas to private subnet keeps low latency to EC2.
-END-
Question:418
...
Answer: B
Explanation: Trust policy lists dev account principal allowing users assuming role; least privilege via role scope not admin creds.
-END-
Question:419
...
Answer: A,E
Explanation: Setting account default EBS encryption (E or EC2 console attribute A) ensures all new volumes auto encrypted with minimal impact; policy denies add overhead.
-END-
Question:420
...
Answer: C
Explanation: Transfer Family SFTP + S3 backend is serverless, scalable high throughput; user permissions via Transfer Family; EFS not needed.
-END-
Question:422
...
C. Direct the requests from the APl into an Amazon Simple Queue Service (Amazon SQS) queue. Deploy the models as AWS Lambda functions that are invoked by SQS events. Use AWS Auto Scaling to increase the number of vCPUs for the Lambda functions based on the SQS queue size.
...
Answer: D
Explanation: SQS decouples irregular/batch requests; ECS services load 1 GB model in memory and scale via Auto Scaling on queue size.
-END-
Question:423
A solutions architect wants to use the following JSON text as an identity-based policy to grant specific permissions: ( “Statement”: { "Action®: Which IAM principals can the solutions architect attach this policy to? (Choose two.) A.Role
...
Answer: A,B
Explanation: Identity-based policies attach to IAM users, groups, or roles; given list includes Role and Group only.
-END-
Question:424
A company is running a custom application on Amazon EC2 On-Demand Instances. The application has frontend nodes that need to run 24 hours aday, 7 days a week and backend nodes that need to run-only forashort time based‘on workload. The number of backend nodes varies during the day. The company needs to scale out and scale in more instances based on workload. Which solution will meet these requirements MOST cost-effectively?
...
Answer: B
Explanation: Frontend 24x7 steady use => Reserved Instances; backend variable short-lived => Spot Instances for cost efficiency.
-END-
Question:425
A company uses high block storage capacity to runs its workloads on premises. The company's daily peak input and output transactions per second are not more than 15,000 I0PS. The company wants to migrate the workloads to Amazon EC2 and to provision disk performance independent of storage capacity. Which Amazon Elastic Block Store (Amazon EBS) volume type will meet these requirements MOST cost-effectively?
...
Answer: C
Explanation: gp3 allows provisioning up to required IOPS independent of capacity cost-effectively versus gp2/io2.
-END-
Question:426
...
Answer: A
Explanation: DataSync securely migrates changing datasets; CloudTrail data events give audit access at object level per regulation.
-END-
Question:427
...
Answer: B
Explanation: Elastic Beanstalk manages Tomcat + HA load-balanced environment with rolling deployments minimal custom ops.
-END-
Question:428
A serverless application uses Amazon AP| Gateway, AWS Lambda, and Amazon DynamoDB. The Lambda function needs permissions to read and write to the DynamoDB table. Which solution will give the Lambda function access to the DynamoDB table MOST securely?
...
Answer: B
Explanation: Lambda execution role with least-privilege policy grants secure DynamoDB read/write without credential storage.
-END-
Question:429
The following IAM policy is attached to an IAM group. This is the only policy applied to the group. "Version": "2012-10-17%, "Statement™: [ ( "sid®: "1°, "Effect”: “"Allow", "Action®: “ec2:*", “"Resource": "** "Condition": ( "StringEquals”: ( “ec2:Region®: “us-east-1°" “sid*: "2, "Effect": “"Deny", "Action®: “"ec2:Stoplnstances", “ec2:TerminateInstances” 1, "Resource": "*", “Condition®: ( “"BoolIfExists”: ("aws:MultiFactorAuthPresent": What are the effective IAM permissions of this policy for group members? false) Topic 1
...
C. Group members are allowed the ec2:StopInstances and ec2:Terminatelnstances permissions for all Regions when logged in with multifactor authentication (MFA). Group members are permitted any other Amazon EC2 action.
D. Group members are allowed the ec2:StopInstances and ec2:Terminatelnstances permissions for the us-east-1 Region only when logged in with multi-factor authentication (MFA). Group members are permitted any other Amazon EC2 action within the us-east-1 Region.

Answer: D
Explanation: Allow ec2:* in us-east-1; Deny stop/terminate unless MFA present (BoolIfExists false) so those actions allowed only with MFA; others still allowed.
-END-
Question:430
...
Answer: B,C
Explanation: Lambda processes immediately on upload for fast image availability; transition CSV to Glacier after 1 day minimizes cost yet retrievable for planned ML training.
-END-
Question:431
...
Answer: B
Explanation: ElastiCache Redis supports sorted sets for real-time top-10 and persistence via RDB/AOF for restore capability.
-END-
Question:432
...
B. Use Amazon SageMaker tobuild and train models. Use Amazon QuickSight to visualize thedata.
...
Answer: B
Explanation: SageMaker handles model build/train; QuickSight integrates for visualization with lowest operational overhead.
-END-
Question:433
...
Answer: C
Explanation: SCPs at org level can restrict tag modification globally except authorized principals.
-END-
Question:434
...
A. Create an Auto Scaling group and a load balancer in the disaster recovery Region. Configure the DynamoDB table as a global table. Configure DNS failover topoint to the new disaster recovery Region's load balancer.
B. Create an AWS CloudFormation template to create EC2 instances, load balancers, and DynamoDB tables to be launched when needed Configure DNS failover topoint to the new disaster recovery Region's load balancer.
...
Answer: A
Explanation: Pre-provision ASG + LB in DR Region and DynamoDB global table enables rapid DNS failover minimal downtime.
-END-
Question:435
A company needs to migrate a MySQL database from its on-premises data center to AWS within 2 weeks. The database is 20 TB in size. The company wants to complete themigration with minimal downtime. Which solution will migrate the database MOST cost-effectively?

A. Order an AWS Snowball Edge Storage Optimized device. Use AWS Database Migration Service (AWS DMS) with AWS Schema Conversion Tool (AWS SCT) to migrate the database with replication of ongoing changes. Send the Snowball Edge device to AWS to finish the migration and continue theongoing replication.
...
Answer: A
Explanation: Snowball Edge Storage Optimized transfers 20 TB quickly; DMS handles ongoing replication minimizing downtime cost‑effectively.
-END-
Question:436
...
Answer: B
Explanation: Multi-AZ adds standby for failover without adding new infra; vertical scale only if needed—question asks accommodate workload without adding infra.
-END-
Question:437
...
C. Deploy rules to the network ACLs associated with the/ALB to-blockthe incomingtraffic.
D. Deploy Amazon GuardDuty and enable rate-limiting protection when configuring GuardDuty.

Answer: B
Explanation: AWS WAF rate-based rule on ALB blocks high-rate illegitimate requests minimizing impact on real users.
-END-
Question:438
...
A. Create aread replica of the database. Configure IAM standard database authentication to grant the auditor access.
...
Answer: D
Explanation: Share encrypted snapshot + KMS key access gives auditor own copy securely without exposing DB/network.
-END-
Question:439
...
Answer: A
Explanation: Adding secondary IPv4 CIDR expands address space in-place with least operational overhead versus new VPC/peering.
-END-
Question:440
...
Answer: C,E
Explanation: Use mysqldump (upload to S3) import to Aurora directly (C) or via DMS (E); snapshot cannot import directly to Aurora.
-END-
Question:441
...
Answer: C
Explanation: Offload static content to S3 + CloudFront reducing ALB/EC2 load and On-Demand scaling cost.
-END-
Question:442
...
Answer: D
Explanation: Lake Formation tag-based access control grants fine-grained cross-account permissions with minimal admin overhead.
-END-
Question:443
...
Answer: A
Explanation: S3 + Transfer Acceleration minimizes global upload/download latency cost-effectively for large unique objects.
-END-
Question:444
...
Answer: B
Explanation: Multi-AZ RDS with deletion protection plus ALB + Auto Scaling across AZs adds resilience and prevents accidental DB deletion impact.
-END-
Question:445
...
Answer: A
Explanation: DataSync leverages existing Direct Connect to move 700 TB incrementally while data remains accessible; meets 90-day window.
-END-
Question:446
...
Answer: C
Explanation: S3 Object Lock compliance mode enforces retention; existing objects must be recopied to apply lock.
-END-
Question:447
A company has a stateless web application that runs on AWS Lambda functions that are invoked by Amazon AP| Gateway. The company wants to deploy the application across multiple AWS Regions to provide Regional failover capabilities. What should a solutions architect do to route traffic to multiple Regions?
...
Answer: A
Explanation: Route 53 health checks with active-active failover route traffic across Regions for Lambda/API Gateway endpoints.
-END-
Question:448
...
Answer: C
Explanation: Add second customer gateway + VPN for Management VPC removes single device SPOF; existing peering stays.
-END-
Question:449
...
Answer: B
Explanation: RDS Custom for Oracle supports privileged access & third-party features with managed infra cost-effectively vs EC2 self-manage.
-END-
Question:450
...
Answer: C,E,F
Explanation: Refactor into separate tiers across AZs (C); load balancers with SG references (E); Multi-AZ RDS cluster with restricted SG access (F) matches Well-Architected principles.
-END-
Question:451
...
Answer: B,C,F
Explanation: Team manages RDS instance creation & maintenance window (B), ECS extra software (C), and encryption in transit via Direct Connect config (F); AWS manages infra patching & physical security.
-END-
Question:452
...
Answer: B
Explanation: Lambda handles short hourly 10s execution with 1 GB memory low cost vs always-on EC2.
-END-
Question:453
...
Answer: D
Explanation: AWS Backup vault lock compliance mode enforces immutability meeting regulatory retention.
-END-
Question:454
...
Answer: C
Explanation: Workload Discovery on AWS auto-generates diagrams and relationships with least manual effort.
-END-
Question:455
...
Answer: B,D,E
Explanation: Budgets created via billing dashboard (B); IAM role for budget actions (D); alert + budget action using IAM identity to apply restriction (E).
-END-
Question:456
...
Answer: B
Explanation: Periodic EBS snapshot copies to second Region are cost-effective; full DR infra not needed yet.
-END-
Question:457
...
Answer: C
Explanation: AWS Transfer Family supports AS2 protocol with custom identity integration via Lambda for IdP auth.
-END-
Question:458
...
Answer: B,C
Explanation: Lambda fits 1 GB memory compute; RDS provides relational storage with minimal admin overhead.
-END-
Question:459
...
Answer: A
Explanation: Activate user-defined cost allocation tag at management account then single Cost Explorer report grouped by tag filtered for EC2.
-END-
Question:460
A company wants to securely exchange data between its software as a service (SaaS) application Salesforce account and Amazon S3. The company must encrypt the data at rest by using AWS Key Management Service (AWS KMS) customer managed keys (CMKs). The company must also encrypt the data in transit. The company has enabled APl access for the Salesforce account.
...
Answer: C
Explanation: AppFlow provides secure managed Salesforce->S3 transfer with KMS and TLS encryption.
-END-
Question:461
...
Answer: B
Explanation: Global Accelerator + NLB supports TCP & UDP globally minimizing latency; ALB lacks UDP support.
-END-
Question:462
...
C. Write orders to Amazon Simple Notification Service (Amazon SNS). Subscribe the database endpoint to the SNS topic. Use EC2 instances in an Auto Scaling group behind an Application Load Balancer toread from the SNS topic.
...
Answer: B
Explanation: SQS buffers spikes; Auto Scaling consumers process queue quickly writing reliably to Aurora.
-END-
Question:463
...
Answer: C
Explanation: Lambda fits 30s, 1 GB memory small nightly 2 MB workloads cost-effectively vs Glue/EMR overhead.
-END-
Question:464
...
Answer: A
Explanation: Modify to Multi-AZ adds automatic failover with no app code changes minimal downtime.
-END-
Question:465
...
C. Use Provisioned I0PS SSD (i02) EBS volumes with Amazon Elastic Block Store (Amazon EBS) Multi-Attach
D. Use General Purpose SSD (gp2) EBS volumes with Amazon Elastic Block Store (Amazon EBS) Multi-Attach

Answer: C
Explanation: io2 supports Multi-Attach for simultaneous write access across Nitro instances; gp2/gp3 do not support Multi-Attach.
-END-
Question:466
...
Answer: A
Explanation: Multi-AZ Auto Scaling + ALB provides HA for stateless tier complementing existing Multi-AZ RDS.
-END-
Question:467
...
Answer: B
Explanation: Discount sharing must be enabled in the management account so unused Savings Plan benefits apply org‑wide.
-END-
Question:468
...
A. Design a WebSocket API by using Amazon API Gateway. Host the application in Amazon Elastic Container Service (Amazon ECS) in a private subnet. Create a private VPC link for APl Gateway to access Amazon ECS.
B. Design a REST API by using Amazon API Gateway. Host the application in Amazon Elastic Container Service (Amazon ECS) in a private subnet. Create a private VPC link for APl Gateway to access Amazon ECS.
C. Design a WebSocket API by using Amazon API Gateway. Host the application in Amazon Elastic Container Service (Amazon ECS) ina private subnet. Create a security group for APl Gateway to access Amazon ECS.
D. Design a REST API by using Amazon API Gateway. Host the application in Amazon Elastic Container Service (Amazon ECS) in a private subnet. Create a security group for APl Gateway to access Amazon ECS.

Answer: B
Explanation: REST API via API Gateway accessing ECS services in private subnets uses VPC link for private integration.
-END-
Question:469
...
Answer: C
Explanation: Intelligent-Tiering automatically optimizes cost for unpredictable access without lifecycle tuning.
-END-
Question:470
...
Answer: D
Explanation: Egress-only internet gateway enables outbound-only IPv6 connections preventing inbound initiation.
-END-
Question:471
...
Answer: C
Explanation: Gateway VPC endpoint keeps S3 traffic off internet and has no data processing charges.
-END-
Question:472
...
Answer: A
Explanation: DAX adds in-memory caching with minimal code changes (endpoint swap) providing microsecond reads.
-END-
Question:473
...
Answer: A
Explanation: CloudFront caches static content reducing EC2/ALB cost; other options unsuitable for static file caching.
-END-
 Question:474
...
Answer: C
Explanation: Transit Gateway centrally connects VPCs; peering TG across Regions scales better than full mesh peering.
-END-
 Question:475
...
Answer: C
Explanation: EFS provides regional mount targets, durability, and supported by AWS Backup cross‑Region replication for RPO ≤8h.
-END-
 Question:476
...
B. Create IAM roles that have least privilege permigsion. Attach-the roles to the-|AM groups
...
Answer: C
Explanation: Attach least‑privilege IAM policies to groups; roles are assumed identities not needed for static user permissions.
-END-
 Question:477
A group requires permissions to list an Amazon S3 bucket and delete objects from that bucket. An administrator has created the following IAM policy to provide access to the bucket and applied that policy to the group. The group is not able to delete objects in the bucket. The company follows least-privilege access rules. "Version": "2012-10-17", "Statement™: [ { "Action": [ "s3:ListBucket", "s3:DeleteObject” 1 "Resource": [ "arn:aws:s3:::bucket-name" 1, "Effect”: "Allow" Which statement should a solutions architect add to the policy to correct bucket access? "Action": [ "s3:*Object" 1,

A. "Resource": [ "arn:aws:s3:::bucket-name/*" 1, "Effect": "Allow" , ®"Action": { g3z 1,
B. "Resource™: [ "arn:aws:s3:::bucket-name/*" 1. "Effect”: "Allow" "Action": [ "s3:DeleteObject" 1,
C. "Resource”: [ "arn:aws:s3:::bucket-name*" 1 "Effect": "Allow" "Action": [ "s3:DeleteObject" 1,
D. "Resource™: [ "arn:aws:s3:::bucket-name/*" 1, "Effect": "Allow"

Answer: B
Explanation: Need object ARN (bucket-name/*) for DeleteObject; listing uses bucket ARN; add statement with specific action.
-END-
 Question:478
...
Answer: B
Explanation: Versioning + Object Lock retention prevents deletion/modification until date; bucket policy grants public read.
-END-
 Question:479
...
Answer: B
Explanation: Define infra as CloudFormation template for automated multi-AZ deployment; fastest repeatable provisioning.
-END-
 Question:480
...
Answer: B
Explanation: Gateway VPC endpoint keeps S3 traffic off public internet between EC2 and S3.
-END-
 Question:481
...
Answer: B
Explanation: Write-through updates cache on writes ensuring cache consistency with DB.
-END-
 Question:482
...
Answer: B
Explanation: DataSync encrypts in transit and simplifies transfer of 100 GB with minimal ops.
-END-
 Question:483
...
Answer: A
Explanation: Lambda supports container images incl. Windows; short periodic job low runtime cost.
-END-
 Question:484
...
Answer: A,E
Explanation: Create org with all features then integrate IAM Identity Center (SSO) with corporate directory for centralized auth.
-END-
 Question:485
...
C. Store the video archives in Amazon S3 Standard-Infrequent Access (S3 Standard-1A).
D. Store the video archives in Amazon S3 One Zone-Infrequent Access (S3 One Zone-IA).

Answer: A
Explanation: Glacier Expedited <5 min access; lowest cost for rarely needed archives requiring fast occasional retrieval.
-END-
 Question:486
...
Answer: A
Explanation: S3 static hosting + ECS Fargate + managed RDS minimizes ops.
-END-
 Question:487
...
Answer: C
Explanation: EFS scalable HA POSIX file system mountable from AWS & on-prem via VPN; no min size.
-END-
 Question:488
...
Answer: C
Explanation: SCP can deny billing access even for root users in member accounts.
-END-
 Question:489
...
Answer: C
Explanation: SNS DLQ using SQS target retains undelivered messages up to 14 days minimal dev.
-END-
 Question:490
...
Answer: B
Explanation: DynamoDB export to S3 continuous; PITR unaffected; no RCU consumption.
-END-
 Question:491
...
B. Use AWS Lambda event source mapping. Use Amazon-Simple Queuge-Service (Amazon SQS) FIFO queues as the event source. Use SQS managed encryption keys (SSE-SQS) for encryption. Add the encryption key invocation permission for the Lambda function.
...
Answer: A
Explanation: SQS standard + Lambda ensures at-least-once, SSE-KMS encryption secure, cost-effective; add kms:Decrypt.
-END-
 Question:492
...
Answer: B
Explanation: SCP restricts disallowed instance types centrally with least effort.
-END-
 Question:493
...
Answer: D,E,F
Explanation: Transcribe audio to text; Translate to English; Comprehend sentiment analysis; no custom model maintenance.
-END-
 Question:494
A company uses Amazon EC2 instances to host its internal systems. As part of a deployment operation, an administrator tries to use the AWS CLI to terminate an EC2 instance. However, the administrator receives a 403 (Access Denied) error message. The administrator is using an IAM role that has the following IAM policy attached: "Version”: "2012-10-17", "Statement": [ { "Effect": "Allow", "Action": ["ec2:Terminatelnstances"], "Resource”: ["*"] "Effect": "Deny", "Action": ["ec2:TerminateInstances"], "Condition": { "NotIpAddress": { "aws:SourcelIp”: [ "192.0.2.0/24", "203.0.113.0/24" N "Resource": ["*"] What is the cause of the unsuccessful request?
...
Answer: D
Explanation: Deny with NotIpAddress blocks request from source outside allowed CIDRs despite Allow; results 403.
-END-
Question:495
...
Answer: C
Explanation: Macie data discovery job uses managed identifiers for PII/financial data detection.
-END-
Question:496
...
C. Deploy AWS Snowball Edge toprovision NFS mounts to on-premises servers.
D. Deploy an AWS Storage Gateway volume gateway to replace the block storage.
E. Deploy Amazon Elastic File System (Amazon EFS) volumes and mount them to on-premises servers.

Answer: B,D
Explanation: File gateway for NFS; Volume gateway for block with local cache; preserves app architecture.
-END-
Question:497
...
Answer: C
Explanation: Gateway endpoint routes S3 traffic internally removing NAT data processing charges.
-END-
Question:498
...
Answer: A
Explanation: Lifecycle + versioning delete noncurrent versions retaining two latest lowers cost minimal ops.
-END-
Question:499
...
A. Set up anew 1 Gbps Direct Connect connection. Share the connection with another AWS account.
...
Answer: D
Explanation: Hosted 200 Mbps via partner lowers cost vs 1 Gbps; secure dedicated connection.
-END-
Question:500
...
Answer: A,D
Explanation: DataSync agents migrate while preserving ACLs; Snowcone + DataSync agent for large consolidation alternative.
-END-
Question:501
...
Answer: C
Explanation: Kinesis Firehose ingests streaming data directly to S3; Kinesis Data Analytics processes in real time minimal ops.
-END-
Question:502
...
Answer: C,E
Explanation: EFS shared scalable image store; ALB + Auto Scaling + CloudFront improves resilience & performance.
-END-
Question:503
...
Answer: A
Explanation: Cross-account role assumption with least privilege & trust policy is most secure access method.
-END-
Question:504
...
Answer: C
Explanation: Transit Gateway is the scalable hub-and-spoke solution for hundreds of VPCs/accounts; far less operational than many peering links.
-END-
Question:505
A company has Amazon EC2 instances that run nightly batch jobs to process data. The EC2 instances run in an Auto Scaling group that uses OnDemand billing. If a job fails on one instance, another instance will reprocess the job. The batch jobs run between 12:00 AM and 06:00 AM local time every day. Which solution will provide EC2 instances to meet these requirements MOST cost-effectively?
...
Answer: C
Explanation: Spot Instances minimize cost for interruptible, retryable nightly batch jobs versus paying 24x7 commitment (RI/Savings Plan).
-END-
Question:506
...
Answer: C
Explanation: Presigned S3 URLs let clients upload directly to S3, removing server bottlenecks and scaling transparently.
-END-
Question:507
...
Answer: B
Explanation: Aurora MySQL primary with cross-Region read replicas preserves single writer and provides low latency global read scalability.
-END-
Question:508
...
Answer: B,D
Explanation: DLM AMI lifecycle with scheduled cross-Region copy (B) plus AWS Backup plan with automatic twice-daily cross-Region copy (D) meets ≤24h RPO with minimal admin effort.
-END-
Question:509
...
Answer: B
Explanation: Network ACLs support deny rules; blocking abusive IPs at the public (web) subnets stops illegitimate requests early.
-END-
Question:510
A global marketing company has applications that run in the ap-southeast-2 Region and the eu-west-1 Region. Applications that run in a VPC in euwest-1 need to communicate securely with databases that run in a VPC in ap-southeast-2. Which network design will meet these requirements?
...
Answer: B
Explanation: Cross-Region VPC peering with route table updates enables private connectivity; allow inbound from app sources on DB security group.
-END-
Question:511
...
Answer: C
Explanation: Aurora Serverless (on-demand) scales per use; pay only while active—cost-saving for part-time development environments.
-END-
Question:512
...
Answer: A
Explanation: Discover untagged resources with Config then tag for inclusion in tag-based AWS Backup plan—lowest ongoing effort.
-END-
Question:513
...
Answer: A
Explanation: S3 static site plus Lambda for on-demand resizing provides serverless, highly available scaling for unpredictable image traffic.
-END-
Question:514
...
A. Grant the required permission in AWS Identity and Access Management (IAM) to the AmazonEKSNodeRole 1AM role.
...
Answer: B
Explanation: Private-only EKS endpoint requires interface VPC endpoints so private subnet nodes can reach control plane APIs.
-END-
Question:515
...
E. Scaling globally tosupport petabytes of data and tens of millions of requests per minute
F. Creating a secondary replica of the cluster by using the AWS Management Console

Answer: B,C,E
Explanation: Redshift excels at encrypted analytics, petabyte scale, and can be used during set hours (pause/resume) to save cost.
-END-
Question:516
A company provides an API interface to customers so the customers can retrieve their financial information. Ehe company expects a larger number of requests during peak usage times of the year. The company requires the API to respond consistently with low latency to ensure customer satisfaction. The company needs to provide a compute host for the API. Which solution will meet these requirements with the LEAST operational overhead?
...
Answer: B
Explanation: API Gateway + Lambda with provisioned concurrency delivers consistently low latency and automatic scaling with minimal ops.
-END-
Question:517
...
C. Create a Systems Manager document to upload alliserver||0gs toa central-S3 bucket: Use Amazon EventBridge to run the Systems Manager document against all servers that are in the account daily.
...
Answer: A
Explanation: Built-in Session Manager option to send session logs directly to S3 is simplest and lowest overhead.
-END-
Question:518
...
C. Change the RDS database instance storage type to Provisioned I0PS
D. Back up the RDS database, increase the storage capacity, restore the database, and stop the previous instance

Answer: A
Explanation: Storage autoscaling automatically expands capacity without downtime or manual intervention.
-END-
Question:519
...
Answer: B
Explanation: Service Catalog publishes curated solutions for self-service deployment across accounts.
-END-
Question:520
...
C. Configure DynamoDB with provisioned read and write|by using the DynamoDB Standard Infrequent Access (DynamoDB Standard-1A) table class. Set DynamoDB auto scaling to a maximum defined capacity.
D. Configure DynamoDB in on-demand mode by using the DynamoDB Standard Infrequent Access (DynamoDB Standard-IA) table class.

Answer: B
Explanation: On-demand capacity handles unpredictable moderate/high traffic cost-effectively; Standard class fits frequent access.
-END-
Question:521
...
Answer: C
Explanation: Cross-account role assumption provides temporary least-privilege access without managing long-lived user keys.
-END-
Question:522
...
B. Use the Kubernetes Metrics Server toactivate horizontal pod autoscaling.
...
Answer: B,C
Explanation: Metrics Server enables Horizontal Pod Autoscaler (pods) while Cluster Autoscaler adjusts nodes—together cover full scaling needs.
-END-
Question:523
...
Answer: A
Explanation: AppSync pipeline resolvers aggregate and parallelize access to multiple DynamoDB tables efficiently with minimal custom code.
-END-
Question:524
...
Answer: C
Explanation: Athena queries CloudTrail data directly without ETL, quickly filtering AccessDenied/Unauthorized events.
-END-
Question:525
...
Answer: A
Explanation: Cost Explorer API gives programmatic historical and forecast cost data (next 12 months) with pagination.
-END-
Question:526
...
A. Create more Aurora PostgreSQL read replicas in the cluster tohandle the load during failover.
...
Answer: D
Explanation: RDS Proxy pools connections and reduces application disruption during Aurora failovers, minimizing downtime.
-END-
Question:527
...
Answer: D
Explanation: Aurora Global Database + multi-Region app/web tiers with Route 53 failover provides highest fault tolerance & rapid recovery.
-END-
Question:528
...
Answer: D
Explanation: Transfer Family provides managed FTP; S3 event triggers Lambda for per-file near real-time processing and deletion.
-END-
Question:529
...
Answer: B
Explanation: Amazon RDS with encryption at rest and managed patching reduces operational overhead while securing transactional data.
-END-
Question:530
A company has an online gaming application thathas TCP and UDP multiplayer gaming capabilities. The company uses Amazon Route 53 to point the application traffic to multiple Network Load Balancers (NLBs) in different AWS Regions. The company needs to improve application performance and decrease latency for the online game in preparation for user growth. Which solution will meet these requirements?
...
Answer: C
Explanation: Global Accelerator improves performance/latency for TCP/UDP game traffic via optimized edge network to regional NLBs.
-END-
Question:531
...
Answer: A
Explanation: Lambda function URL exposes HTTPS endpoint directly; simplest way for third party to invoke webhook.
-END-
Question:532
...
C. Create hosted zones for each customer as required in Route 53. Create zone records that point to the APl Gateway endpoint.
...
Answer: A,D,F
Explanation: Use registered domain + wildcard cert (same Region) + API Gateway custom domain with wildcard to serve individual secure URLs.
-END-
Question:533
A company stores data in Amazon S3. According to regulations, the data must not contain personally identifiable information (PIl). The company recently discovered that S3 buckets have some objects that contain Pll. The company needs to automatically detect PIl in S3 buckets and to notify the company'’s security team. Which solution will meet these requirements?

A. Use Amazon Macie. Create an Amazon EventBridge rule to filter the SensitiveData event type from Macie findings and to send an Amazon Simple Notification Service (Amazon SNS) notification to the securityiteam.
...
Answer: A
Explanation: Macie detects PII in S3; EventBridge rule on SensitiveData findings sends SNS notification to alert security team proactively.
-END-
Question:534
...
B. Transition objects to the S3 Standard-Infrequent Access (S3 Standard-1A) storage class 30 days after creation. Move all objects to the S3 Glacier Flexible Retrieval storage class after 90 days. Write an expiration action that directs Amazon S3 to delete objects after 90 days.
...
D. Transition objects to the S3 One Zone-Infrequent Access (S3 One Zone-lA) storage class 30 days after creation. Move all objects to the S3 Glacier Flexible Retrieval storage class after 90 days. Write an expiration action that directs Amazon S3 to delete objects after 90 days.

Answer: C
Explanation: Keep logs in Standard for 30 days; then move to Glacier Flexible Retrieval for low-cost backup until deletion at 90 days.
-END-
Question:535
...
Answer: B
Explanation: Enable EKS secrets encryption with a customer KMS key to encrypt etcd-stored Kubernetes secrets.
-END-
Question:536
...
Answer: D
Explanation: Multi-AZ DB cluster provides HA and reader endpoints for near real-time analytics without impacting primary.
-END-
Question:537
...
Answer: C
Explanation: AWS Elastic Disaster Recovery continuously replicates block-level changes achieving low RPO/RTO with minimal cost.
-END-
Question:540
...
Answer: C
Explanation: RDS Oracle Multi-AZ cluster offers HA plus readable replicas for offloading reporting.
-END-
Question:541
...
A. Create an AWS Lambda function to retrieve user information from Amazon DynamoDB. Create an Amazon AP| Gateway endpoint to accept RESTful APIs. Send the API calls to the Lambda function.
...
Answer: A,C,E
Explanation: Cognito user pool (C) for auth; API Gateway + Lambda + DynamoDB (A) for serverless backend; Amplify + CloudFront (E) for cost-effective static frontend.
-END-
Question:542
...
Answer: B
Explanation: CloudFront signed URLs restrict premium access and support time‑bound rentals/downloads; S3 origin stays private.
-END-
Question:543
...
Answer: A,E
Explanation: Form organization in existing payer account (E) then enable discount sharing in billing preferences (A) to share Savings Plan.
-END-
Question:544
A retail company uses a regional Amazon AP| Gateway API for its public REST APIs. The API Gateway endpoint is a custom domain name that points to an Amazon Route 53 alias record. A solutions architect needs to create a solution that has minimal effects on customers and minimal data loss to release the new version of APIs. Which solution will meet these requirements?

A. Create a canary release deployment stage for APl Gateway. Deploy the latest API version. Point an appropriate percentage of traffic to the canary stage. After API verification, promote the canary stage to the production stage.
B. Create a new API Gateway endpoint with a new version of the APl in OpenAPI YAML file format. Use the import-to-update operation in merge mode into the APl in API Gateway. Deploy the new version of the API to the production stage.
C. Create a new AP| Gateway endpoint with a new version of the APl in OpenAPI JSON file format. Use the import-to-update operation in overwrite mode into the APl in API Gateway. Deploy the new version of the API to the production stage.
...
Answer: A
Explanation: Canary stage shifts a portion of traffic to new API version allowing validation before full promotion, minimizing impact.
-END-
Question:545
...
Answer: B
Explanation: Route 53 active-passive failover with health check directs traffic to S3 static error page if ALB endpoint unhealthy.
-END-
Question:546
...
Answer: D
Explanation: Storage Gateway VTL (iSCSI) emulates existing tape workflows, eliminating physical tapes with minimal process change.
-END-
Question:547
...
A. Use Amazon Kinesis Data Firehose todeliver streaming data to Amazon S3.
...
Answer: A
Explanation: Kinesis Data Firehose ingests and buffers streaming data then delivers to S3 with near real-time scale & minimal ops.
-END-
Question:548
...
Answer: B
Explanation: Organizations OUs + SCPs centrally govern allowed/denied services across accounts with least operational overhead.
-END-
Question:549
...
Answer: B
Explanation: NAT gateway in public subnet lets private instances reach third-party internet securely without exposing inbound access.
-END-
Question:550
...
Answer: B,D
Explanation: Lambda execution role must have kms:Decrypt and key policy must trust that role for ciphertext decryption of env vars.
-END-
Question:551
...
B. Use S3 Standard. Use an S3 Lifecycle rule to transition the reports to S3 Standard-Infrequent Access (S3 Standard-lA) after 7 days.
C. Use S3 Intelligent-Tiering. Configure S3 Intelligent-Tiering to transition the reports to S3 Standard-Infrequent Access (S3 Standard-1A) and S3 Glacier.
D. Use S3 Standard. Use an S3 Lifecycle rule to transition the reports to S3 Glacier Deep Archive after 7 days.

Answer: A
Explanation: Keep reports in Standard for frequent first-week access then transition to S3 Glacier Flexible Retrieval (6h retrieval OK) for long-term retention.
-END-
Question:552
...
Answer: B
Explanation: Compute Savings Plan (no upfront, 1-year) provides family/type flexibility needed for regular changes.
-END-
Question:553
A solutions architect needs to review a company's Amazon S3 buckets to discover personally identifiable information (PIl). The company stores the PIl data in the us-east-1 Region and us-west-2 Region. Which solution will meet these requirements with the LEAST operational overhead?
...
Answer: A
Explanation: Amazon Macie classification jobs in each Region automatically discover PII in S3 with minimal setup.
-END-
Question:554
...
Answer: C
Explanation: Both SAP app and database are memory intensive; memory-optimized instances best match high RAM demands.
-END-
Question:555
...
Answer: A
Explanation: Interface VPC endpoint for SQS in private subnets and SG allowing instance access secures traffic without Internet.
-END-
Question:556
...
Answer: B
Explanation: Attach IAM role with required DynamoDB permissions via instance profile—no embedded static credentials in template.
-END-
Question:557
...
Answer: B
Explanation: Amazon EMR provides distributed parallel processing on S3 data; EMR jobs can join/enrich using Redshift via JDBC efficiently.
-END-
Question:558
...
Answer: C
Explanation: VPC peering is simplest and most cost-effective for direct same-Region inter-VPC traffic versus Transit Gateway or VPN.
-END-
Question:559
...
Answer: B,E
Explanation: Use the Billing console to select user-defined cost allocation tag (B) and activate it in the payer (management) account (E).
-END-
Question:560
...
Answer: A
Explanation: AWS Control Tower provisions accounts and provides drift (change) notifications including OU hierarchy changes with minimal ops.
-END-
Question:561
...
Answer: A
Explanation: DAX is purpose-built in-memory cache for DynamoDB, reducing read latency with least management effort.
-END-
Question:562
...
A. Create aroute table entry for the endpoint.
...
Answer: A,B
Explanation: Use a DynamoDB gateway endpoint and add route table entry for the prefix list; traffic stays on AWS network (no Internet).
-END-
Question:563
...
Answer: B
Explanation: EKS Connector registers external/on-prem Kubernetes clusters for unified visibility and inventory in EKS console.
-END-
Question:564
...
Answer: B
Explanation: RDS MySQL with KMS client-side encryption ensures data protected even from DB admins (encryption done before sending to DB).
-END-
Question:565
...
B. Migrate the database to Amazon Redshift by using the mysgldump utility. Turn on Auto Scaling for the Amazon Redshift cluster.
...
Answer: C
Explanation: Aurora MySQL compatible, supports Auto Scaling of read replicas/storage; DMS handles migration with minimal changes.
-END-
Question:566
...
C. Create a file system on a Provisioned 10PS SSD (i02) Amazon Elastic Block Store (Amazon EBS) volume. Attach the EBS volume to all the EC2 instances.
...
Answer: B
Explanation: EFS provides shared, concurrent, low-latency POSIX file system across AZs for many Linux instances.
-END-
Question:567
...
C. Use Amazon API Gateway with AWS Lambda functions to receive the data from the sensors, process the data, and store the data ina Microsoft SQL Server Express database on an Amazon EC2 instance.
...
Answer: A
Explanation: API Gateway + Lambda + DynamoDB is fully managed serverless, low ops, and extensible for future components.
-END-
Question:568
...
Answer: A
Explanation: S3 stores petabyte objects; CloudFront caches drawings at edge reducing latency for repeated views.
-END-
Question:569
...
Answer: A
Explanation: CloudWatch metrics (AWS/Events) show rule Invocations/Triggered counts to verify matching & target invocation.
-END-
Question:570
...
Answer: B
Explanation: Scheduled action scales to 6 instances every Friday then back down; minimal ongoing management.
-END-
Question:571
...
A. Use a local machine to create a certificate that is signed by the third-party Cimport the certificate into AWS Certificate Manager (ACM). Create an HTTP APl in Amazon API Gateway with a custom domain. Configure the custom domain to use the certificate.
B. Create a certificate in AWS Certificate Manager (ACM) that is signed by the third-party CA. Create an HTTP APl in Amazon API Gateway with a custom domain. Configure the custom domain to use the certificate.
...
Answer: A
Explanation: Import third-party CA-signed cert into ACM and attach to API Gateway custom domain supporting TLS 1.3 requirement.
-END-
Question:572
...
Answer: C
Explanation: Aurora Serverless v2 scales ACUs automatically for variable workload, minimizing admin overhead while MySQL-compatible.
-END-
Question:573
...
Answer: D
Explanation: SnapStart snapshots initialized Java runtime to reduce cold start latency cost-effectively versus always-on provisioned concurrency.
-END-
Question:574
...
Answer: A
Explanation: Aurora Serverless v2 scales to zero/near-zero outside 2-hour weekly window; most cost-efficient for sporadic usage.
-END-
Question:575
...
Answer: C
Explanation: RDS Multi-AZ DB cluster provides two writer endpoints with high availability and supports reader replicas for scale.
-END-
Question:576
...
Answer: D
Explanation: Edge-optimized API Gateway uses CloudFront edge network to reduce latency for globally distributed users.
-END-
Question:577
...
Answer: C
Explanation: ACM with DNS validation auto-renews certificates for CloudFront with minimal operational effort.
-END-
Question:578
...
Answer: A
Explanation: DAX is purpose-built in-memory cache for DynamoDB delivering microsecond responses; fully managed, minimal ops.
-END-
Question:579
...
Answer: A
Explanation: Instance Scheduler starts/stops DB to run only during business hours minimizing cost without manual scripting.
-END-
Question:580
...
Answer: D
Explanation: gp3 offers better baseline performance and tunable IOPS at lower cost than gp2 for lift-and-shift low-latency local storage.
-END-
Question:581
...
Answer: A
Explanation: Minimum capacity of two across separate AZs with On-Demand ensures HA & fault tolerance for stateful app.
-END-
Question:582
...
Answer: A
Explanation: Geolocation directs users to closest hosted location (on-prem near us-west-1 vs eu-central-1) minimizing load times.
-END-
Question:583
...
Answer: C
Explanation: Snowball with Tape Gateway ingests large tape archive offline then lifecycle to Glacier Deep Archive cost-effectively.
-END-
Question:584
...
Answer: A
Explanation: Spread placement group ensures instances placed on distinct hardware reducing failure domain sharing.
-END-
Question:585
...
Answer: D
Explanation: Capacity Reservation guarantees required instance capacity availability in failover Region.
-END-
Question:586
...
Answer: B
Explanation: Account must first leave original org then be invited to new organization; can't belong to two simultaneously.
-END-
Question:587
...
Answer: C
Explanation: API Gateway + Kinesis Firehose + Lambda authorizer handles bursty ingest with built-in auth and scalable delivery to S3.
-END-
Question:588
...
Answer: D
Explanation: Daily automated snapshot copy meets 24h RPO/RTO at lowest cost versus continuous replication or cross-Region read replica.
-END-
Question:589
...
Answer: B
Explanation: ElastiCache Redis provides in-memory, durable (AOF/snapshot) shared session store with high availability.
-END-
Question:590
...
A. Create aread replica of the database. Direct the queries to the read replica.
...
Answer: A
Explanation: Read replica offloads monthly reporting reads preserving primary performance with minimal downtime.
-END-
Question:591
...
A. Use the AWS Load Balancer Controller toprovision a Network Load Balancer.
B. Use the AWS Load Balancer Controller toprovision an Application Load Balancer.
...
Answer: B
Explanation: ALB provides L7 routing (path/host) to EKS Ingress targets cost-effectively; NLB lacks HTTP routing features.
-END-
Question:592
...
Answer: D
Explanation: S3 + CloudFront with geo restriction and signed URLs enforces country blocks and per-customer secure access at low cost.
-END-
Question:593
...
Answer: A
Explanation: Multi-AZ Redis replication groups with multi-node shards (cluster mode) give node & AZ resilience and failover.
-END-
Question:594
...
Answer: C
Explanation: Hibernation + warm pools keeps pre-initialized instances for faster startup reducing launch latency.
-END-
Question:595
...
Answer: C
Explanation: Dynamic scaling reacts to sudden unpredictable spikes using metrics (CPU/ALB request count) cost-effectively.
-END-
Question:596
...
Answer: A
Explanation: Aurora Serverless v2 auto-scales capacity seamlessly for unpredictable monthly traffic bursts; cost-efficient.
-END-
Question:597
...
Answer: B
Explanation: Scheduled provisioned concurrency warms containers before daily usage reducing initial latency.
-END-
Question:598
...
Answer: A,C,F
Explanation: File Gateway exposes SMB share to write CSV to S3; Glue crawler builds schema; Athena queries data on demand cost-effectively.
-END-
Question:599
...
Answer: A,C,F
Explanation: Customer responsible for facility power/network (A), physical security (C), and planning extra local capacity (F); AWS manages hardware.
-END-
Question:600
...
Answer: A
Explanation: NLB handles millions RPS TCP with low latency high throughput; ALB/API GW not suited for raw high-performance TCP.
-END-
Question:601
...
Answer: B
Explanation: Aurora read replica of RDS enables minimal downtime migration; promote when caught up for switchover.
-END-
Question:602
...
Answer: C
Explanation: AWS Backup centrally manages backup/restore for many EC2/EBS resources with minimal effort.
-END-
Question:603
...
B. Use the AWS Step Functions Map state in Distributed mode toprocess the data in parallel.
C. Use AWS Glue to process the data in parallel.
D. Use several AWS Lambda functions to process the data in parallel.

Answer: B
Explanation: Distributed Map scales massive parallel S3 list/object processing across child executions serverlessly with minimal management; Inline Map concurrency limits.
-END-
Question:604
A company will migrate 10 PB of data to Amazon S3 in 6 weeks. The current data center has a 500 Mbps uplink tothe internet. Other on-premises applications share the uplink. The company can use 80% of the internet bandwidth for this one-time migration task. Which solution will meet these requirements?
...
Answer: D
Explanation: Available bandwidth (400 Mbps ≈ 50 MB/s) over 6 weeks moves only ~181 TB << 10 PB; offline Snowball scales throughput cost-effectively.
-END-
Question:605
...
Answer: D
Explanation: Cached volume gateway keeps frequently accessed blocks locally (low latency) while bulk data resides in S3 reducing on-prem footprint.
-END-
Question:606
...
B. Store all the objects in S3 Standard with an S3 Lifecycle rule to transition the objects to S3 Standard-Infrequent Access (S3 Standard-1A) after 30 days.
...
Answer: B
Explanation: S3 Standard for initial frequent access then lifecycle to Standard-IA after 30 days lowers storage cost while retaining durability & immediate retrieval.
-END-
Question:607
...
B. Increase the RDS DB instance size. Increase the storage capacity to 24 TiChange the storage type to Provisioned I0PS.
...
Answer: C
Explanation: Offload large blobs to S3; keep only metadata in RDS shrinking DB size, improving performance & lowering cost while preserving HA.
-END-
Question:608
...
Answer: A
Explanation: Attach AWS WAF to ALB using large IP set(s) to allow only registered store IPs; scalable vs NACL limits and simpler than custom auth logic.
-END-
Question:609
...
Answer: B
Explanation: Lake Formation row- & cell-level data filters natively enforce fine-grained access to sensitive columns/rows with least operational overhead.
-END-
Question:610
A company deploys Amazon EC2 instances that run in a VPC. The EC2 instances load source data into Amazon S3 buckets so that the data can be processed in the future. According to compliance laws, the data must not be transmitted over the public internet. Servers in the company's onpremises data center will consume the output from an application that runs on the EC2 instances. Which solution will meet these requirements?
...
Answer: B
Explanation: S3 gateway endpoint keeps traffic off public internet; Direct Connect provides private link to on-prem for processed output; meets compliance.
-END-
Question:611
...
Answer: A
Explanation: Kinesis Data Streams buffers spikes; Lambda consumers scale horizontally preventing 503s and smoothing ingest.
-END-
Question:612
...
Answer: D
Explanation: S3 gateway VPC endpoint allows private access from private subnets without internet/NAT exposure; bucket policy can restrict to endpoint.
-END-
Question:613
...
Answer: B
Explanation: Enabling EKS secrets encryption with KMS integrates natively, automates at rest encryption, minimal code or ops changes.
-END-
Question:614
...
Answer: D
Explanation: Internal ALB fronts app tier; security group permits traffic only from web tier SG isolating access cleanly.
-END-
Question:615
...
Answer: D
Explanation: CloudWatch Container Insights centralizes EKS metrics & logs with minimal setup for microservices.
-END-
Question:616
...
Answer: C
Explanation: GuardDuty continuously detects account, workload & S3 access anomalies; Security Hub aggregates & dashboards findings.
-END-
Question:617
...
Answer: B,E
Explanation: EFS provides shared NFS access in AWS; DataSync agent migrates data incrementally with minimal disruption & verification.
-END-
Question:618
...
Answer: C
Explanation: Multi-AZ FSx improves local availability; AWS Backup daily cross-Region copy with Vault Lock compliance mode enforces 5-year immutability.
-END-
Question:619
...
Answer: C
Explanation: SCP denying CloudTrail modifications prevents any user (even root) in child accounts from altering mandatory trail configuration.
-END-
Question:620
A company is planning to deploy a business-critical application in the AWS Cloud. The application requires durable storage with consistent, lowlatency performance. Which type of storage should a solutions architect recommend to meet these requirements?

A. Instance store volume
B. Amazon ElastiCache for Memcached cluster
C. Provisioned I0PS SSD Amazon Elastic Block Store (Amazon EBS) volume
D. Throughput Optimized HDD Amazon Elastic Block Store (Amazon EBS) volume

Answer: C
Explanation: Provisioned IOPS SSD (io1/io2) delivers consistent low-latency, durable storage for business-critical workloads.
-END-
Question:621
...
Answer: A
Explanation: S3 Cross-Region Replication automatically copies new objects to secondary region with minimal ops.
-END-
Question:622
...
Answer: A,D
Explanation: DynamoDB on-demand scales for spiky access & schema flexibility; S3 + CloudFront serves static single page globally and cost-effectively.
-END-
Question:623
A company uses Amazon AP| Gateway to manage its REST APIs that third-party service providers access. The company must protect the REST APIs from SQL injection and cross-site scripting attacks. What is the MOST operationally efficient solution that meets these requirements?
...
Answer: B
Explanation: AWS WAF directly protects API Gateway REST APIs against SQLi & XSS with least extra components.
-END-
Question:624
A company wants to provide users with access to AWS resources. The company has 1,500 users and manages their access to on-premises resources through Active Directory user groups on the corporate network. However, the company does not want users to have to maintain another identity to access the resources. A solutions architect must manage user access to the AWS resources while preserving access to the onpremises resources. What should the solutions architect do to meet these requirements?
...
Answer: D
Explanation: SAML federation maps AD groups to IAM roles avoiding new identities while centralizing auth in AD.
-END-
Question:625
...
Answer: C
Explanation: Route 53 geolocation routing directs users to region-specific ALB endpoints ensuring correct content per distribution rights.
-END-
Question:626
...
Answer: B
Explanation: DataSync agent online transfer to S3 with built-in integrity verification & scheduling reduces ops.
-END-
Question:627
...
Answer: A
Explanation: Route 53 hosted zones eliminate server management and provide high availability for DNS.
-END-
Question:628
...
Answer: C
Explanation: S3 Storage Lens delivers org-wide metrics including incomplete multipart uploads with minimal setup.
-END-
Question:629
...
Answer: D
Explanation: RDS Blue/Green Deployments create synchronized staging environment for version upgrade, test, then fast switchover.
-END-
Question:630
...
Answer: C
Explanation: Scheduled Fargate task handles 2‑hour job reliably serverlessly (Lambda max 15 min) with lowest ops overhead.
-END-
Question:631
...
Answer: B
Explanation: Neptune graph DB models relationships; Neptune Streams emits change logs for real-time recommendation processing.
-END-
Question:632
...
B. Store the data in an Amazon Elastic Block Store (Amazon EBS) volumel-Mount the EBS volume on the application instances.
...
D. Store the data in an Amazon Elastic Block Store (Amazon EBS) Provisioned I0PS volume shared between the application instances.

Answer: C
Explanation: EFS provides elastic, shared POSIX file system across AZs, supports growth & concurrent modifications.
-END-
Question:633
...
Answer: C
Explanation: Read replica offloads read queries causing bottleneck; Multi-AZ standby cannot serve reads.
-END-
Question:634
...
Answer: C
Explanation: Cross-account bucket access grants each agency secure least-privilege access; avoids public exposure or per-user IAM overhead.
-END-
Question:635
...
Answer: C
Explanation: FSx ONTAP + SnapMirror provides native low-latency replication & same protocols (NFS/SMB) with minimal ops.
-END-
Question:636
...
Answer: C
Explanation: SNS -> SQS decouples & scales; SQS triggers Lambda concurrently; ECS/EKS not needed; SMS unrelated.
-END-
Question:637
...
Answer: B,C
Explanation: Lambda handles bursty unpredictable traffic; DynamoDB key-value scalable <1GB dataset simple queries.
-END-
Question:638
...
Answer: A
Explanation: Presigned URLs grant time-bound secure access without per-user IAM management; lowest operational overhead.
-END-
Question:639
...
Answer: A
Explanation: Uneven traffic often due to sticky sessions; disabling affinity allows ALB round-robin across instances.
-END-
Question:640
...
E. Create a new |AM role with the kms:decrypt permission and attach the execution role to the Lambda function.

Answer: B,D
Explanation: KMS key policy must allow Lambda role decrypt; Lambda execution role needs kms:Decrypt permission via IAM policy.
-END-
Question:641
...
C. Enable Cost and Usage Reports for member accounts. Deliver the reportsto Amazon'S3 Use Amazon Redshift for analysis.
...
Answer: B
Explanation: CUR to S3 + Athena provides scalable serverless SQL analysis; no EMR/Redshift infra, single enablement in management account.
-END-
Question:642
...
Answer: A
Explanation: NLB supports UDP and scales with Auto Scaling group; ALB is HTTP/HTTPS only.
-END-
Question:643
...
Answer: A
Explanation: Store logs in S3 and query on demand with Athena using SQL; pay per query, minimal cost vs always-on clusters.
-END-
Question:644
...
Answer: A,E
Explanation: Public cert for apex and wildcard covers subdomains; DNS validation automates renewal.
-END-
Question:645
...
Answer: B
Explanation: KMS external key store integrates with external key manager vendors keeping keys outside AWS with least ops.
-END-
Question:646
...
Answer: C
Explanation: FSx for Lustre optimized for HPC low-latency shared parallel file access; link to S3 for postprocessing persistence.
-END-
Question:647
...
Answer: A
Explanation: Global Accelerator gives static anycast IPs, health checks, multi-Region failover, low latency no client caching reliance.
-END-
Question:648
...
Answer: B
Explanation: Persistent FSx Lustre provides sustained high throughput & <1ms latency for large datasets accessed concurrently.
-END-
Question:649
An ecommerce company runs a PostgreSQL database on premises. The database stores data by using high I0PS Amazon Elastic Block Store (Amazon EBS) block storage. The daily peak I/0 transactions per second do not exceed 15,000 I0PS. The company wants to migrate the database to Amazon RDS for PostgreSQL and provision disk I0PS performance independent of disk storage capacity. Which solution will meet these requirements MOST cost-effectively?

A. Configure the General Purpose SSD (gp2) EBS volume storage type and provision 15,000 IOPS.
B. Configure the Provisioned IOPS SSD (io1) EBS volume storage type and provision 15,000 I0PS.
...
Answer: C
Explanation: gp3 lets provisioning independent IOPS up to needs at lower cost than io1/io2; gp2 ties IOPS to size.
-END-
Question:650
...
Answer: A
Explanation: RDS SQL Server Enterprise supports read replicas for reporting reducing ops; meets both OLTP & analytics.
-END-
Question:651
...
A. Transition the objects to S3 One Zone-Infrequent Access (S3 One Zone-lA) after 180 days. S3 Glacier Instant Retrieval after 360 days, and S3 Glacier Deep Archive after 5 years.
...
C. Transition the objects to S3 Standard-Infrequent Access (S3 Standard-1A) after 180 days, S3 Glacier Instant Retrieval after 360 days, and S3 Glacier Deep Archive after 5 years.
...
Answer: C
Explanation: Standard first 180d, Standard-IA next 180d, then Glacier Instant Retrieval for instant access after 360d, Deep Archive after 5y for audits.
-END-
Question:652
...
Answer: B
Explanation: Transient cluster for 6h job avoids idle cost; On-Demand primary/core ensure data durability; Spot tasks reduce cost without risking core HDFS.
-END-
Question:653
...
Answer: B
Explanation: EventBridge (CloudTrail) invokes Lambda tagging resources immediately using RDS lookup with least friction.
-END-
Question:654
...
Answer: D
Explanation: CloudFront+S3 for static, Fargate ECS for PHP, ElastiCache Redis for sessions gives HA managed stack.
-END-
Question:655
...
Answer: C,E
Explanation: Public ALB supports sticky sessions; associate WAF web ACL for security.
-END-
Question:656
...
D. Store images in Amazon S3 Standard-Infrequent Access (S3 Standard-lA). Use S3 Standard-IA to directly deliver images by using a static website.

Answer: D
Explanation: S3 Standard-IA suits infrequent yearly access after initial uploads; static website hosting direct delivery cost-effective.
-END-
Question:657
...
C. Create an AWS managed prefix list. Use an AWS Security Hub policy to enforce the security group update across the organization. Use an AWS Lambda function toupdate the prefix list automatically when the CIDR ranges change.
...
Answer: B
Explanation: Shared prefix list centrally updates CIDRs; referenced in SG rules across accounts via RAM minimizing admin overhead.
-END-
Question:658
...
Answer: A,E
Explanation: Cluster placement group minimizes latency; FSx NetApp ONTAP supplies multi-protocol NFS/SMB low latency.
-END-
Question:659
A company is relocating its data center and wants to securely transfer 50 TB of data to AWS within 2 weeks. The existing data center has a Site-toSite VPN connection to AWS that is 90% utilized. Which AWS service should a solutions architect use to meet these requirements?
...
Answer: C
Explanation: Snowball Edge offline transfer bypasses saturated VPN for 50TB within 2 weeks securely.
-END-
Question:660
...
Answer: D
Explanation: Scheduled scaling pre-warms instances before predictable daily peak eliminating initial sluggishness.
-END-
Question:661
...
Answer: B
Explanation: RDS Proxy pools & multiplexes connections reducing database load during app scaling with minimal ops.
-END-
Question:662
...
Answer: D
Explanation: Remove nonessential snapshots then automate lifecycle with DLM; least effort continuous cost control.
-END-
Question:663
...
Answer: C
Explanation: Restrict S3 access by bucket policy to the ECS task execution role and lock RDS access by security group (allow only ECS task subnets). Other options misuse KMS for access control or overly broad endpoint access.
-END-
Question:664
...
Answer: B
Explanation: Compute optimized instances handle sudden CPU spikes better than burstable T instances; request-based scaling reacts to real traffic, unlike schedule or predictive (insufficient data for twice-monthly spikes).
-END-
Question:665
...
Answer: B
Explanation: CloudFormation provides infrastructure as code (automation); AWS Config continuously records and audits configuration changes for tracking.
-END-
Question:666
...
Answer: B,E
Explanation: RDS Multi-AZ (B) adds managed high availability for MySQL without code changes; ALB + Auto Scaling across AZs (E) gives resilient stateless app tier.
-END-
Question:667
...
Answer: A
Explanation: S3 gateway endpoint keeps traffic on AWS network via Direct Connect without traversing the public internet; PrivateLink (interface endpoint) not required for this use case.
-END-
Question:668
...
Answer: D
Explanation: Organizations tag policy centrally defines and enforces allowed tag values (approved application names) across accounts; IAM or resource groups do not enforce tag creation values.
-END-
Question:669
...
Answer: C
Explanation: Secrets Manager integrates with RDS to automate credential rotation (configure 30‑day schedule) with minimal operational work.
-END-
Question:670
...
Answer: A
Explanation: On-demand billing suits infrequent, short weekly tests (pay per request only during 4‑hour run). Provisioned or reserved capacity would incur idle cost the rest of the week.
-END-
Question:671
...
B. Create an AWS Cost Anomaly Detection monitor|in-the- AWS Billing-and Gost Management console.
...
Answer: B
Explanation: Cost Anomaly Detection monitors spend patterns and alerts on unusual spikes, preventing unanticipated cost overruns.
-END-
Question:672
...
Answer: B
Explanation: Glue crawler builds schema; Athena queries data in S3 immediately with serverless model and minimal admin overhead.
-END-
Question:673
...
Answer: B
Explanation: S3 File Gateway presents SMB shares, caches frequently accessed (first 7 days) files locally, and lifecycle transitions to Glacier Deep Archive after 7 days (≤24h retrieval acceptable).
-END-
Question:674
...
Answer: B,D
Explanation: Read replica offloads heavy read traffic; ElastiCache caches frequent query results reducing repeated database reads.
-END-
Question:675
...
Answer: C
Explanation: Tags + Recycle Bin retention rules guard against accidental snapshot deletion without altering admin IAM rights.
-END-
Question:676
...
A. Create alog group in Amazon CloudWatch Logs. Configure VPC Flow Logs to send the log data to the log group. Use Amazon Kinesis Data Streams to stream the logs from the log group to OpenSearch Service.
B. Create a log group in Amazon CloudWatch Logs. Configure VPC Flow Logs to send the log data to the log group. Use Amazon Kinesis Data Firehose tostream the logs from the log group to OpenSearch Service.
...
D. Create a trail in AWS CloudTrail. Configure VPC Flow Logs to send the log data to the trail. Use Amazon Kinesis Data Firehose tostream the logs from the trail to OpenSearch Service.

Answer: B
Explanation: VPC Flow Logs -> CloudWatch Logs then Firehose delivers near real-time streaming directly to OpenSearch Service with minimal custom plumbing.
-END-
Question:677
...
Answer: A
Explanation: Managed node group with Spot Instances minimizes cost for infrequently used dev cluster while retaining managed operations.
-END-
Question:678
...
Answer: B
Explanation: Customer managed KMS key (SSE-KMS) gives full control over creation, rotation, and disabling of encryption keys for S3 objects.
-END-
Question:679
...
Answer: A,C,E
Explanation: Enable Object Lock (A) and set default 30‑day retention (C) to prevent early deletion; lifecycle expiration (E) then removes objects after 30 days.
-END-
Question:680
...
Answer: A
Explanation: DataSync tasks with S3 and EFS locations incrementally transfer only changed files continuously with minimal ops.
-END-
Question:681
...
Answer: A
Explanation: Customer managed KMS key lets you control (enable/disable) rotation and policies while encrypting all EBS volumes.
-END-
Question:682
...
A. Use an |AM policy that allows users to create only encrypted Amazon Elastic Block Store (Amazon EBS) volumes. Use AWS Config and AWS Systems Manager to automate the detection and remediation of unencrypted EBS volumes.
...
C. Use Amazon Macie todetect unencrypted Amazon Elastic Block Store (Amazon EBS) volumes. Use AWS Systems Manager Automation rules to automatically encrypt existing and new EBS volumes.
...
Answer: A
Explanation: IAM policy restricts creation to encrypted volumes; AWS Config detects existing unencrypted volumes; Systems Manager automates remediation.
-END-
Question:683
...
Answer: A,C
Explanation: Auto Scaling group + ALB improves web tier resiliency; RDS Multi-AZ adds managed HA for MySQL with minimal application changes.
-END-
Question:684
...
Answer: B
Explanation: Local Zones extend VPC closer to users for single-digit ms latency when certain apps cannot run in Region due to regulations.
-END-
Question:685
A company is launching a new application deployed on an Amazon Elastic Container Service (ECS) cluster and is using the Fargate launch type for ECS tasks. The company is monitoring CPU and memory usage and is expecting high traffic upon launch. The company wants to reduce costs when utilization decreases. What solution should a Solutions Architect recommend?

A. Use Amazon EC2 Auto Scaling with simple scaling policies.
B. Use AWS Lambda functions to stop tasks when utilization is low.
C. Use Amazon EC2 Auto Scaling with a scheduled scaling policy.
D. Use Application Auto Scaling with Target Tracking policies to scale when Amazon CloudWatch alarms are triggered based on the ECS task CPU utilization metric.

Answer: D
Explanation: Target tracking via Application Auto Scaling adjusts Fargate task count on CPU (and/or memory) automatically to reduce cost when utilization drops.
-END-
Question:686
A company wants to run an in-memory database for a latency-sensitive application that runs on Amazon EC2 instances. The application processes more than 100,000 transactions each minute and requires high network throughput. A Solutions Architect needs to provide a cost-effective network design that minimizes data transfer charges.

A. Launch all EC2 instance in the same Availability Zone within the same AWS region. Specify a placement group with cluster strategy when launching EC2 instances.
B. Launch all EC2 instances in different Availability Zones within the same AWS Region. Specify a placement group with a spread strategy when launching the EC2 instances.
C. Deploy an Auto Scaling group across multiple Regions, configure the launch template to specify a placement group with a partition strategy.
D. Launch all EC2 instances in a single Availability Zone. Use an AWS Direct Connect connection to minimize data transfer charges.

Answer: A
Explanation: Cluster placement in single AZ gives highest throughput/lowest latency for chatty in-memory traffic and reduces inter-AZ transfer costs.
-END-
Question:687
A company running application servers on-premises wants to migrate to AWS. The company wants to minimize its need to scale its on-premises iSCSI storage and only wants its recently accessed data to remain stored locally. Which AWS solution should meet this requirement?

A. AWS Storage Gateway File Gateway.
B. AWS Storage Gateway Tape Gateway.
C. AWS Storage Gateway with Gateway Stored Volumes.
D. AWS Storage Gateway with Gateway Cached Volumes.

Answer: D
Explanation: Gateway Cached Volumes keep recent data locally while storing full dataset in S3, reducing on-prem capacity expansion.
-END-
Question:688
A company has multiple AWS accounts that use Consolidated billing. The company runs several active high-performance Amazon RDS for Oracle on-demand DB instances for 90 days. The finance team has access to AWS Trusted Advisor in the Consolidated billing account and all other AWS accounts. The finance team needs to use the appropriate account to access the Trusted Advisor check recommendations for RDS. The finance team must review the appropriate Trusted Advisor check to reduce RDS costs. Which combination of steps should the finance team take to meet this requirement?

A. Use the Trusted Advisor recommendations from the account where the RDS instances are running.
B. Use the Trusted Advisor recommendations from the Consolidated billing account to see all the RDS instances checks at the same time.
C. View the Trusted Advisor check for Amazon RDS Reserved Instance optimization.
...
Answer: B,D
Explanation: Consolidated (payer) account shows all member RDS checks; Idle DB instances check identifies cost reduction opportunities after 90 days usage review.
-END-
Question:689
A company sells large datasets to customers who do research in AIML. The datasets are stored in an Amazon S3 bucket in the US East-1 Region. Customers, who are distributed across North America and Europe, purchase access and receive an S3 signed URL to download the files. The company wants to reduce the cost associated with data transfers and wants to maintain or improve performance.

A. Configure S3 Transfer Acceleration on the existing S3 bucket.
B. Deploy an Amazon CloudFront distribution with the existing S3 bucket as the origin. Direct customer requests to the CloudFront URL. Switch to CloudFront signed URLs for access control.
C. Set up a second S3 bucket in the EU Central-1 region.
D. Modify the web application to use AWS Transfer Family.

Answer: B
Explanation: CloudFront distribution caches and distributes globally; signed URLs maintain access control and reduce S3 direct transfer costs.
-END-
Question:690
A company has an application that runs on several Amazon EC2 instances. Each EC2 instance has multiple EBS data volumes attached. The application, EC2 instance configuration, and data need to be backed up nightly. The application also needs to be recoverable in a different AWS Region.

A. Write an AWS Lambda function that schedules snapshots and then copies them to another Region.
B. Create a backup plan by using AWS Backup to perform nightly backups. Copy the backups to another Region. Add the application's EC2 instances and resources to the backup plan.
C. Create an AWS Backup plan to perform nightly backups. Add only the EBS volumes to the backup plan.
D. Write an AWS Lambda function that schedules nightly snapshots of the EBS volumes and copies the snapshots to a different Availability Zone.

Answer: B
Explanation: AWS Backup plan covers EC2 (instances + attached EBS) with cross-Region copy automatically; simplest nightly backup + DR.
-END-
Question:691
A company is building a mobile app on AWS and wants to expand its reach to millions of users. The company needs to build a platform so that authorized users can watch the company's content on their mobile devices.

A. Publish to an S3 bucket and use AWS KMS to stream content.
B. Set up an IPsec VPN between the mobile app and AWS.
C. Use Amazon CloudFront. Provide signed URLs to stream the content.
D. Set up AWS Client VPN between the mobile app and AWS.

Answer: C
Explanation: CloudFront + signed URLs securely stream to global mobile users at scale; S3/KMS alone not streaming solution.
-END-
Question:692
A company uses a legacy application to produce data in CSV format and stores the output in Amazon S3. A new application needs to perform complex SQL queries to analyze data stored in Amazon Redshift and Amazon S3 only. The new application cannot process the CSV files that the legacy application produces, and the legacy application cannot be updated to produce a different format. The company needs a solution so that the new application can use the data.

A. Create an AWS Glue ETL job that runs on a schedule. Configure the ETL job to process the CSV files and store the processed data in Amazon Redshift.
B. Develop a Python script that runs on EC2 instances to transform the data.
C. Use an AWS Lambda function to transform the data and save it in Redshift.
D. Use Amazon EventBridge to launch an Amazon EMR cluster on a weekly schedule.

Answer: A
Explanation: Glue ETL job transforms CSV to schema suitable for Redshift; scheduled automation minimal code changes.
-END-
Question:693
A company has hundreds of Amazon EC2 Linux-based instances. System administrators have used SSH keys to manage the instances. After a recent audit, the security team is mandating the removal of all shared keys. A Solutions Architect must design a solution that provides secure access to the EC2 instances.

A. Use AWS Systems Manager Session Manager to connect to the EC2 instances.
B. Use AWS STS to generate one-time SSH keys on demand.
C. Allow shared SSH access to a set of Bastion instances.
D. Configure Amazon Cognito to manage a pool of user credentials.

Answer: A
Explanation: Systems Manager Session Manager removes need for shared SSH keys and provides auditable secure access.
-END-
Question:694
A company wants to implement a Disaster Recovery (DR) plan for its primary on-premises file storage volume. The volume is mounted on an iSCSI interface. The company wants to ensure that end users retain immediate access to all file types from the on-premises system without experiencing latency. The solution must meet the requirements with the least amount of change to the existing infrastructure.

A. AWS Storage Gateway with Tape Gateway.
B. AWS Storage Gateway with Gateway Stored Volumes.
C. AWS Storage Gateway with Gateway Cached Volumes. Set the local cache to 10 TB. Mount the volume Gateway Cached Volume to the existing file server using iSCSI. Copy all the files to the storage volume. Configure scheduled snapshots of the storage volume to recover from a disaster. Restore a snapshot to an EBS volume and attach the EBS volume to an EC2 instance.
D. AWS Storage Gateway with S3 File Gateway.

Answer: C
Explanation: Gateway Cached Volumes over iSCSI keep hot data local with low latency while asynchronously replicating to AWS for DR.
-END-
Question:695
An image hosting company uploads large assets to Amazon S3 Standard buckets. For the first 30 days, the objects will be accessed frequently. After 30 days, access will be less frequent, but inconsistent. The company must optimize S3 storage costs while maintaining high availability and resiliency.

A. Move assets to S3 Intelligent-Tiering after 30 days.
B. Create an S3 life cycle policy to clean up incomplete multi-part uploads.
C. Create an S3 life cycle policy to clean up expired object delete markers.
D. Move assets to S3 Standard-IA after 30 days. E) Move assets to S3 Glacier Flexible Retrieval after 60 days.

Answer: A,B
Explanation: Intelligent-Tiering auto optimizes cost after 30 days; lifecycle cleanup of incomplete multipart uploads lowers unnecessary storage charges.
-END-
Question:696
A Solutions Architect must secure a VPC network that hosts Amazon EC2 instances in a private subnet. The EC2 instances can access only approved third-party software repositories on the internet for updates using the third-parties' URL. Other internet traffic must be blocked.

A. Update the route table for the private subnet to route the outbound traffic to an AWS Network Firewall. Configure the domain list rule groups in the Network Firewall.
B. Update the Security Group rules for the EC2 instances to only allow outbound traffic on port 443 to the third-party IP addresses.
C. Configure an Application Load Balancer to direct all outbound traffic.
D. Deploy AWS WAF on the outbound internet gateway.

Answer: A
Explanation: Network Firewall with domain list egress filtering allows only approved repository domains blocking all other internet traffic.
-END-
Question:697
A security audit reveals that Amazon EC2 instances are not being patched regularly. A Solutions Architect needs a solution that will run regular security scans across a large fleet of EC2 instances, patch the EC2 instances on a regular schedule, and provide a report of each instance's patch status.

A. Set up Amazon Macie to scan the EC2 instances for software vulnerabilities.
B. Set up Amazon GuardDuty to monitor for malicious activity.
C. Use AWS Systems Manager Patch Manager to patch the EC2 instances on a regular schedule.
D. Use Amazon Inspector to scan the EC2 instances for software vulnerabilities. E) Use Amazon Detective to analyze and visualize security data.

Answer: C,D
Explanation: Inspector identifies vulnerabilities; Patch Manager automates scheduled patching and reporting of compliance.
-END-
Question:698
A company needs to provide employees with secure access to confidential files currently on a Windows file server that is running out of capacity. The company wants to move the files to AWS using a fully managed file system that supports the Server Message Block (SMB. protocol. The files must be downloaded securely to employees' devices.
...
Answer: B
Explanation: Amazon FSx for Windows File Server delivers fully managed SMB shares with Windows compatibility and AD integration for secure controlled access.
-END-
Question:699
A company's application runs on Amazon EC2 instances behind an ALB. The instances run in an Auto Scaling group across multiple AZs. On the first day of every month at midnight, the application becomes much slower when the month-end batch runs, causing the CPU utilization to immediately peak to 100%, disrupting the application. What should a Solutions Architect recommend to ensure the application is able to handle the workload and avoid downtime?

A. Use an Elastic Load Balancing simple scaling policy based on the CPU utilization metric.
B. Configure a scheduled scaling policy for the Auto Scaling group to increase the desired capacity at midnight on the first day of every month.
C. Configure a target tracking scaling policy based on the CPU utilization metric.
D. Deploy Amazon CloudFront to cache the content.

Answer: B
Explanation: Scheduled scaling increases desired capacity just before predictable monthly batch load then scales down afterward.
-END-
Question:700
A company wants to give a customer the ability to use on-premises Microsoft Active Directory to download files that are stored in Amazon S3. The customer's application uses an SFTP client to download the files. The solution must meet the requirements with the least operational overhead and no changes to the customer's application.

A. Set up AWS Transfer Family with SFTP for Amazon S3. Configure integrated Active Directory authentication.
B. Set up AWS DMS to replicate data from the on-premises directory.
C. Set up AWS DataSync to transfer files to S3.
D. Set up a Windows Amazon EC2 instance with an SFTP server.

Answer: A
Explanation: AWS Transfer Family provides managed SFTP endpoints backed by S3 and supports Microsoft AD integration; no application changes required.
-END-
Question:701
A company needs to provision large Amazon EC2 instances from an AMI. The instances will run in an Auto Scaling group. The company needs a solution that provides minimum initialization latency to meet the demand.

A. Use AWS EC2 Register Image command to create an AMI from a snapshot.
B. Enable Amazon Elastic Block Store (EBS Fast Snapshot Restore) on the snapshot that is used to provision the AMI. Replace the AMI in the Auto Scaling group with the new AMI.
C. Enable AMI creation and define archival life cycle rules in Amazon Data Lifecycle Manager (DLM).
D. Use Amazon EventBridge to invoke AWS Backup life cycle policies.

Answer: B
Explanation: Enabling Fast Snapshot Restore on snapshot used by AMI accelerates volume hydration reducing instance launch latency.
-END-
Question:702
A company hosts a multi-tier web application that uses an Amazon Aurora MySQL DB cluster. The company's IT security guidelines mandate that the database credentials be encrypted and rotated every 14 days. What should a Solutions Architect do to meet this requirement with the least operational overhead?

A. Create a new AWS Key Management Service (KMS) encryption key. Use AWS Secrets Manager to create a new secret that uses the KMS key with the appropriate credentials. Associate the secret with the Aurora DB cluster. Configure a custom rotation period of 14 days.
B. Create two parameters in AWS Systems Manager Parameter Store.
C. Store a file that contains the credentials in AWS KMS.
D. Store a file that contains the credentials in an encrypted Amazon S3 bucket.

Answer: A
Explanation: Secrets Manager stores encrypted creds with automatic 14-day rotation via KMS CMK minimizing operational overhead.
-END-
Question:703
A company has a web application with a backend on Amazon RDS for MySQL with a primary DB instance and five read replicas. The read replicas must lag no more than 1 second behind the primary DB instance. As traffic increases, the replicas experience additional lag. The Solutions Architect must reduce the replication lag as much as possible, minimize changes to the application code, and minimize ongoing operational overhead.

A. Migrate the database to Amazon Aurora MySQL. Replace the read replicas with Aurora read replicas and configure Aurora auto scaling.
...
Answer: A
Explanation: Aurora MySQL offers faster replication and auto scaling read replicas reducing lag with minimal app changes.
-END-
Question:704
A Solutions Architect must create a Disaster Recovery (DR) plan for a high-volume SaaS platform. All data is stored in an Amazon Aurora MySQL DB cluster. The DR plan must replicate data in a secondary AWS Region. Which solution will meet these requirements most cost-effectively?

A. Use MySQL binary log replication to an Aurora cluster in the secondary Region.
B. Set up an Aurora Global Database for the DB cluster. When the setup is complete, remove the DB instance from the secondary Region.
C. Use AWS Data Migration Service (DMS) to continuously replicate data.
D. Set up an Aurora Global Database for the DB cluster. Specify a minimum of 1 DB instance in the secondary Region.

Answer: D
Explanation: Aurora Global Database requires at least one DB instance in secondary Region to replicate for DR.
-END-
Question:705
A company has a custom application with embedded credentials that retrieves information from an Amazon RDS MySQL DB instance. Management says that the application must be made more secure with the least amount of programming effort.

A. Use AWS KMS to create the keys. Configure the application to load the database credentials from AWS KMS.
B. Create credentials on RDS for MySQL database... and store the credentials in AWS Secrets Manager. Create an AWS Lambda function to rotate the credentials.
C. Create the credentials on RDS for MySQL database... and store the credentials in AWS Secrets Manager. Set up a credentials rotation schedule for the application in the RDS for MySQL database using Secrets Manager.
D. Set up a credentials rotation schedule for the application user using the Parameter Store.

Answer: C
Explanation: Secrets Manager stores and automatically rotates RDS credentials; app retrieves dynamically without code overhaul.
-END-
Question:706
A media company hosts its website on AWS. The architecture includes an ALB and an Aurora database. The company's cyber security team reports that the application is vulnerable to SQL injection. How should the company resolve this issue?

A. Use AWS WAF in front of the ALB.
B. Use an ALB listener rule to reply to SQL injection with a fixed response.
C. Subscribe to AWS Shield Advanced.
D. Deploy Amazon Inspector.

Answer: A
Explanation: AWS WAF rules (managed or custom) mitigate SQL injection at ALB layer without app code changes.
-END-
Question:707
A company has an Amazon S3 data lake governed by AWS Lake Formation. The company wants to create a visualization in Amazon QuickSight by joining the data in the data lake with operational data in Amazon Aurora MySQL database. The company wants to enforce column-level authorization so that the marketing team can access only a subset of the columns. The solution must meet this requirement with the least operational overhead.

A. Use Amazon EMR to ingest the data directly from the database to QuickSight's SPICE engine.
B. Use AWS Glue Studio to ingest data. Attach an IAM policy to QuickSight users to enforce column-level Access Control.
C. Use Glue Elastic Views to create a materialized view for the database in Amazon S3.
D. Use Amazon Athena to query the data lake. Use AWS Lake Formation to enforce column-level authorization and use Amazon QuickSight to visualize the data.

Answer: D
Explanation: Athena + Lake Formation enforces column-level access and QuickSight visualization with low operational overhead.
-END-
Question:708
A transaction Processing Company has weekly scripted batch jobs that run on Amazon EC2 instances in an Auto Scaling group. The company needs to provision the capacity 30 minutes before the job runs. The company does not have the resources to analyze the required capacity trends. The company needs an automated way to modify the desired capacity.

A. Create a dynamic scaling policy for the Auto Scaling group.
B. Create a scheduled scaling policy for the Auto Scaling group.
C. Create a predictive scaling policy for the Auto Scaling group. Configure the policy to scale based on forecast. Set the scaling metric to CPU utilization. Set the instances to pre-launch 30 minutes before the job runs.
D. Create an EventBridge event to invoke an AWS Lambda function.

Answer: C
Explanation: Predictive scaling forecasts recurring weekly batch and pre-scales 30 minutes ahead automatically.
-END-
Question:709
A Solutions Architect is designing a company's Disaster Recovery (DR) architecture. The company has a MySQL database that runs on an Amazon EC2 instance in a private subnet. The DR design needs to include multiple AWS Regions. The solution must meet the requirements with the least operational overhead.

A. Migrate the MySQL to multiple EC2 instances.
B. Migrate the MySQL database to Amazon RDS. Use a Multi-AZ deployment.
C. Migrate the MySQL database to Amazon Aurora Global Database. Host the primary DB cluster in a primary Region. Host the secondary DB cluster in the DR Region.
D. Store the scheduled backup of the MySQL database on an Amazon S3 bucket.

Answer: C
Explanation: Aurora Global Database provides cross-Region replication/DR with low write latency impact and minimal ops.
-END-
Question:710
A company has a Java application that uses Amazon SQS to pass messages. The application cannot pass messages larger than 256 KB in size. The company wants to implement a solution to give the application the ability to pass messages as large as 50 MB. The solution must meet this requirement with the fewest changes to the code.

A. Use Amazon EventBridge to post large messages.
B. Use the Amazon SQS Extended Client Library for Java to store messages that are larger than 256 KB in Amazon S3.
C. Change the maximum message size of the SQS queue to 50 MB.
D. Store the messages larger than 256 KB in EFS.

Answer: B
Explanation: SQS Extended Client transparently stores payloads >256KB in S3 with minimal code changes.
-END-
Question:711
A company wants to restrict access to its main web application content and serve it globally using a serverless architecture and an authentication solution. The solution must provide the lowest login latency possible.

A. Use Amazon Cognito for authentication. Use Lambda@Edge for authorization. Use CloudFront to serve the web application globally.
B. Use AWS Directory Service for authentication.
C. Use Amazon Cognito for authentication. Use an AWS Lambda function for authorization. Use S3 Transfer Acceleration.
D. Use AWS Directory Service for authentication. Use an AWS Lambda function for authorization.

Answer: A
Explanation: Cognito for auth + Lambda@Edge for authorization + CloudFront global edge distribution gives lowest login latency.
-END-
Question:712
A company has an aging Network Attached Storage (NAS) array that presents SMB file shares and NFS shares. The company wants to migrate the data to Amazon S3, use S3 life cycle policies, and maintain the same look and feel for the client workstations. Which type of Storage Gateway should the Solutions Architect provision?

A. AWS Storage Gateway with Volume Gateway (stored volumes).
B. AWS Storage Gateway with Tape Gateway.
C. AWS Storage Gateway with Volume Gateway (cached volumes).
D. AWS Storage Gateway with S3 File Gateway.

Answer: D
Explanation: S3 File Gateway presents SMB/NFS shares backed by S3 enabling lifecycle policies while preserving user access semantics.
-END-
Question:713
A Solutions Architect has standardized the company on a particular EC2 instance family and various instance sizes. The company wants to maximize the cost savings for the application over the next 3 years. The company needs to be able to change the instance family and sizes in the next 6 months based on usage. Which solution will meet the requirements most cost-effectively?
...
Answer: A
Explanation: Compute Savings Plan offers flexibility to change families and sizes while maximizing 3-year savings.
-END-
Question:714
A company stores data in an Amazon DynamoDB table. The data workload is constant and predictable. The company wants to stay at or below its forecasted budget for DynamoDB.

A. Use provisioned mode for DynamoDB standard infrequent access.
B. Use provisioned mode for the DynamoDB table. Specify the Read Capacity Units (RCUs) and Write Capacity Units (WCUs) based on the predicted constant workload.
C. Use on-demand mode.
D. Use on-demand mode and enable DynamoDB Auto Scaling.

Answer: B
Explanation: Predictable steady workload best served by provisioned capacity explicitly set to forecasted RCUs/WCUs.
-END-
Question:715
A company stores confidential data in an Amazon Aurora PostgreSQL database in AP Southeast 3 region. The database is encrypted with AWS KMS customer managed key (CMK). The company was recently acquired and must securely share a backup of the database with the acquiring company's AWS account in AP Southeast 3. What should a Solutions architect do to meet this requirement? Options:

A. Create a database snapshot. Copy the snapshot to a new unencrypted snapshot. Share the new snapshot with the acquiring company's AWS account.
B. Create a database snapshot. Add the acquiring company's AWS account to the KMS key policy. Share the snapshot with the acquiring company's AWS account.
C. Create the database snapshot with a different AWS managed KMS key and all this stuffs.
D. Create a database snapshot. Download the database snapshot. Upload the database snapshot to an Amazon S3 bucket.

Answer: B
Explanation: Add acquiring account to CMK key policy then share encrypted snapshot so recipient can decrypt without re-encryption.
-END-
Question:716
A company uses a 100 GB Amazon RDS for MySQL server single AZ DB instance in us-east-1 region to store customer transactions. The company needs high availability and automatic recovery for the DB instance. The company must also run reports on the RDS database several times a year. The report processes cause transactions to take longer than usual to process the customers' accounts. The company needs a solution that will improve the performance of the report process. Which combination of steps will meet the requirement? Options: (Note: Options are fragmented in the transcript. The correct option encompasses both Multi-AZ and Read Replicas.)

A. Modify the DB instance from a single AZ to multi AZ. Take a snapshot of the current DB instance. Restore the snapshot...
B. Modify the DB instance from single AZ to Multi-AZ. Create a Read Replica for the DB instance in a different Availability Zone and point all requests for the reports to the Read Replica.
C. Migrate the database to RDS Custom.
D. Use RDS proxy to limit reporting request to maintenance window.

Answer: B
Explanation: Multi-AZ deployment provides high availability and automatic recovery. A Read Replica offloads the resource-intensive reporting queries from the primary transaction database, improving performance.
-END-
Question:717
A company is moving its data management application to AWS. The company wants to transition to an event-driven architecture. The architecture needs to be more distributed and to use serverless concepts while performing the different aspects of the workflow. The company also wants to minimize operational overhead. Which solution will meet these requirements? Options:

A. Build out the workflow in AWS Glue. Use AWS Glue to invoke AWS Lambda functions to process the workflow steps.
B. Build the workflow in a Step Functions. Deploy the application on Amazon EC2...
C. Build out the workflow in Amazon EventBridge.
D. Build the workflow in AWS Step Functions. Use the Step Function to create a state machine and use the state machine to invoke AWS Lambda functions to process the workflow.

Answer: D
Explanation: Step Functions state machine orchestrates Lambda tasks serverlessly for event-driven workflow with minimal ops.
-END-
Question:718
A company is designing the network for an online multiplayer game. The game uses UDP networking protocol and will be deployed in eight AWS regions. The network architecture needs to minimize latency and packet loss to give end users a high-quality gaming experience. Which solution will meet this requirement? Options:

A. Set up a Transit Gateway in each region... and create inter-region peering attachments.
B. Use AWS Global Accelerator to route UDP traffic over the AWS global network infrastructure.
C. Use Amazon CloudFront for content distribution.
D. Use VPC peering.

Answer: B
Explanation: Global Accelerator optimizes UDP routing across AWS global backbone reducing latency and packet loss for gamers.
-END-
Question:719
A company hosts a three-tier web application on Amazon EC2 instances on a single Availability Zone. The web application uses a self-managed MySQL database that is hosted on an EC2 instance to store data in an EBS volume. The data currently uses a 1 TB provisioned IOPS SSD (io2) EBS volume. The company expects a traffic of 1,000 IOPS for both reads and writes at peak traffic. The company wants to minimize any disruptions and stabilize performance and reduce cost while retaining the capacity for double the IOPS. The company wants to move the database to a fully managed solution that is highly available and fault tolerant. Options:

A. Multi-AZ deployment of Amazon RDS for MySQL DB instance with an io2 Block Express EBS volume.
B. Multi-AZ deployment of an Amazon RDS for MySQL DB instance with a General Purpose SSD (gp2) EBS volume.
C. S3 Intelligent Tiering...
D. Use two large EC2 instances to host the database in active-passive mode.

Answer: B
Explanation: RDS MySQL Multi-AZ with gp2 volume gives managed HA and baseline 3k IOPS at lower cost satisfying current/expected load.
-END-
Question:720
A company hosts a serverless application on AWS. The application uses Amazon API Gateway, AWS Lambda, and an Amazon RDS for PostgreSQL database. The company notices an increase in application errors that results from database connection timeouts during times of peak traffic or unpredictable traffic. The company needs a solution that reduces the application failures with the least amount of change to the code. Options:

A. Reduce the Lambda concurrency rate.
B. Resize the RDS DB instance class.
C. Migrate the database to Amazon DynamoDB.
D. Use an Amazon RDS Proxy to manage database connections and connection pooling from the Lambda functions.

Answer: D
Explanation: RDS Proxy pools/reuses DB connections for spiky Lambda concurrency mitigating timeouts with minimal code change.
-END-
Question:721
A company is migrating an old application to AWS. The company runs a batch job every hour and is CPU intensive. The batch jobs take 15 minutes on average with on-premises server. The server has 64 virtual CPUs (vCPU) and 512 GB of memory. Which solution will run with the least operational overhead? Options:

A. AWS Lambda.
B. Elastic Container Service with AWS Fargate.
C. Amazon Lightsail with Auto Scaling.
D. AWS Batch on Amazon EC2.

Answer: D
Explanation: AWS Batch handles large periodic CPU-intensive jobs provisioning compute automatically lowering operational overhead.
-END-
Question:722
A company stores its data objects in Amazon S3 Standard storage. A Solutions architect has found that 75% of the data is rarely accessed after 30 days. The company needs all the data to remain immediately accessible with the same high availability and resiliency but the company wants to minimize storage costs. Which storage solution will meet these requirements? Options:

A. Move the data objects to S3 Glacier Deep Archive after 30 days.
B. Move the data objects to S3 Standard-Infrequent Access (S3 Standard-IA. after 30 days.
C. Use S3 One Zone-Infrequent Access.
D. Move the data objects to S3 One Zone-Infrequent Access immediately.

Answer: B
Explanation: S3 Standard-IA keeps immediate availability & resilience while lowering cost for data rarely accessed after 30 days.
-END-
Question:723
A gaming company is moving its public scoreboard from a data center to AWS Cloud. The company uses Amazon EC2 Windows Server instances behind an Application Load Balancer to host its dynamic application. The company needs a highly available storage solution for the application. The application consists of static files and dynamic server-side code. Which combination of steps should a Solutions architect take to meet these requirements? (Choose two) Options:

A. Store the static files on Amazon S3. Use Amazon CloudFront to cache objects at the edge.
B. Store the static files on Amazon S3. Use Elasticache to cache the objects at the edge.
C. Store the server-side code on EFS. Mount the EFS volume on each instance to share the files.
D. Use an Amazon FSx for Windows File Server volume to share the files.
E. Store the server-side code on a general-purpose SSD.

Answer: A,D
Explanation: A is the best practice for highly available, performant serving of static content.
D is the best solution for highly available, shared file storage compatible with Windows Server EC2 instances.
-END-
Question:724
A social media company runs its application on Amazon EC2 instances behind an Application Load Balancer. The ALB is the origin for an Amazon CloudFront distribution. The application has more than a billion images stored on Amazon S3 bucket and processes thousands of images each second. The company wants to resize the images dynamically and serve appropriately for appropriate formats to the clients. Options:

A. Install an external image management library on EC2 instance...
B. Create a CloudFront origin request policy. Use the policy to automatically resize...
C. Use Lambda@Edge functions with an external image management library associated with the Lambda@Edge function with the CloudFront distribution.
D. Create a CloudFront response headers policy. Use the policy to automatically...

Answer: C
Explanation: Lambda@Edge executes image transformation logic at CloudFront edge locations for low-latency dynamic resizing and format optimization.
-END-
Question:725
A hospital needs to store patient records in an Amazon S3 bucket. The hospital's compliance team must ensure that all Protected Health Information (PHI) is encrypted in transit and at rest. The compliance team must administer the encryption key for data at rest. Which solution will meet these requirements? Options:

A. Create a public SSL/TLS certificate in ACM... Configure default encryption for each S3 bucket to use server-side encryption with Amazon KMS Keys (SSE-KMS). Assign the compliance team to manage the KMS Keys.
B. Use AWS secure transport condition on S3 bucket policies to allow only encrypted connections over HTTPS/TLS. Configure the default encryption for S3 bucket to use SSE-S3.
C. Use AWS secure transport condition on S3 bucket policies to allow only encrypted connection over HTTPS/TLS. Configure default encryption for each S3 bucket to use server-side encryption with AWS KMS Keys (SSE-KMS) and assign the compliance team to manage the keys.
D. Use AWS secure transport... Amazon Macie to protect the sensitive data.

Answer: C
Explanation: Enforce TLS in transit via aws:SecureTransport condition; use SSE-KMS with customer managed CMK for at-rest encryption and key admin.
-END-
Question:726
A company uses Amazon API Gateway to run a private Gateway with two REST APIs in the same VPC. One web service calls another web service. The company has noticed that the APIs communicate over the internet. A Solutions architect must implement a solution so that the APIs communicate through the VPC instead of the public internet. Options:

A. Use an X-API-HEADER...
B. Create a VPC Gateway Endpoint for API Gateway.
C. Use Amazon Simple Queue Service (SQS)...
D. Create a VPC Interface Endpoint for API Gateway in the VPC.

Answer: D
Explanation: Private REST APIs communicate within VPC via Interface Endpoint; gateway endpoints only support S3/DynamoDB.
-END-
Question:727
A company hosts a multiplayer gaming application on AWS. The company wants the application to read data with submillisecond latency and run one-time queries on historical data. Which solution will meet these requirements with the least operational overhead? Options:

A. Use DynamoDB with DynamoDB Accelerator (DAX) for data that is frequently accessed. Export the data to S3 bucket by using DynamoDB table export. Run one-time queries on the data in Amazon S3 by using Amazon Athena.
B. Use DynamoDB... turn on streaming to Kinesis Data Stream.
C. Use Amazon RDS...
D. Store the data directly in Amazon S3...

Answer: A
Explanation: DynamoDB with DAX provides the required sub-millisecond latency for real-time gaming data. DynamoDB Export to S3 combined with Athena is the most cost-effective and low-overhead method for running historical ad-hoc queries on the data.
-END-
Question:728
A company uses a payment processing system that requires messages for a particular payment ID to be received in the same order that they are sent otherwise the payments might be processed incorrectly. Which two actions should a Solutions architect take to meet this requirement? (Choose two) Options:

A. Write the messages to Amazon DynamoDB table with the payment ID as a partition key.
B. Write the messages to Amazon Kinesis Data Stream with the payment ID as the partition key.
C. Write the messages to an Elasticache for Memcached.
D. Write the messages to an Amazon Simple Queue Service (SQS) Standard Queue.
E. Write the messages to an Amazon Simple Queue Service (SQS) FIFO Queue.

Answer: B,E
Explanation: Both SQS FIFO (E) and Kinesis Data Streams (B. are services that guarantee the order of messages/records. Using a Partition Key (B. or a Message Group ID (implied for FIFO) ensures that messages for the same payment ID are always processed in order.
-END-
Question:729
A company is building a game system that needs to send unique events to a separate leaderboard, matchmaking, and authentication Services concurrently. The company needs an AWS event-driven system that guarantees the order of the event. Options:

A. Amazon EventBridge event bus...
B. Simple Notification Service (SNS) FIFO Topics.
C. Simple Notification Service (SNS) Standard Topics.
D. Use SQS...

Answer: B
Explanation: SNS FIFO Topics is the ideal solution as it supports fan-out to multiple subscribers (concurrently) while strictly guaranteeing message ordering (FIFO).
-END-
Question:730
A hospital has decided to use Amazon Simple Queue Service (SQS) and Amazon Simple Notification Service (SNS) in the architecture. Data must be encrypted at rest and in transit. Only authorized personnel of the hospital should be able to access the data. Which combination of steps should the solutions architect take to meet the requirements? (Choose two) Options:

A. Turn on server-side encryption on the SQS components. Update a default key policy to restrict key usage...
B. Turn on server-side encryption on the SNS components by using an AWS KMS customer managed key. Apply a key policy to restrict key usage to a set of authorized principles.
C. Turn on encryption on the SNS components. Update the default key policy...
D. Turn on server-side encryption on SQS components by using an AWS KMS customer managed key. Apply a key policy to restrict key usage to a set of authorized principles.
E. Use an IAM policy to restrict key usage...

Answer: B,D
Explanation: Both SQS and SNS must be encrypted at rest using a KMS Customer Managed Key (CMK). A Key Policy on the CMK is the mechanism used to restrict access to only the authorized personnel.
-END-
Question:731
A company runs a web application that is backed by Amazon RDS. A database administrator caused data loss by accidentally editing information in a database table. To help recover from this type of incident, the company wants the ability to restore the database to its state from 5 minutes before any change within the last 30 days. What feature should the solutions architect include in the design to meet the requirements? Options:
...
Answer: C
Explanation: Automated Backups (which include transaction logs) enable Point-in-Time Recovery (PITR), allowing the database to be restored to any second within the retention window (e.g., 5 minutes before the incident).
-END-
Question:732
A company runs a web application that consists of an Amazon API Gateway API... The company uses Amazon Cognito user pools to identify the individual users. A Solutions architect needs to update the application so that only users who have a subscription can access the premium content. Options:

A. Enable API caching and throttling on the API Gateway API.
B. Set up a Web Application Firewall (WAF) on the API Gateway API. Create a rule to filter users' subscription.
C. Apply fine-grained IAM permissions to the premium content in the DynamoDB table.
D. Implement API usage plans and API keys to limit the access of the users who do not have a subscription.

Answer: D
Explanation: API Usage Plans and API Keys are the features within API Gateway used to meter and control access to specific API stages or methods, making it the most straightforward solution for subscription-based access control.
-END-
Question:733
An application is hosted on on-premises data centers. It uses Route 53 latency based routing and UDP based application. The company wants to improve the performance and availability of the application. Options:

A. Configure three Network Load Balancers (NLB.. Deploy that in three AWS regions to address the on-premises end points. Create an accelerator by using AWS Global Accelerator and register the NLB as its end points.
B. Use Application Load Balancers (ALB..
C. Configure three Network Load Balancers. Use it as an origin for CloudFront distribution.
D. Use Application Load Balancers (ALB..

Answer: A
Explanation: Global Accelerator provides static IP addresses and routes traffic over the low-latency AWS network backbone. NLBs are required because they are the only AWS load balancer that supports UDP traffic (Layer 4).
-END-
Question:734
A Solutions architect wants all new users to have specific complexity requirements and mandatory rotation periods for IAM user passwords. What solution should the Solutions architect do to accomplish this? Options:

A. Set an overall password policy for the entire AWS account.
B. Set a password policy for each IAM user.
C. Use third-party vendors.
D. Attach an Amazon CloudWatch rule to create a new user and event.

Answer: A
Explanation: Setting an AWS IAM Password Policy for the root account automatically enforces password rules (complexity, rotation, etc.) for all IAM users across the entire account.
-END-
Question:735
An EC2 instance runs several 1-hour tasks on a schedule. These tasks were written by different teams and have no common programming language. The company is concerned about performance and scalability. A Solutions architect needs to implement a solution. Options:

A. Use AWS Batch to run the tasks as Jobs. Schedule the jobs by using Amazon EventBridge.
B. Convert the EC2 to a container... then use App Runner...
C. Copy the task into a Lambda function...
D. Create AMI of the EC2 instance... create an Auto Scaling Group...

Answer: A
Explanation: AWS Batch is ideal for coordinating and running heterogeneous, resource-intensive jobs. EventBridge (formerly CloudWatch Events) can be used to schedule the batch jobs (Cron-style scheduling).
-END-
Question:736
EC2 instances that run in private subnets need to communicate with a licensed server over the Internet. The company needs a managed solution that minimizes operational maintenance. Which solution will meet the requirement? Options:

A. Provision a NAT Gateway in the public subnet. Modify each private subnet's route table with a default route that points to the NAT Gateway.
B. Use a NAT instance...
C. Provision a NAT Gateway in the private subnet...
D. Use a NAT instance...

Answer: A
Explanation: A NAT Gateway is the fully managed and highly available solution that enables instances in private subnets to initiate outbound connections to the internet (for the license server) while preventing inbound connections. The NAT Gateway must reside in a public subnet.
-END-
Question:737
An EKS cluster will use a managed node group that is backed by EBS volumes. The company must encrypt all data at Rest by using a customer managed key (CMK) that is stored in AWS KMS. Which combination of actions will meet this requirement with the least operational overhead? (Choose two) Options:

A. Use a Kubernetes plugin...
B. After creation of EKS cluster, locate the EBS volumes and enable encryption manually.
C. Enable EBS encryption by default in the AWS region where the EKS cluster will be created. Select the CMK as a default key.
D. Create an IAM role that has a policy that grants permission to the CMK. Associate the role with the EKS cluster.
E. Store the customer managed key as a Kubernetes Secret...

Answer: C,D
Explanation: C automatically enforces the correct encryption key for all new EBS volumes created in the region, minimizing overhead.
D ensures that the EKS node group's IAM role has the necessary permissions to use the CMK for encrypting and decrypting the attached EBS volumes.
-END-
Question:738
A database contains millions of GIS images (high resolution) that are identified by Geographic code. Tens of thousands of images get updated every few minutes. The company wants a solution that is highly available and scalable. Options:

A. Store the images and Geographic codes in a database table. Use Oracle running on Amazon RDS.
B. Store the images in Amazon S3 buckets. Use Amazon DynamoDB with Geographic code as the key and image S3 URL as the value.
C. Store the images and Geographic code in Amazon DynamoDB table.
D. Store the images in S3 bucket. Store Geographic codes and image S3 URLs in a database table. Use Oracle running on Amazon RDS.

Answer: B
Explanation: This is a classic separation-of-concerns problem: S3 is the most scalable and cost-effective solution for storing the large image objects. DynamoDB is a highly available and scalable NoSQL database ideal for storing the frequently updated, low-latency metadata (the geographic code and the S3 URL).
-END-
Question:739
A company collects data that produces trillions of S3 objects each year. Access pattern: Daily access (0-30 days), Quarterly access (30 days-1 year). Data must be available with minimum delay for up to 1 year. After 1 year, data is archived. What storage solution meets the requirements most cost effectively? Options:

A. Use S3 Intelligent Tiering...
B. Use S3 Infrequent Access (S3-IA. storage class immediately...
C. Use S3 Standard (for 0-30 days) and S3 Glacier Deep Archive (after 30 days)...
D. Start with S3 Standard storage (0-30 days). Create an S3 life cycle policy to transition objects to S3 Standard-Infrequent Access (S3-IA. after 30 days, and then to S3 Glacier Deep Archive after 1 year.

Answer: D
Explanation: This lifecycle policy matches the known access patterns: S3 Standard (frequent, daily access 0-30 days) → S3-IA (infrequent, quarterly access 30 days-1 year) → S3 Glacier Deep Archive (archival after 1 year). This is more cost-effective than using S3 Intelligent-Tiering for known patterns.
-END-
Question:740
Applications in three separate VPCs need to communicate with a latency-sensitive application that runs in a single on-premises data center. Needs to consistently send hundreds of gigabytes of data each day and maximize cost effectiveness. Options:

A. Configure three Site-to-Site VPN connections from the data center to AWS...
B. Launch a third-party virtual Network Appliance in each VPC...
C. Set up three Direct Connect connections from the data center to the Direct Connect Gateway...
D. Establish a single AWS Direct Connect connection from the data center to AWS. Create a Transit Gateway and attach each VPC to the Transit Gateway. Establish connectivity between the Direct Connect connection and the Transit Gateway.

Answer: D
Explanation: Direct Connect provides the high bandwidth and low latency required for consistent, large-volume data transfer to on-premises. Transit Gateway acts as a central hub, efficiently connecting all three VPCs to each other and to the Direct Connect connection using a single, cost-effective setup.
-END-
Question:741
A distributed application involves several serverless functions and requires manual approvals as part of the workflow. The solution must be able to combine multiple AWS Lambda functions and orchestrate data and services (Lambda, EC2, containers). Options:

A. Use AWS Step Functions.
B. Use AWS Glue.
C. Use SQS...
D. Use Lambda functions and EventBridge events...

Answer: A
Explanation: AWS Step Functions is the serverless orchestrator that uses state machines to define and manage multi-step workflows, including integrating manual approval steps and coordinating diverse compute services (Lambda, EC2, ECS).
-END-
Question:742
Most connections to an RDS for MySQL DB instance come from serverless applications (LambdA.. At times of high demand, users report database connection rejection errors. Options:

A. Use an Amazon RDS Proxy.
B. Use Elasticache.
C. Migrate the DB instance to a different instance class that has higher IO capacity.
D. Configure Multi-AZ.

Answer: A
Explanation: RDS Proxy prevents connection rejection errors by maintaining a pool of established database connections and efficiently multiplexing application connections (like those from LambdA. to reuse them.
-END-
Question:743
All instances provisioned through an EC2 Auto Scaling Group must successfully send reports to the auditing system as soon as they are launched and terminated. Options:

A. Use a scheduled AWS Lambda function and run a script remotely...
B. Use EC2 Auto Scaling life cycle hooks to run a custom script to send data to the audit system when instances are launched and terminated.
C. Use an Auto Scaling launch configuration to run a custom script through user data.
D. Run a custom script on the instance operating system...

Answer: B
Explanation: Auto Scaling Life Cycle Hooks are the built-in mechanism designed to pause instance launch or termination to allow custom actions (like running an auditing script) to be executed at both the launch and terminate phases.
-END-
Question:744
A real-time multiplayer game uses UDP for communications. Developers want to store non-relational data (gamer scores) in a database solution that will scale without intervention. Options:

A. Use Amazon Route 53 for traffic distribution and Amazon Aurora Serverless for data storage.
B. Use a Network Load Balancer (NLB. for traffic distribution. Use Amazon DynamoDB On-Demand for data storage.
C. Use a Network Load Balancer (NLB. for traffic distribution. Use Amazon Aurora for data storage.
D. Use an Application Load Balancer (ALB. to traffic distribution and Amazon DynamoDB.

Answer: B
Explanation: An NLB is required because it is the only load balancer that supports UDP (Layer 4) traffic. DynamoDB On-Demand is the fully managed, non-relational database that scales automatically and provides the low-latency performance required for real-time game data.
-END-
        `;

// Adjust q-container top margin based on q-header height
function adjustContainerOffset() {
    const header = document.querySelector('.q-header');
    const container = document.querySelector('.q-container');
    if (!header || !container) return;
    const h = header.getBoundingClientRect().height;
    // Add small gap below header
    container.style.marginTop = (h + 16) + 'px';
}
window.addEventListener('load', adjustContainerOffset);
window.addEventListener('resize', adjustContainerOffset);
// If fonts load async
if (document.fonts) {
    document.fonts.addEventListener('loadingdone', adjustContainerOffset);
}
// Dynamically load all questions from q.txt (parsed)
// For performance, only load first 2000 questions (can be increased)
// If you want all 8991, consider server-side rendering or pagination
// Load questions from q.txt (inserted as raw string)
// If you use server-side loading, replace this with AJAX/fetch
...
function parseQuestions(raw) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^Question #\d+/.test(line)) {
            ...
            q = { question: '', options: [], answer: '', multi: false };
            questionActive = true;
            optionActive = false;
            continue;
        }
        if (q && /^[A-F]\. /.test(line)) {
            ...
            optionActive = true;
            q.options.push(line);
        } else if (q && line.startsWith('Answer:')) {
            let ans = line.replace('Answer:', '').trim();
            // Multi-select if answer contains comma or multiple letters
            if (/^[A-F](,[A-F])+$/.test(ans) || (ans.length > 1 && /^[A-F]+$/.test(ans))) {
                q.multi = true;
                // Normalize to array
                if (ans.includes(',')) {
                    q.answer = ans.split(',').map(a => a.trim());
                } else {
                    q.answer = ans.split('').map(a => a.trim());
                }
            } else {
        ...
        } else if (questionActive && q && line.length > 0 && !/^[A-F]\. /.test(line) && !line.startsWith('Answer:')) {
            if (q.question.length > 0) q.question += ' ';
            q.question += line;
        } else if (optionActive && q && line.length > 0 && !/^[A-F]\. /.test(line) && !line.startsWith('Answer:')) {
...
// New function: parseQuestions1
// Parses raw question text into objects with 'question', 'options', 'answer', 'multi', and 'explanation' fields
function parseQuestions1(raw) {
    const lines = raw.split(/\r?\n/);
    const questions = [];
    let q = null;
    let questionActive = false;
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        if (line.length === 0 || line.startsWith('-END-')) continue; // Skip empty lines
        // Start of a new question
        if (/^Question:? ?\d+/.test(line) || /^Question:? ?#?\d+/.test(line)) {
            if (q && q.options.length > 0) questions.push(q);
            q = { question: '', options: [], answer: '', multi: false, explanation: '', id: qsCount++ };
            questionActive = true;
            optionActive = false;
            explainActive = false;
            continue;
        ...
        // Option line
        if (q && (/^[A-Z]\. /.test(line)) || /^[A-Z]\./.test(line)) {
            questionActive = false;
            optionActive = true;
            explainActive = false;
            q.options.push(line);
            ...
            explainActive = false;
        } else if (q && line.startsWith('Explanation:')) {
            explainActive = true;
            optionActive = false;
            questionActive = false;
            q.explanation = line.replace('Explanation:', '').trim();;
        } else if (explainActive && q && line.length > 0 && !/^[A-F]\. /.test(line) && !line.startsWith('Answer:') && !(/^Question:? ?\d+/.test(line) || /^Question:? ?#?\d+/.test(line))) {
            if (q.explanation.length > 0) q.explanation += '\n';
            q.explanation += line;
        } else if (questionActive && q && line.length > 0 && !/^[A-F]\. /.test(line) && !line.startsWith('Answer:') && !line.startsWith('Explanation:')) {
            if (q.question.length > 0) q.question += ' ';
            q.question += line;
        } else if (optionActive && q && line.length > 0 && !/^[A-F]\. /.test(line) && !line.startsWith('Answer:') && !line.startsWith('Explanation:')) {
            // Append to last option
            if (q.options.length > 0) {
                q.options[q.options.length - 1] += ' ' + line;
            }
...
const questions = localStorage.getItem('questions') ? JSON.parse(localStorage.getItem('questions')) : parseQuestions1(raw);
localStorage.setItem('questions', JSON.stringify(questions));
const attemptedAnswers = localStorage.getItem('attemptedAnswers') ? JSON.parse(localStorage.getItem('attemptedAnswers')) : [];
...
function updateQuestionTimerDisplay() {
    ...
    if (el) {
        el.textContent = `${questionElapsedSeconds}s`;
    }
}

function renderQuestion(idx = current) {
    if (!questions[idx]) {
        document.getElementById('questionArea').innerHTML = '<div class="error">No question found.</div> <a href="index.html">Go back</a>';
        document.getElementById('prevBtn').disabled = true;
        document.getElementById('nextBtn').disabled = true;
        document.getElementById('guessBtn').disabled = true;
        document.getElementById('showBtn').disabled = true;
        document.getElementById('resetAllAnsBtn').disabled = true;
        document.getElementById('resultMsg').textContent = '';
        // Stop timer if no question
    ...
    q.options.forEach((opt, i) => {
        if (q.multi) {
            html += `<label class="option"><input type="checkbox" name="option" value="${String.fromCharCode(65 + i)}"> ${opt}</label>`;
        } else {
            html += `<label class="option"><input type="radio" name="option" value="${String.fromCharCode(65 + i)}"> ${opt}</label>`;
        }
    });
    html += '</div>';

    let currentAnswers = JSON.parse(localStorage.getItem('attemptedAnswers')) || {};
    currentAnswers = currentAnswers[q.id] || {};

    document.getElementById('question-header').innerHTML = `<b>Q# ${current + 1}/${totalQ}</b>`;
    document.getElementById('guessed-ans').innerHTML = `${currentAnswers.ansStatus === 'CT' ? '<span style="color:#0F0">Correct</span>' : currentAnswers.ansStatus === 'WG' ? '<span style="color:#F00">Wrong</span>' : ''}`;
    ...
    document.getElementById('resetAllAnsBtn').disabled = false;
    ...
    // Show Explain button if already correct or show answer was toggled
    maybeShowExplainBtn();
...
document.getElementById('jumpBtn').onclick = function () {
    current = parseInt(document.getElementById('jumpInput').value) - 1;
    renderQuestion();
};

document.getElementById('guessBtn').onclick = function () {
    q = checkAnswer(current);
    attemptAns(q);

    if (!q.ansStatus) {
        document.getElementById('resultMsg').textContent = 'Please select an option.';
        document.getElementById('resultMsg').style.color = 'orange';
    } else if (q.ansStatus === 'CT') {
        document.getElementById('resultMsg').textContent = 'Correct!';
        document.getElementById('resultMsg').style.color = 'green';
        showExplainBtn();
    } else if (q.ansStatus === 'WG') {
    ...
    if (q.multi) {
        ...
        document.querySelectorAll('input[name="option"]').forEach(input => {
            if (correct.includes(input.value)) {
                input.parentElement.style.background = '#ffe066';
            }
        });
    } else {
...
// Show Explain button if correct or show answer was toggled
function maybeShowExplainBtn() {
    const q = questions[current];
    const explainBtnArea = document.getElementById('explainBtnArea');
    if (!explainBtnArea) return;
    // Only show if already correct or show answer was toggled
    if (q.ansStatus === 'CT' || q._showedAnswer) {
        showExplainBtn();
    } else {
        explainBtnArea.innerHTML = '';
        document.getElementById('explainArea').style.display = 'none';
    }
}

// Show the Explain button
function showExplainBtn() {
    const q = questions[current];
    const explainBtnArea = document.getElementById('explainBtnArea');
    if (!explainBtnArea) return;
    explainBtnArea.innerHTML = '<button id="explainBtn" style="margin-top:10px;">Explain</button>';
    document.getElementById('explainBtn').onclick = function () {
        showExplanation();
    };
}

// Show the explanation below options
function showExplanation() {
    const q = questions[current];
    const explainArea = document.getElementById('explainArea');
    if (!explainArea) return;
    explainArea.innerHTML = q.explanation ? `<div class="explain-box" style="background:#f8f8e0;padding:10px;border-radius:5px;border:1px solid #ccc;">${q.explanation}</div>` : '<div class="explain-box" style="color:#888;">No explanation available.</div>';
    explainArea.style.display = 'block';
}
document.getElementById('resetCurrentAnsBtn').onclick = function () {
    if (confirm('Are you sure you want to reset current?')) {
        let attemptedAnswers = JSON.parse(localStorage.getItem('attemptedAnswers')) || {};
        delete attemptedAnswers[current];
        localStorage.setItem('attemptedAnswers', JSON.stringify(attemptedAnswers));
...
document.getElementById('showAllAnsBtn').onclick = function () {
    ...
    const summaryDiv = document.getElementById('ans-summary');
    ...
    if (Object.keys(attemptedAnswers).length === 0) {
        ...
        msg.textContent = 'No answers recorded yet.';
        msg.style.marginTop = '6px';
        modal.appendChild(msg);
    } else {
        const list = document.createElement('ul');
        for (const qIndex in attemptedAnswers) {
            const li = document.createElement('li');
            const link = document.createElement('a');
            link.href = '#';
            link.style.textDecoration = 'none';
            link.textContent = `Question ${parseInt(qIndex) + 1}: ${Array.isArray(attemptedAnswers[qIndex].selected) ? attemptedAnswers[qIndex].selected.join(', ') : (attemptedAnswers[qIndex].selected || 'No answer')} (${attemptedAnswers[qIndex].timeTaken}s)`;
            link.onclick = e => {
                e.preventDefault();
                modal.style.display = 'none';
                renderQuestion(parseInt(qIndex));
            };
            li.appendChild(link);
            list.appendChild(li);
            linkRefs.push({ qIndex: parseInt(qIndex), el: link });
        }
    ...
    checkBtn.onclick = () => {
        let correctCount = 0;
        const attempted = linkRefs.length;
        linkRefs.forEach(ref => {
            const userAns = attemptedAnswers[ref.qIndex].selected;
            let isCorrect = checkAnswer(ref.qIndex, userAns).ansStatus === 'CT';

            if (isCorrect) {
                ref.el.style.color = 'green';
                correctCount++;
            } else {
                ref.el.style.color = 'red';
            }
        });
        summaryDiv.innerHTML = `Attempted: ${attempted}<br>Correct: ${correctCount}<br>Accuracy: ${attempted ? ((correctCount / attempted) * 100).toFixed(1) : '0.0'}%`;
    };
...
function attemptAns(guessedAns) {
    const q = questions[current];
    let selected = getSelectedAnswer(q);
    if (selected || (Array.isArray(selected) && selected.length > 0)) {
        let currentAnswers = JSON.parse(localStorage.getItem('attemptedAnswers')) || {};

        let ans = { id: q.id, question: q.question, selected: selected, timeTaken: questionElapsedSeconds };
        if (guessedAns) {
            ans.guessedTime = questionElapsedSeconds;
            ans = { ...ans, ...guessedAns };
        }

        currentAnswers[current] = { ...currentAnswers[current], ...ans };

        localStorage.setItem('attemptedAnswers', JSON.stringify(currentAnswers));
    }
}
function restoreSelectedAnswer() {
    const q = questions[current];
    let attemptedAnswers = JSON.parse(localStorage.getItem('attemptedAnswers')) || {};
    if (attemptedAnswers[current] && attemptedAnswers[current].selected) {
        const selected = attemptedAnswers[current].selected;
        if (q.multi) {
            // Multi-select
            document.querySelectorAll('input[name="option"]').forEach(input => {
...
if (questions.length > 0) {
    ...
    document.getElementById('questionArea').innerHTML = '<div class="error">No questions loaded.</div>';
    document.getElementById('prevBtn').disabled = true;
    document.getElementById('nextBtn').disabled = true;
    document.getElementById('guessBtn').disabled = true;
    document.getElementById('showBtn').disabled = true;
...
function getSelectedAnswer(q) {
    if (q.multi) {
        return Array.from(document.querySelectorAll('input[name="option"]:checked')).map(e => e.value);
    } else {
        const selected = document.querySelector('input[name="option"]:checked');
        return selected ? selected.value : null;
    }
}
function checkAnswer(id, selectedAnswer) {
    const q = questions.filter(item => item.id === id)[0];
    let ansStatus = '';
    let correctAns = '';
    let selected = selectedAnswer || getSelectedAnswer(q);

    if (!selected || selected.length === 0) {
        q.ansStatus = null;
        return {};
    }
    const correct = Array.isArray(q.answer) ? q.answer.slice().sort().join(',') : String(q.answer);
    let qq = {};

    if (q.multi)
        selected = selected.slice().sort().join(',');

    if (selected === correct) {
        qq.ansStatus = 'CT';
        qq.correctAns = selected;
    } else {
        qq.ansStatus = 'WG';
    }

    qq.timeTaken = questionElapsedSeconds;
